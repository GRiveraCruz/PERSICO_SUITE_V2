...
def api_backup_all():
    ...
    buf = _io.BytesIO()
    dbs = ["quotes","jobs","rates","wh","po","ivp","cpo","fx","pt","sv","stock","reassign","recovery"]
...
def api_create_stock():
    try:
        ...
        with lock:
            ...
            if existing:
                existing["quantity"]   = int(data.get("quantity", existing["quantity"]))
                existing["last_cost"]  = float(data.get("last_cost", existing["last_cost"]))
                existing["updated_at"] = datetime.datetime.now().isoformat()
                if data.get("section"): existing["section"] = str(data["section"]).strip()
                if data.get("box"):     existing["box"]     = str(data["box"]).strip()
                stock_save(records)
                ...
                rec = {
                    ...
                    "last_cost":    float(data.get("last_cost",0)),
                ...
                records.append(rec)
                stock_save(records)
                # Auto-create recovery entry (negative cost)
                recovery_job = str(data.get("recovery_job","")).strip()
                if recovery_job:
                    qty   = int(data.get("quantity",0))
                    cost  = float(data.get("last_cost",0))
                    total = round(qty * cost * -1, 2)  # negative
                    rec_recovery = {
...
def api_update_stock(item_id):
    try:
        ...
        with lock:
            ...
            for k in ["manufacturer","part_number","description","unit","section","box","recovery_job"]:
                if k in data: rec[k] = str(data[k]).strip()
