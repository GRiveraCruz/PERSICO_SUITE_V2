...
@app.route("/api/po/<int:year>/<int:clave>", methods=["DELETE"])
...
def api_delete_gpo(po_number):
    ...
    try:
        with lock:
            # Remove from GPO list
            records = gpo_load()
            new = [r for r in records if r.get("po_number","") != po_number.upper()]
            if len(new) == len(records):
                return jsonify({"error": "PO no encontrada"}), 404
            gpo_save(new)
            # Remove all related IPO items for current year
            year = datetime.datetime.now().year
            ipo = po_load(year)
            ipo_new = [r for r in ipo if r.get("gpo_number","") != po_number.upper()]
...
<script>
  JsBarcode("#barcode", "{rec['po_number']}", {{
    format: "CODE128",
    width: 1.6,
    height: 40,
    displayValue: true,
