...
if __name__ == "__main__":
    # Scheduler already started by _ensure_fx_scheduler() above
    print("=" * 60)
...
# Arrancar scheduler una sola vez (compatible con Gunicorn multi-worker)
_fx_sched_started = False
def _ensure_fx_scheduler():
    global _fx_sched_started
    if not _fx_sched_started:
        _fx_sched_started = True
        import threading as _thr
        _thr.Thread(target=_fx_scheduler, daemon=True, name="fx-scheduler").start()
        print("[FX-SCHED] Thread iniciado")

_ensure_fx_scheduler()
