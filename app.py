...
def api_gpo_search():
    """Find a single GPO by partial number (suffix match)."""
    try:
        q = str(request.args.get("q","")).strip().lstrip("0")
        if not q: return jsonify({"error":"Parámetro q requerido"}), 400
        records = gpo_load()
        # Match by number ending (e.g. "17454" matches "PO-000017454")
        match = next((r for r in records
            if str(r.get("po_number","")).lstrip("PO-").lstrip("0") == q or
               str(r.get("po_number","")).endswith(q)), None)
        if not match:
            return jsonify({"error": f"No se encontró la orden {q}"}), 404
        return jsonify({"record": match})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_gpo_split(po_number):
    ...
    try:
        ...
        with lock:
            gpos = gpo_load()
            rec  = next((r for r in gpos if r.get("po_number","").upper()==po_upper), None)
            if not rec:
                return jsonify({"error": f"PO {po_number} no encontrada"}), 404
            # Determine year from creation date
            try: yr = int(rec.get("created_at","")[:4])
            except: yr = CURRENT_YEAR
            moneda  = rec.get("moneda","USD")
            fx_rate = float(rec.get("fx_rate",1) or 1)
            now     = datetime.datetime.now().isoformat()
            supplier= rec.get("supplier_name","")
            # ── Create one IPO record per item
            ipo_records = po_load(yr)
            new_entries = []
            for i, it in enumerate(items):
                ...
                entry = {
                    "clave":                po_upper,
                    "fecha_doc":            rec.get("created_at","")[:10] or now[:10],
                    "entregar_a":           job,
                    "nombre":               supplier,
                    "subtotal":             total,
                    "moneda":               moneda,
            ...
            po_save(yr, ipo_records)
            # ── Zero original IPO records
            _gpo_update_ipos(po_upper, {"subtotal": 0.0, "subtotal_mxn": 0.0,
                                         "estatus": "Dividida", "_split_zeroed": True})
            # ── Mark GPO as split
            rec["modificacion_tipo"] = "Dividida"
            rec["effective_total"]   = 0.0
            rec["modificacion_at"]   = now
            rec["modificacion_by"]   = session.get("user","")
            gpo_save(gpos)
