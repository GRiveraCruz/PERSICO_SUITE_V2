...
def _gpo_update_ipos(po_number, fields: dict):
    ...
    for year in range(CURRENT_YEAR - 2, CURRENT_YEAR + 2):
        ...
        changed = False
        for r in records:
            if str(r.get("gpo_number","")).upper() == str(po_number).upper() or \
               str(r.get("clave","")).upper() == str(po_number).upper():
                r.update(fields)
                changed = True
        if changed:
    ...
    try:
        ...
        with lock:
            ...
            if tipo == "cancelar":
                ...
                val_recibido = float(data.get("valor_recibido", 0) or 0)
                rec["status"]              = "Cierre Anticipado"
                rec["effective_total"]     = round(val_recibido, 2)
                rec["effective_total_usd"] = round(val_recibido, 2)
                rec["modificacion_tipo"]   = "Cierre Anticipado"
                rec["modificacion_by"]     = user
                rec["modificacion_at"]     = now
                rec["modificacion_nota"]   = str(data.get("nota","")).strip()
                # IPO records already reflect received amounts — just update status
                _gpo_update_ipos(po_number, {"estatus": "Cierre Anticipado"})
...
def api_gpo_lookup(po_number):
    """Returns a single GPO record by PO number for the modification modal."""
    try:
        records = gpo_load()
        rec = next((r for r in records
            if str(r.get("po_number","")).upper() == po_number.upper()), None)
