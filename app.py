...
_SEED_DONE = _os.path.join(_DATA, ".seed_done")
if _os.path.isdir(_SEED) and not _os.path.exists(_SEED_DONE):
    import shutil as _shutil
    print(f"[INIT] Volumen vacío — copiando datos iniciales: data_seed/ → {_DATA}")
    _shutil.copytree(_SEED, _DATA, dirs_exist_ok=True)
    # Marca permanente para no volver a hacer seed
    open(_SEED_DONE, "w").close()
    print("[INIT] Seed OK")
