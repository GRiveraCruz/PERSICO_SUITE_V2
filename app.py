...
@app.route("/api/apartados/<part_number>", methods=["DELETE"])
...
# ══════════════════════════════════════════════════════════════════
#  RESPALDO COMPLETO + ENVÍO POR CORREO
# ══════════════════════════════════════════════════════════════════
BACKUP_CFG_FILE = _os.path.join(_DATA, "backup_config.json")

def backup_cfg_load():
    p = Path(BACKUP_CFG_FILE)
    if p.exists():
        try:
            with open(p,"r",encoding="utf-8") as f: return json.load(f)
        except: pass
    return {"recipients": ["guillermo.rivera@persico.com"]}

def backup_cfg_save(cfg):
    Path(BACKUP_CFG_FILE).parent.mkdir(parents=True, exist_ok=True)
...
def _send_backup_email(zip_bytes, zip_name, recipients):
    """Send backup ZIP via SMTP."""
    import smtplib
    ...
    if not all([host, user, pw]):
        return False, "SMTP no configurado (faltan SMTP_HOST, SMTP_USER, SMTP_PASS en Railway)"

    msg = MIMEMultipart()
    msg["From"]    = user
    msg["To"]      = ", ".join(recipients)
    msg["Subject"] = f"[Persico Suite] Respaldo de Datos — {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}"
    msg.attach(MIMEText(
        f"Respaldo automático adjunto.\n\nFecha: {datetime.datetime.now().strftime('%d/%m/%Y %H:%M')}\n"
        f"Archivos: Excel + JSON raw por módulo.\n\n— Persico Suite", "plain"))
...
def api_admin_backup():
    """Generate backup ZIP, return for download, and email to recipients."""
    if not is_admin(): return jsonify({"error":"Sin permiso"}), 403
    try:
        ...
        email_ok, email_msg = False, "Sin destinatarios"
        if recipients:
            email_ok, email_msg = _send_backup_email(zip_bytes, zip_name, recipients)
        import base64 as _b64
        return jsonify({
            "ok": True,
...
def api_backup_cfg_get():
    if not is_admin(): return jsonify({"error":"Sin permiso"}), 403
    return jsonify(backup_cfg_load())

@app.route("/api/admin/backup-config", methods=["POST"])
def api_backup_cfg_set():
    if not is_admin(): return jsonify({"error":"Sin permiso"}), 403
    try:
        data = request.get_json()
        cfg = backup_cfg_load()
        if "recipients" in data:
            cfg["recipients"] = [str(e).strip().lower() for e in data["recipients"] if "@" in str(e)]
        backup_cfg_save(cfg)
        return jsonify({"ok": True, "config": cfg})
    except Exception as e:
        return jsonify({"error": str(e)}), 500

# ── Daily backup scheduler (23:50 Mexico City time)
def _backup_scheduler():
    import zoneinfo, time as _t
    TZ_MX = zoneinfo.ZoneInfo("America/Mexico_City")
    print("[BACKUP-SCHED] Scheduler de respaldo iniciado")
    while True:
        now = datetime.datetime.now(TZ_MX)
        target = now.replace(hour=23, minute=50, second=0, microsecond=0)
        if now >= target: target += datetime.timedelta(days=1)
        secs = (target - now).total_seconds()
        print(f"[BACKUP-SCHED] Próximo respaldo: {target.strftime('%Y-%m-%d %H:%M')} MX (en {int(secs//3600)}h {int((secs%3600)//60)}m)")
        _t.sleep(secs)
        try:
            zip_bytes, zip_name = _build_backup_zip()
            cfg = backup_cfg_load()
            recipients = cfg.get("recipients", [])
            if recipients:
                ok, msg = _send_backup_email(zip_bytes, zip_name, recipients)
                print(f"[BACKUP-SCHED] Email {zip_name}: {msg}")
            else:
                print("[BACKUP-SCHED] Sin destinatarios configurados")
        except Exception as e:
            print(f"[BACKUP-SCHED] Error: {e}")
        _t.sleep(90)

_backup_sched_started = False
def _ensure_backup_scheduler():
    global _backup_sched_started
    if not _backup_sched_started:
        _backup_sched_started = True
        import threading as _thr2
        _thr2.Thread(target=_backup_scheduler, daemon=True, name="backup-scheduler").start()
        print("[BACKUP-SCHED] Thread iniciado")
