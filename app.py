    ...
    try:
        ...
        if q:
            orders = [o for o in orders if
                ...
                any(q in str(i.get("part_number","")).lower() or
                    q in str(i.get("job","")).lower()
                    for i in o.get("items",[]))]
        return jsonify({"records": orders, "orders": orders,
                        "total": len(orders), "next_number": reassign_next_number()})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
