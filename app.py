...
@app.route("/api/util/pdf-to-po-excel", methods=["POST"])
def api_pdf_to_po_excel():
    ...
    try:
        ...
        job_clean = re.sub(r'\s+(MF|SF|[A-Z]{1,3})$', '', job_raw.strip(), flags=re.IGNORECASE).strip()
        supplier  = find_field(r"PROVEEDOR\s*\(\s*\d+\s*\)\s*\n?([\w\s\.,]+?)(?:\n|RFC|AVENIDA|$)", full_text)

        # ── Parse line items
        # Pattern: line starts with digit (partida), then qty, part_num, desc, cost, importe
        items = []
        lines = full_text.split('\n')

        # Find the items section (between "PARTIDA" header and "SUBTOTAL")
        in_items = False
        item_buffer = {}
        desc_extra = []
        ...
        for line in lines:
            ...
            if re.match(r'PARTIDA\s+CANTIDAD', line, re.IGNORECASE):
                in_items = True
                continue
            if re.match(r'SUBTOTAL|DESCUENTO|TOTAL|IVA', line, re.IGNORECASE) and in_items:
                if item_buffer:
            ...
            # New item row: starts with digit, then qty
            ...
            if m:
                ...
                # The description and part_number often overlap — take full middle part
                middle = m.group(3).strip()
                # Try splitting by cost: everything before cost is desc+part
                # Part number extraction: look for pattern like "5257 D1050" in following line
                item_buffer = {
                    "part_number": "",      # filled from next DIBUJO line
                    "description": middle,
            ...
            # Continuation line — look for DIBUJO: XXXX for part number
            if item_buffer:
                dibujo = re.search(r'DIBUJO:\s*([\w\s\d]+?)(?:\*|$)', line, re.IGNORECASE)
                if dibujo:
                    raw_pn = dibujo.group(1).strip()
                    item_buffer["part_number"] = raw_pn.replace(' ', '-')
                    # Add any extra description text
                    rest = line[dibujo.end():].strip().strip('*').strip()
                    if rest: desc_extra.append(rest)
                else:
                    # Regular continuation line — append to description if meaningful
