...
def api_gpo_split(po_number):
    ...
    try:
        ...
        with lock:
            ...
            # ── Determine the correct year for IPO records ──────────
            ipo_yr = CURRENT_YEAR
            po_stripped = po_upper.lstrip("PO-").lstrip("0")
            year_found = False
            for search_yr in range(CURRENT_YEAR + 1, CURRENT_YEAR - 4, -1):
                for r in po_load(search_yr):
                    ...
                    c_strip = clave_r.lstrip("PO-").lstrip("0")
                    if clave_r == po_upper or c_strip == po_stripped:
                        ipo_yr = search_yr
                        year_found = True
                        break
                if year_found:
