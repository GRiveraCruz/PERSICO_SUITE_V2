...
# Lanzar scheduler de tipo de cambio (funciona con Gunicorn y con python directo)
def _start_fx_scheduler():
    import threading as _thr
    t = _thr.Thread(target=lambda: None, daemon=True, name="fx-scheduler-init")
    # Defined later in the file — use a late-binding lambda
    def _run():
        import time as _t2
        _t2.sleep(5)  # Espera a que el resto del módulo cargue
        _fx_scheduler()
    t2 = _thr.Thread(target=_run, daemon=True, name="fx-scheduler")
    t2.start()
...
def _build_report_data(job_number, rate_year, wh_year, po_year):
    ...
    svc_via  = round(sum(r.get("valor_usd",0) for r in _svc_load(VIATICOS_FILE) if r.get("job","").upper()==job_number.upper()), 4)
    svc_gv   = round(sum(r.get("valor_usd",0) for r in _svc_load(GASTOS_FILE)   if r.get("job","").upper()==job_number.upper()), 4)
    svc_env  = round(sum(r.get("valor_usd",0) for r in _svc_load(ENVIOS_FILE)   if r.get("job","").upper()==job_number.upper()), 4)
...
def api_create_ingreso():
    ...
    try:
        ...
        with lock:
            ...
            ing_items = []
            for it in items:
                qty_ord = float(it.get("quantity_ordered", it.get("quantity_delivered",1)))
                qty_del = float(it.get("quantity_delivered", 0))
            ...
            rec = {
                "id":         f"ING-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                "tipo":       tipo,
                "po_number":  po_num,
                "recibe":     recibe,
                "items":      ing_items,
            ...
            for it in ing_items:
                if it["quantity_delivered"] <= 0: continue
                pnum = it["part_number"]
                job  = it["job"]
...
def api_create_salida():
    """Registra una salida PENDIENTE (no descuenta aún de apartados)."""
    try:
        data       = request.get_json()
        ...
        with lock:
            ...
            rec = {
                "id":           f"SAL-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                "job":          job,
...
def api_surtir_salida(sal_id):
    """Ejecuta la salida: descuenta de apartados y marca como Surtida."""
    try:
        with lock:
            records = salida_load()
            rec = next((r for r in records if r["id"] == sal_id), None)
            if not rec: return jsonify({"error":"Salida no encontrada"}), 404
            if rec.get("status") == "Surtida":
                return jsonify({"error":"Esta salida ya fue surtida"}), 400
            now = datetime.datetime.now().isoformat()
            # Deduct from apartados
            apartados = apartado_load()
            for it in rec.get("items",[]):
                pnum = it["part_number"].upper()
                job  = rec["job"].upper()
                qty  = float(it.get("quantity",0))
                apt  = next((a for a in apartados
                    if a.get("part_number","").upper() == pnum), None)
                if apt:
                    job_entry = next((j for j in apt.get("jobs",[])
                        if j.get("job","").upper() == job), None)
                    if job_entry:
                        job_entry["quantity"] = max(0, float(job_entry.get("quantity",0)) - qty)
                        # Remove job entry if quantity reaches 0
                        if job_entry["quantity"] <= 0:
                            apt["jobs"] = [j for j in apt.get("jobs",[])
                                          if j.get("job","").upper() != job]
                    # Recalc total (keep record even if all jobs are 0)
                    apt["total_quantity"] = max(0, sum(
                        float(j.get("quantity",0)) for j in apt.get("jobs",[])))
                    apt["updated_at"] = now
            apartado_save(apartados)
            # Mark salida as Surtida
            rec["status"]     = "Surtida"
            rec["surtido_at"] = now
            rec["surtido_by"] = session.get("user","")
            salida_save(records)
        return jsonify({"ok": True, "record": rec})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_delete_salida(sal_id):
    ...
    try:
        with lock:
            records = salida_load()
            new = [r for r in records if r["id"] != sal_id]
            if len(new) == len(records):
...
def api_salida_pdf(sal_id):
    try:
        ...
        if _os.path.exists(logo_path):
            ...
            with open(logo_path,"rb") as lf:
                logo_b64 = _b64.b64encode(lf.read()).decode()
        logo_tag = f'<img src="data:image/webp;base64,{logo_b64}" style="height:48px">' \
                   if logo_b64 else '<span style="font-size:22px;font-weight:900;color:#c8102e">PERSICO</span>'

        fmt = lambda v: f"${float(v):,.2f}"
        items = rec.get("items",[])
        subtotal = sum(float(it.get("total", float(it.get("quantity",0))*float(it.get("unit_cost",0)))) for it in items)
        ...
        status_color = "#48c78e" if rec.get("status")=="Surtida" else "#ffdd57"
        surtido_info = ""
        if rec.get("status") == "Surtida":
            surtido_info = f"""
            <div style="display:flex;gap:24px;padding:8px 14px;background:#1f3864;color:#fff;border-radius:6px;margin-bottom:14px;font-size:11px">
              <div><span style="opacity:.7">Surtido por</span> <b>{rec.get('surtido_by','—')}</b></div>
              <div><span style="opacity:.7">Fecha de surtido</span> <b>{(rec.get('surtido_at','') or '')[:10]}</b></div>
            </div>"""

        html = f"""<!DOCTYPE html>
<html lang="es"><head><meta charset="UTF-8">
<title>Salida {rec['id']}</title>
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  body{{font-family:Arial,sans-serif;font-size:11px;color:#111;padding:28px 32px}}
  .header{{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:18px;border-bottom:3px solid #c8102e;padding-bottom:14px}}
  .doc-num{{font-size:22px;font-weight:900;color:#c8102e;letter-spacing:1px;text-align:right}}
  .doc-label{{font-size:10px;color:#888;text-transform:uppercase;letter-spacing:1px;text-align:right}}
  .company-block{{font-size:10px;line-height:1.6;color:#444}}
  .company-name{{font-size:13px;font-weight:700;color:#111;margin-bottom:2px}}
  .section-title{{font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#888;margin-bottom:6px;margin-top:14px}}
  .info-grid{{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px 20px;margin-bottom:14px;padding:10px 14px;background:#f7f7f7;border-radius:6px;border:1px solid #e8e8e8}}
  .info-field{{font-size:10px}}.info-field b{{color:#444;display:block;font-size:9px;text-transform:uppercase}}
  table{{width:100%;border-collapse:collapse;margin-bottom:12px;font-size:10px}}
  thead th{{background:#1f3864;color:#fff;padding:7px 8px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:.8px;white-space:nowrap}}
  tbody tr:nth-child(even){{background:#f7f7f7}}
  tbody td{{padding:6px 8px;border-bottom:1px solid #ebebeb;vertical-align:top}}
  .total-row{{display:flex;justify-content:flex-end;gap:20px;margin-top:4px;padding:10px 14px;background:#1f3864;color:#fff;border-radius:6px;font-size:13px;font-weight:700}}
  .status-badge{{display:inline-block;padding:3px 10px;border-radius:4px;font-size:11px;font-weight:700;background:{status_color}20;color:{status_color};border:1px solid {status_color}}}
  .sig-grid{{display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-top:40px}}
  .sig-box{{text-align:center}}
  .sig-line{{border-top:1px solid #aaa;margin-top:50px;padding-top:6px;font-size:10px;color:#555}}
  .footer{{margin-top:24px;border-top:1px solid #ddd;padding-top:10px;font-size:9px;color:#aaa;display:flex;justify-content:space-between}}
  @media print{{body{{padding:10px 14px}}}}
</style></head>
<body>
<div class="header">
  ...
  <div>
    <div class="doc-label">Salida de Material</div>
    <div class="doc-num">{rec['id']}</div>
    <div style="font-size:10px;color:#888;text-align:right;margin-top:4px">{rec.get('fecha','')}</div>
    <div style="text-align:right;margin-top:6px"><span class="status-badge">{rec.get('status','—')}</span></div>
  </div>
</div>

<div class="section-title">Información de la Salida</div>
<div class="info-grid">
  <div class="info-field"><b>Job Destino</b>{rec.get('job','—')}</div>
  <div class="info-field"><b>Solicitante</b>{rec.get('solicitante','—')}</div>
  <div class="info-field"><b>Fecha de Solicitud</b>{rec.get('fecha','—')}</div>
</div>

{surtido_info}

<div class="section-title">Materiales Despachados</div>
<table>
  <thead><tr>
    <th>#</th><th>Código</th><th>No. Parte</th><th>Marca</th>
    <th>Descripción</th><th>Etiqueta</th>
...
<div class="sig-grid">
  <div class="sig-box">
    <div class="sig-line">
      <strong>{rec.get('solicitante','—')}</strong><br>Solicitante
    </div>
  </div>
  <div class="sig-box">
    <div class="sig-line">
      <strong>{rec.get('surtido_by','—') if rec.get('status')=='Surtida' else '____________________'}</strong><br>
      {'Surtido por' if rec.get('status')=='Surtida' else 'Recibido por / Almacenista'}
    </div>
  </div>
</div>
...
<div class="footer">
  <span>Generado por Persico Suite · {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}</span>
  <span>{rec['id']} · Job {rec.get('job','—')}</span>
</div>
</body></html>"""
...
def api_mover_a_stock():
    """
    Mueve items de Apartados a Stock en un solo paso.
    Body: { job, solicitante, items:[{part_number, brand, description,
            cat_code, label_code, quantity, unit_cost}] }
    """
    try:
        data       = request.get_json()
        job        = str(data.get("job","")).strip().upper()
        ...
        with lock:
            ...
            for it in items:
                ...
                mov_items.append({
                    ...
                    "total":       round(qty * float(it.get("unit_cost",0) or 0), 2),
                })
            if not mov_items:
                return jsonify({"error":"Ningún item con cantidad > 0"}), 400
            ...
            for it in mov_items:
                ...
                existing = next((r for r in stock_records
                    if r.get("part_number","").upper() == pnum and
                       r.get("manufacturer","").upper() == mfr), None)
                if existing:
                    existing["quantity"]   = float(existing.get("quantity",0)) + it["quantity"]
                    existing["last_cost"]  = it["unit_cost"] if it["unit_cost"] > 0 else existing.get("last_cost",0)
            ...
            # ── Registrar movimiento PRIMERO (para tener el ID)
            rec = {
                "id":         f"MOV-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                "job":        job,
                "solicitante":solicitante,
                "items":      mov_items,
                "fecha":      now[:10],
            ...
            movimientos = movimiento_load()
            movimientos.append(rec)
            movimiento_save(movimientos)

            # ── Crear registros de Recuperación (resta costo al job)
            recoveries = recovery_load()
            for it in mov_items:
                qty  = it["quantity"]
                uc   = it["unit_cost"]
                total_value = round(qty * uc, 2)
                stk_rec = next((r for r in stock_records
                ...
                recoveries.append({
                    ...
                    "stock_id":     stk_rec["id"] if stk_rec else "",
                    "movimiento_id":rec["id"],
                    "created_at":   now,
                })
            recovery_save(recoveries)

        return jsonify({"ok": True, "record": rec,
                        "items_moved": len(mov_items)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
    ...
    try:
        ...
        for i, it in enumerate(items, 1):
            ...
            <tr>
              ...
              <td>{it.get('brand','—')}</td>
              <td>{it.get('description','—')}</td>
              <td style="text-align:right">{int(it.get('quantity',0))}</td>
...
<style>
  *{{box-sizing:border-box;margin:0;padding:0}}
  body{{font-family:Arial,sans-serif;font-size:11px;color:#111;padding:28px 32px}}
  .header{{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:18px;border-bottom:3px solid #f5a623;padding-bottom:14px}}
  .doc-num{{font-size:20px;font-weight:900;color:#f5a623;letter-spacing:1px;text-align:right}}
  .doc-label{{font-size:10px;color:#888;text-transform:uppercase;letter-spacing:1px;text-align:right}}
  .company-block{{font-size:10px;line-height:1.6;color:#444}}
  .company-name{{font-size:13px;font-weight:700;color:#111;margin-bottom:2px}}
  .info-grid{{display:grid;grid-template-columns:1fr 1fr;gap:10px 20px;margin:14px 0;padding:10px 14px;background:#f7f7f7;border-radius:6px;border:1px solid #e8e8e8}}
  .info-field{{font-size:10px}}.info-field b{{color:#444;display:block;font-size:9px;text-transform:uppercase}}
  table{{width:100%;border-collapse:collapse;margin-bottom:12px;font-size:10px}}
  thead th{{background:#1f3864;color:#fff;padding:7px 8px;text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:.8px;white-space:nowrap}}
  tbody tr:nth-child(even){{background:#f7f7f7}}
  tbody td{{padding:6px 8px;border-bottom:1px solid #ebebeb;vertical-align:top}}
  .total-row{{display:flex;justify-content:flex-end;gap:20px;padding:10px 14px;background:#1f3864;color:#fff;border-radius:6px;font-size:13px;font-weight:700;margin-bottom:28px}}
...
<div class="header">
  <div>
    ...
    <div class="company-block" style="margin-top:8px">
      <div class="company-name">PERSICO MÉXICO S.A. DE C.V.</div>
      RFC: PME200101AB1<br>Blvd. Forjadores de Puebla 3120, Puebla, Pue.
    </div>
  </div>
  <div>
    <div class="doc-label">Movimiento a Stock</div>
    <div class="doc-num">{rec['id']}</div>
    <div style="font-size:10px;color:#888;text-align:right;margin-top:4px">{rec.get('fecha','')}</div>
    <div style="text-align:right;margin-top:6px">
      <span style="font-size:11px;font-weight:700;background:#f5a62320;color:#f5a623;padding:3px 10px;border-radius:4px;border:1px solid #f5a623">De Apartados → Stock</span>
    </div>
  </div>
</div>

<div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#888;margin-bottom:6px">Información del Movimiento</div>
<div class="info-grid">
  <div class="info-field"><b>Job Origen</b>{rec.get('job','—')}</div>
  <div class="info-field"><b>Solicitante</b>{rec.get('solicitante','—')}</div>
  <div class="info-field"><b>Fecha</b>{rec.get('fecha','—')}</div>
  <div class="info-field"><b>Total Items</b>{len(items)}</div>
</div>

<div style="font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:#888;margin-bottom:6px">Materiales Movidos a Stock</div>
<table>
  <thead><tr>
    <th>#</th><th>Código</th><th>No. Parte</th><th>Marca</th>
    <th>Descripción</th>
...
<div class="total-row">
  <span>TOTAL USD</span><span>{fmt(subtotal)}</span>
</div>

<div class="sig-grid">
  <div class="sig-box">
    <div class="sig-line">
      <strong>{rec.get('solicitante','—')}</strong><br>Solicitante / Autoriza
    </div>
  </div>
  <div class="sig-box">
    <div class="sig-line">
      ____________________<br>Almacenista / Recibe en Stock
    </div>
  </div>
</div>
...
# ══════════════════════════════════════════════════════════════════
#  SCHEDULER — Tipo de cambio automático (23:59 hora México)
# ══════════════════════════════════════════════════════════════════
import threading as _threading
import time as _time

def _banxico_fetch_and_save():
    """Llama a Banxico y guarda el tipo de cambio del día."""
    import urllib.request, json as _json2
    try:
        token = _os.environ.get("BANXICO_TOKEN", "")
        url   = "https://www.banxico.org.mx/SieAPIRest/service/v1/series/SF43718/datos/oportuno"
        hdrs  = {"Bmx-Token": token} if token else {}
        req   = urllib.request.Request(url, headers=hdrs)
        with urllib.request.urlopen(req, timeout=10) as resp:
            raw = _json2.loads(resp.read().decode("utf-8"))
        datos = raw.get("bmx",{}).get("series",[{}])[0].get("datos",[])
        if not datos: raise ValueError("Sin datos")
        entry   = datos[-1]
        fecha_mx= entry.get("fecha","")
        dato    = entry.get("dato","N/E")
        if dato in ("N/E","N/D",""): raise ValueError(f"N/E para {fecha_mx}")
        rate    = round(float(dato), 6)
        d       = datetime.datetime.strptime(fecha_mx, "%d/%m/%Y")
        iso_key = d.strftime("%Y-%m-%d")
        year    = d.year
        with lock:
            ...
            existing[iso_key] = rate
            fx_save(year, existing)
        print(f"[FX-AUTO] {iso_key} → {rate} MXN/USD ✓")
    except Exception as e:
        print(f"[FX-AUTO] Error: {e}")

def _fx_scheduler():
    """Hilo que espera hasta las 23:59 hora de México y ejecuta el fetch."""
    import zoneinfo
    TZ_MX = zoneinfo.ZoneInfo("America/Mexico_City")
    print("[FX-SCHED] Scheduler de tipo de cambio iniciado")
    while True:
        now_mx = datetime.datetime.now(TZ_MX)
        # Target: 23:59:00 del día actual
        target = now_mx.replace(hour=23, minute=59, second=0, microsecond=0)
        if now_mx >= target:
            # Already past today's target — schedule for tomorrow
            target += datetime.timedelta(days=1)
        secs = (target - now_mx).total_seconds()
        print(f"[FX-SCHED] Próxima ejecución: {target.strftime('%Y-%m-%d %H:%M')} MX "
              f"(en {int(secs//3600)}h {int((secs%3600)//60)}m)")
        _time.sleep(secs)
        _banxico_fetch_and_save()
        _time.sleep(60)  # evitar doble ejecución al cruzar el minuto


# ══════════════════════════════════════════════════════════════════
#  SERVICIO — VIÁTICOS / GASTOS DE VIAJE / ENVÍOS DE MENSAJERÍA
# ══════════════════════════════════════════════════════════════════
...
def _get_fx_for_date(date_str):
    """Get the closest available FX rate for a given date."""
    try:
        d = datetime.datetime.strptime(date_str[:10], "%Y-%m-%d")
        year = d.year
        fx = fx_load(year)
        if date_str[:10] in fx:
            return fx[date_str[:10]]
        # Find closest previous date
        all_dates = sorted(fx.keys(), reverse=True)
        for fd in all_dates:
            if fd <= date_str[:10]:
                return fx[fd]
        # Try adjacent years
    ...
    try:
        ...
        with lock:
            ...
            for d in rows:
                ...
                fx     = float(d.get("tipo_cambio",0) or 0) or _get_fx_for_date(fecha) or 1.0
                usd    = round(monto / fx, 4) if fx else monto
                rec = {
...
def api_delete_viatico(vid):
    ...
    try:
        with lock:
            ...
            new  = [r for r in recs if r.get("id","") != vid]
            _svc_save(VIATICOS_FILE, new)
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def api_import_viaticos():
    try:
        ...
        def ci(name): return next((i for i,h in enumerate(hdr) if name in h), None)
        ci_id   = ci('id')
        ci_fecha= ci('fecha')
        ci_tipo = ci('tipo')
        ci_monto= ci('monto')
        with lock:
            ...
            for row in rows_data[1:]:
                ...
                fecha = str(fecha_raw)[:10] if hasattr(fecha_raw,'isoformat') else str(fecha_raw).strip()[:10]
                try: fecha = datetime.datetime.strptime(str(fecha_raw).split(' ')[0], "%Y-%m-%d").strftime("%Y-%m-%d")
                except:
                    try: fecha = datetime.datetime.strptime(str(fecha_raw), "%d/%m/%Y").strftime("%Y-%m-%d")
                    except: continue
                monto = float(cv(ci_monto) or 0)
                fx    = _get_fx_for_date(fecha) or 1.0
                usd   = round(monto / fx, 4)
                records.append({
                    "id":              f"VIA-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
...
# ── GASTOS DE VIAJE
GASTO_TIPOS = ["Vuelo","Autobús","Hospedaje","Cambio de Vuelo"]
...
def api_create_gasto():
    try:
        ...
        with lock:
            ...
            for d in rows:
                fecha  = str(d.get("fecha","")).strip()[:10]
                moneda = str(d.get("moneda","USD")).upper()
                costo  = float(d.get("costo",0) or 0)
                fx     = float(d.get("tipo_cambio",0) or 0) or (_get_fx_for_date(fecha) if moneda=="MXN" else 1.0) or 1.0
                usd    = round(costo / fx, 4) if moneda=="MXN" else costo
                rec = {
                    "id":          f"GV-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                    "tipo_gasto":  str(d.get("tipo_gasto","")).strip(),
                    "fecha":       fecha,
                    "moneda":      moneda,
                    "tipo_cambio": fx,
                    "costo":       costo,
...
UPLOAD_FOLDER = _os.path.join(_DATA, "envio_pods")
...
def api_create_envio():
    try:
        ...
        with lock:
            ...
            fx      = float(data.get("tipo_cambio",0) or 0) or (_get_fx_for_date(fecha) if moneda=="MXN" else 1.0) or 1.0
            usd     = round(costo / fx, 4) if moneda=="MXN" else costo
            rec = {
                "id":          f"ENV-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                "fecha":       fecha,
...
def api_upload_pod(eid):
    """Upload POD (Proof of Delivery) file for a shipment."""
    try:
        f = request.files.get("file")
        if not f: return jsonify({"error":"No se recibió archivo"}), 400
        ...
        filename = f"{eid}{ext}"
        filepath = _os.path.join(UPLOAD_FOLDER, filename)
        f.save(filepath)
        with lock:
            records = _svc_load(ENVIOS_FILE)
            for r in records:
...
def api_view_pod(eid):
    try:
        ...
        if not rec or not rec.get("pod_file"):
            return jsonify({"error":"Sin POD"}), 404
        filepath = _os.path.join(UPLOAD_FOLDER, rec["pod_file"])
...
# ── Service costs aggregated by job (for reports)
@app.route("/api/servicio/por-job/<job_number>")
def api_servicio_por_job(job_number):
    """Returns total service costs (USD) for a job."""
    try:
        job = job_number.upper()
        via = sum(r.get("valor_usd",0) for r in _svc_load(VIATICOS_FILE)    if r.get("job","").upper()==job)
        gas = sum(r.get("valor_usd",0) for r in _svc_load(GASTOS_FILE)     if r.get("job","").upper()==job)
        env = sum(r.get("valor_usd",0) for r in _svc_load(ENVIOS_FILE)     if r.get("job","").upper()==job)
        return jsonify({"job": job, "viaticos_usd": round(via,4),
                        "gastos_usd": round(gas,4), "envios_usd": round(env,4),
                        "total_usd": round(via+gas+env,4)})
    except Exception as e:
        return jsonify({"error": str(e)}), 500


    # Lanzar scheduler en hilo daemon (se detiene si el proceso principal muere)
    _t = _threading.Thread(target=_fx_scheduler, daemon=True, name="fx-scheduler")
    _t.start()
    print("=" * 60)
    print("  Persico Mex — Suite Unificada")
    print(f"  Job Register    : {JOBS_FOLDER}")
