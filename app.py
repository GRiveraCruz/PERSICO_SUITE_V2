...
def api_gpo_split(po_number):
    ...
    try:
        ...
        with lock:
            ...
            # Determine year — from GPO or from search hint
            try:
                yr = int((gpo_rec or {}).get("created_at","")[:4]) or CURRENT_YEAR
            except: yr = CURRENT_YEAR
            # Also search IPO years for the clave
            ipo_yr = CURRENT_YEAR
            for search_yr in range(CURRENT_YEAR - 2, CURRENT_YEAR + 2):
                for r in po_load(search_yr):
                    clave = str(r.get("clave","") or r.get("gpo_number","")).upper()
                    if clave == po_upper or clave.lstrip("PO-").lstrip("0") == po_upper.lstrip("PO-").lstrip("0"):
                        ipo_yr = search_yr; break

            moneda   = str(data.get("moneda","USD")).upper()
            fx_rate  = float(data.get("tipo_cambio", (gpo_rec or {}).get("fx_rate",1)) or 1)
            if moneda == "MXN" and fx_rate <= 1: fx_rate = 1.0
            supplier = (gpo_rec or {}).get("supplier_name", data.get("supplier_name",""))
            now      = datetime.datetime.now().isoformat()
            fecha_doc= (gpo_rec or {}).get("created_at","")[:10] or now[:10]

            # ── Step 1: Zero the original IPO records FIRST (before adding new ones)
            ipo_records = po_load(ipo_yr)
            for r in ipo_records:
                clave_r = str(r.get("clave","") or r.get("gpo_number","")).upper()
                if (clave_r == po_upper or
                    clave_r.lstrip("PO-").lstrip("0") == po_upper.lstrip("PO-").lstrip("0")) \
            ...
            # ── Step 2: Create new IPO records per item and append
            new_entries = []
            for i, it in enumerate(items):
                qty   = float(it.get("quantity", 0) or 0)
                price = float(it.get("unit_cost", it.get("unit_price", 0)) or 0)
                total    = round(qty * price, 2)
                # Convert to USD if MXN
                ...
                entry = {
                    ...
                    "entregar_a":           job,
                    "nombre":               supplier,
                    "subtotal":             total,          # original currency
                    "moneda":               moneda,
                    "tipo_cambio":          fx_rate,
                    "subtotal_mxn":         total if moneda=="MXN" else round(total*fx_rate,2),
                    "subtotal_usd":         total_usd,      # always USD
                    "estatus":              "Emitida",
                    "descuento_financiero": 0,
                    "pct_descuento":        0,
                    "fecha_recepcion":      "",
                    ...
                    "quantity":             qty,
                    "unit_price":           price,
                    "_split_origin":        True,   # flag: never zero this record
                    "created_at":           now,
                }
                ipo_records.append(entry)
                new_entries.append(entry)

            # ── Step 3: Save all at once (zeroed originals + new items)
            po_save(ipo_yr, ipo_records)
            # ── Mark GPO as split (if it exists)
            if gpo_rec:
                ...
                gpo_save(gpos)
        return jsonify({"ok": True, "created": len(new_entries),
                        "po_number": po_upper})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
