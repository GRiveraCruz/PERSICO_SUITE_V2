...
@app.route("/api/gpo/<path:po_number>/modificar", methods=["POST"])
def api_gpo_modificar(po_number):
    ...
    try:
        ...
        with lock:
            ...
            if tipo == "cancelar":
                # Only allowed if status is 'Emitida' (nothing received)
                ...
                # Allowed when partial delivery exists
...
@app.route("/api/gpo/<path:po_number>/lookup", methods=["GET"])
