...
def api_set_cap_codigos():
    ...
    for r in rows[:10]:
        ...
        colorFondo = (r.get("colorFondo") or "").strip()
        if not (codigo or desc or color or colorFondo):
            continue
        clean.append({"codigo": codigo, "descripcion": desc, "color": color, "colorFondo": colorFondo})
    with lock:
