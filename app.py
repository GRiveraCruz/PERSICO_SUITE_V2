...
def api_get_reassign():
    try:
        ...
        if q:
            orders = [o for o in orders if any(
                q in (i.get("part_number","")).lower() or
                q in (i.get("manufacturer","")).lower() or
                q in (i.get("description","")).lower() or
                q in (i.get("label_code","")).lower()
...
def api_create_gpo():
    """
    Crea una nueva PO generada.
    Body JSON:
      supplier      : dict con datos del proveedor
      pt_sv         : str (opcional)
      job_type      : 'Shopfloor' | 'Fix Asset' | 'Unico' | 'Multiple'
      job           : str (para Unico/Shopfloor/FixAsset)
      items         : lista de items (ver estructura abajo)
      notes         : str (opcional)

    Cada item:
      cat_type      : 'electrico' | 'mecanico' | 'servicios' | 'major'
      cat_code      : str código CE/CM/CS (vacío si major o nuevo)
      part_number   : str
      brand         : str
      description   : str
      label_code    : str
      ...
      job           : str (específico del item — para Multiple)
      notes         : str
    """
    try:
        data = request.get_json()
        ...
        with lock:
            ...
            po_items = []
            # Calcular job por item según job_type
            ...
            # ── Registrar items en el módulo de PO existente (IPOs) por item
            ...
            for i, it in enumerate(po_items):
                ipo_records.append({
                    ...
                    "gpo_pdf":             i==0,   # PDF solo ligado al item 1
            ...
            # ── Actualizar ultimo precio en catálogo si item tiene cat_code
            for it in po_items:
                ct = it.get("cat_type","")
                cc = it.get("cat_code","")
...
def api_gpo_pdf(po_number):
    ...
    try:
        ...
        pt_sv_html = f"<b>PT/SV:</b> {rec['pt_sv']} &nbsp;&nbsp;" if rec.get("pt_sv") else ""
        job_header = f"<b>Job:</b> {rec['job']}" if rec.get("job_type") in ("Unico","Shopfloor","Fix Asset") else f"<b>Tipo Job:</b> {rec.get('job_type','')}"
...
<div class="header">
  ...
  <div>
    ...
    <div class="po-num">{rec['po_number']}</div>
