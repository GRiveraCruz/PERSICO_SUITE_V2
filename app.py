...
@app.route("/api/gastos-viaje/import", methods=["POST"])
def api_import_gastos():
    """Import Gastos de Viaje from Excel. Columns: id, fecha, tipo_gasto, costo, tipo_cambio, valor_usd, job"""
    try:
        import io, openpyxl
        f = request.files.get("file")
        if not f: return jsonify({"error": "No se recibió archivo"}), 400
        ...
        with lock:
            ...
            for row in rows_data[1:]:
                ...
                costo = _parse_money(cv(ci_costo))
                if costo == 0: continue
                job_xl = str(cv(ci_job) or '').strip().upper()
                if not job_xl: continue
                tc_raw = cv(ci_tc)
                if tc_raw and str(tc_raw).strip() not in ('', 'nan', 'NaN', 'None'):
                    tc = _parse_money(tc_raw) or _get_fx_for_date(fecha) or 1.0
                else:
                ...
                usd_raw = cv(ci_usd)
                if usd_raw and str(usd_raw).strip() not in ('', 'nan', 'NaN', 'None'):
                    usd = _parse_money(usd_raw) or round(costo / tc, 4)
                else:
                    usd = round(costo / tc, 4)
                records.append({
                    "id":          f"GV-{datetime.datetime.now().strftime('%Y%m%d%H%M%S%f')}",
                    "id_externo":  str(cv(ci_id) or '').strip(),
                    "tipo_gasto":  str(cv(ci_tipo) or '').strip(),
                    "fecha":       fecha,
                    "moneda":      "MXN",
                    "tipo_cambio": tc,
                    "costo":       costo,
