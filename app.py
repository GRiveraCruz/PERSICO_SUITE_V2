...
def _os_public_view(rec, personal_by_tid):
    ...
    return {
        "id": rec["id"], "sv_number": rec.get("sv_number"), "jobs": rec.get("jobs", []),
...
def api_create_os():
    ...
    if not sv_number or not jobs:
        return jsonify({"error": "Se requiere el número de SV y al menos un Job"}), 400
    with lock:
        records = _os_load()
        rec = {
            "id": _os_gen_id(records),
            "sv_number": sv_number, "jobs": jobs,
...
def api_update_os(oid):
    ...
    with lock:
        ...
        if "sv_number" in data: rec["sv_number"] = (data["sv_number"] or "").strip().upper()
...
def _os_pdf_html(rec, personal_by_tid):
    ...
    html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><title>{rec['id']}</title><style>
      ...
      <div class="hd">
        ...
        <div class="meta">SV: {_esc_html(rec.get('sv_number',''))} · Jobs: {_esc_html(', '.join(rec.get('jobs',[])))}<br>Generado {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}</div>
      </div>
      <div class="grid">
