...
def _catalog_routes(name, url, perm_mod):
    @app.route(f"/api/{url}", methods=["GET"], endpoint=f"api_list_{name}")
    def _list():
        if not can("view", perm_mod): return jsonify({"error":"Sin permiso"}), 403
        return jsonify(_load_catalog(name))

    @app.route(f"/api/{url}", methods=["POST"], endpoint=f"api_create_{name}")
    def _create():
        if not can("create", perm_mod): return jsonify({"error":"Sin permiso"}), 403
        nombre = ((request.json or {}).get("nombre") or "").strip()
        if not nombre: return jsonify({"error": "El nombre es obligatorio"}), 400
        with lock:
            recs = _load_catalog(name)
            if any((r.get("nombre") or "").strip().lower() == nombre.lower() for r in recs):
                return jsonify({"error": "Ese registro ya existe"}), 400
            recs.append({"nombre": nombre})
            _save_catalog(name, recs)
        return jsonify({"ok": True, "nombre": nombre})

    @app.route(f"/api/{url}/<int:idx>", methods=["DELETE"], endpoint=f"api_delete_{name}")
    def _delete(idx):
        if not can("delete", perm_mod): return jsonify({"error":"Sin permiso"}), 403
        with lock:
            recs = _load_catalog(name)
            if 0 <= idx < len(recs):
                recs.pop(idx)
...
_catalog_routes("perfiles", "perfiles", "personal-perfiles")
