...
function backupRenderRecipients(list) {
  ...
  el.innerHTML = list.map((email,i) => `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 10px;background:rgba(255,255,255,.04);border:1px solid var(--border);border-radius:6px">
...
async function backupNow() {
  ...
  if(statusEl) statusEl.textContent='Generando ZIP con todas las bases de datos…';
  try {
    const d = await fetch('/api/admin/backup',{method:'POST'}).then(r=>r.json());
    if(d.error){ if(statusEl) statusEl.style.color='var(--red)'; toast(d.error,'er'); return; }

    // Download ZIP from base64
    ...
    // Status
    const emailMsg = d.email_ok
      ? `✓ ZIP descargado · Correo enviado a: ${d.recipients.join(', ')}`
      : `✓ ZIP descargado · ${d.email_msg}`;
    if(statusEl){ statusEl.style.color = d.email_ok?'var(--green)':'var(--amber)'; statusEl.textContent=emailMsg; }
    toast(d.email_ok?'Respaldo generado y enviado ✓':'Respaldo generado (sin correo)','ok',6000);
  } catch(e){
    if(statusEl){ statusEl.style.color='var(--red)'; statusEl.textContent='Error: '+e.message; }
    toast('Error: '+e.message,'er');
  } finally {
    if(btn){ btn.disabled=false; btn.textContent='📦 Generar Respaldo Ahora'; }
  }
}
