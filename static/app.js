...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-qimp','mo-pt-new','mo-pt-confirm','mo-sv-new','mo-stk-imp','mo-stk-ing','mo-reassign','mo-prov-new','mo-prov-imp','mo-cpo-new','mo-cpo-imp','mo-po-imp','mo-wh-imp','mo-ivp-imp','mo-fx-imp','mo-refuse','mo-award'];
...
function stockRender() {
  ...
  const rows = stockData.filter(r=>
    [r.manufacturer,r.part_number,r.description].join(' ').toLowerCase().includes(gs));
...
async function openIngressStock() {
  ...
  ['stk-ing-mfr','stk-ing-pnum','stk-ing-desc','stk-ing-sec','stk-ing-box'].forEach(id=>document.getElementById(id).value='');
...
function stockSearch(q) {
  ...
  const matches = stockData.filter(r=>
    ...
    (r.part_number||'').toLowerCase().includes(q.toLowerCase())).slice(0,8);
  ...
  res.innerHTML = matches.map(r=>`
    <div onclick="selectStockItem('${r.id}')" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center;transition:background .15s" onmouseenter="this.style.background='rgba(255,255,255,.05)'" onmouseleave="this.style.background=''">
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:var(--muted2)">${esc(r.manufacturer)} · ${esc(r.description||'')}</div>
      </div>
...
function recoveryRender() {
  ...
  const rows = recoveryData.filter(r=>
    [r.manufacturer,r.part_number,r.description,r.job].join(' ').toLowerCase().includes(gs));
  const fmt  = v=>'$'+Number(v||0).toLocaleString('en-US',{minimumFractionDigits:2});
