...
function jobOpen(jobNum){
  ...
  jCalcGM();jStab('det',document.querySelector('#j-panel .ptab'));
  openPanel('j-panel');
}
...
async function jobDelete(){
  if(!jobCurrentJob||!confirm('¿Eliminar datos de '+jobCurrentJob.job_number+'?\n(La carpeta NAS se conserva)'))return;
  try{
    const r=await apiCall('DELETE','/jobs/'+jobCurrentJob.job_number);
    if(r.error){toast(r.error,'er');return;}
    await loadJobs();closePanel();toast('Datos del Job eliminados','ok');
...
function quoteRender(){
  ...
  tb.innerHTML=data.map(r=>{
    ...
    return`<tr class="${sc}${r.row===quoteCurrentRow?' sel':''}${r.refused?' refused-row':''}" onclick="quoteOpen(${r.row})">
      ...
      <td>${r.refused?'<span class="badge b-done" style="background:rgba(200,16,46,.3);color:#ff6b6b">Refused</span>':r.awarded?'<span class="badge b-yes">Awarded</span>':'—'}</td>
    </tr>`;
  }).join('');
}
...
function quoteOpen(row){
  ...
  if (awardBtn)  awardBtn.style.display  = (r.awarded && !r.refused) ? '' : 'none';
  if (refuseBtn) refuseBtn.style.display = r.refused ? 'none' : '';
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-po-imp'];
