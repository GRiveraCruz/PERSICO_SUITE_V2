...
async function gpoSearchSupplier(q) {
  ...
  try {
    ...
    res.innerHTML = list.map(p=>`
      <div onclick="gpoSelectSupplier(${JSON.stringify(JSON.stringify(p))})"
...
function gpoSelectSupplier(jsonStr) {
...
async function gpoSearchItem(q) {
  ...
  res.style.display='';
  res.innerHTML = all.map(r=>`
    <div onclick="gpoFillItemForm(${JSON.stringify(JSON.stringify(r))})"
...
function gpoFillItemForm(jsonStr) {
