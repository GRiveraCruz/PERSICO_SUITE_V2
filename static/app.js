...
async function imConvertPDF(inp) {
  const file = inp?.files?.[0];
  if(!file) return;
  const statusEl = document.getElementById('im-pdf-status');
  statusEl.style.color='var(--muted2)'; statusEl.textContent='⏳ Procesando PDF…';

  try {
    const fd = new FormData(); fd.append('file', file);
    const resp = await fetch('/api/util/pdf-to-po-excel', {method:'POST', body:fd});
    ...
    const fname= cd.match(/filename="?([^"]+)"?/)?.[1] || 'PO_normalizada.xlsx';
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob); a.download = fname; a.click();

    statusEl.style.color='var(--green)';
    statusEl.textContent=`✓ Excel descargado: ${fname}`;
    inp.value='';
  } catch(e) {
