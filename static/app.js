...
function rptRender(d){
  ...
  if(hasSvc) {
    ...
    const gvRows = (d.svc_gastos_items||[]).map((r,i)=>`
      <tr class="rpt-tr" style="${i%2?'background:rgba(255,255,255,.02)':''}">
        <td class="rpt-td" style="color:var(--muted2);font-size:11px">${r.fecha||'—'}</td>
        <td class="rpt-td"><span style="font-size:10px;background:rgba(255,255,255,.07);padding:2px 8px;border-radius:4px">${esc(r.tipo_gasto||'—')}</span></td>
...
function gpoModShowPO(rec) {
  ...
  if(status === 'Parcial') {
    // Only cierre anticipado
    const valorRecibido = gpoModCalcRecibido(rec);
    btns.innerHTML = `<button onclick="gpoModSelectTipo('cierre_anticipado')" class="btn" style="background:rgba(255,193,7,.15);border:1px solid var(--amber);color:var(--amber);font-size:12px;padding:10px 20px">
      ⏸ Cierre Anticipado<br><span style="font-size:10px;opacity:.7">Valor recibido: ${fmt(valorRecibido)}</span>
    </button>`;
  } else {
    // Emitida — cancelar or nueva version
    btns.innerHTML = `
      <button onclick="gpoModSelectTipo('cancelar')" class="btn" style="background:rgba(200,16,46,.12);border:1px solid var(--red);color:var(--red);font-size:12px;padding:10px 20px">
        🚫 Cancelar Orden
      </button>
...
function gpoModStatusBadge(s) {
  ...
  const [bg, color] = map[s] || ['rgba(255,255,255,.06)','var(--muted)'];
...
function gpoModSelectTipo(tipo) {
  ...
  ['cancelar','cierre','nueva-version'].forEach(t => {
    document.getElementById(`gpo-action-${t}`).style.display = 'none';
  });
  document.getElementById('gpo-mod-action').style.display = '';
  document.getElementById('btn-gpo-mod-save').style.display = '';

  if(tipo === 'cancelar') {
    document.getElementById('gpo-action-cancelar').style.display = '';
    document.getElementById('gpo-mod-nota').value = '';
  } else if(tipo === 'cierre_anticipado') {
    document.getElementById('gpo-action-cierre').style.display = '';
    const fmt = v => '$'+Number(v||0).toLocaleString('en-US',{minimumFractionDigits:2});
    document.getElementById('gpo-mod-recibido-val').textContent = fmt(gpoModCalcRecibido(gpoModRec));
    document.getElementById('gpo-mod-nota').value = '';
...
async function gpoModGuardar() {
  ...
  try {
    ...
    if(gpoModTipo === 'cancelar') {
      ...
      body.valor_recibido = gpoModCalcRecibido(gpoModRec);
      body.nota = document.getElementById('gpo-mod-nota')?.value?.trim() || '';
      ...
      document.querySelectorAll('#gpo-mod-items-body tr').forEach(tr => {
        ...
        if(qty > 0 || desc) items.push({
          description: desc, part_number: pnum, brand, quantity: qty,
          unit_price: price, total: round2(qty*price)
        });
      });
    ...
    if(d.error){ toast(d.error,'er'); return; }
    closeMo('mo-gpo-mod');
    toast(`✓ Orden ${gpoModRec.po_number} modificada: ${gpoModTipo.replace('_',' ')}`, 'ok', 5000);
    await loadGPO();
  } catch(e){ toast('Error: '+e.message,'er'); }
