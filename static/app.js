...
function areaRenderList(){
  ...
  c.innerHTML=areasList.map((a,idx)=>`<div style="display:flex;align-items:center;justify-content:space-between;background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:10px 14px">
...
async function areaCreate(){
  ...
  try{
    const r=await(await fetch('/api/areas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre})})).json();
    if(r.error){toast(r.error,'er');return;}
    inp.value=''; toast('Área agregada ✓','ok'); await loadAreas();
  }catch(e){toast('Error: '+e,'er');}
}
async function areaDelete(idx){
  if(!confirm('¿Eliminar el área "'+areasList[idx].nombre+'"?'))return;
