...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-qimp','mo-pt-new','mo-pt-confirm','mo-sv-new','mo-stk-imp','mo-stk-ing','mo-reassign','mo-cpo-new','mo-cpo-imp','mo-po-imp','mo-wh-imp','mo-ivp-imp','mo-fx-imp','mo-refuse','mo-award'];
...
async function openIngressStock() {
  ...
  document.getElementById('stk-ing-qty').value='';
...
async function saveStockIngress() {
  ...
  const payload = {
    ...
    quantity:     parseInt(document.getElementById('stk-ing-qty').value)||0,
  ...
  try {
    ...
    closeMo('mo-stk-ing');
    await loadStock();
    toast(d.action==='updated'?'Stock actualizado ✓':'Material ingresado ✓','ok');
  } catch(e){toast('Error: '+e.message,'er');}
}
