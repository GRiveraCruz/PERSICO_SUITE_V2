...
function applyPermsToDom(d) {
  ...
  const tabMap = {
    jobs:'jobs', rates:'rates', quotes:'quotes', pt:'pt', sv:'sv', cpo:'cpo',
    'cat-electrico':'cat-electrico', 'cat-mecanico':'cat-mecanico', 'cat-servicios':'cat-servicios',
    proveedores:'proveedores', gpo:'gpo', po:'po', wh:'wh', ivp:'ivp',
    stock:'stock', recovery:'recovery', reassign:'reassign',
    report:'report', multirpt:'multirpt', fx:'fx'
  };
  document.querySelectorAll('.nav-tab').forEach(btn => {
...
function renderAdminUsers(d) {
  ...
  // Group modules by section for visual clarity
  const MODULE_GROUPS = [
  ...
  const html = Object.entries(users).map(([uname, info]) => {
    ...
    const rows = MODULE_GROUPS.map(grp => {
      ...
      const groupHeader = `<tr style="background:rgba(255,255,255,.03)">
        <td colspan="${actions.length+1}" style="padding:6px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:var(--red);border-bottom:1px solid var(--border)">${grp.label}</td>
      </tr>`;
      const modRows = groupMods.map(mod => {
        ...
        return `<tr style="border-bottom:1px solid rgba(255,255,255,.04)">
          <td style="padding:5px 10px 5px 18px;font-size:11px;color:var(--muted2);white-space:nowrap">${MODULE_LABELS[mod]||mod}</td>
          ${cols}
        </tr>`;
      }).join('');
      return groupHeader + modRows;
    }).join('');
