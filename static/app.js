...
function closePanel(){
  ...
  ['j-panel','r-panel','q-panel','t-panel','pf-panel','pm-panel'].forEach(id=>document.getElementById(id).classList.remove('on'));
  _currentPanel=null;
  ...
  if(personalCurrentRow!==null){personalCurrentRow=null;personalRender();}
  if(pmCurrentId){pmCurrentId=null;pmRender();}
}
...
// ── Debounce global para campos de búsqueda ──
// Los campos de búsqueda/filtro de texto de la Suite usan oninput="miFuncion()"
// inline, disparándose en cada tecla — con datasets grandes (Purchase Orders,
// Stock, Work Hours) esto genera bloqueos notorios en equipos más viejos. En vez
// de tocar cada uno de los ~65 lugares uno por uno (alto riesgo de error), se
// intercepta el evento en la fase de captura (antes de llegar al manejador
// inline) y se retrasa su ejecución ~250ms tras la última tecla — el resultado
// de la búsqueda no cambia, solo CUÁNDO se calcula. Los campos numéricos
// (type="number", usados para cálculos en vivo) se excluyen a propósito: ahí sí
// se espera respuesta instantánea.
(function(){
  ...
  document.addEventListener('input', function(e){
    ...
    if((el.type||'text') !== 'text' && el.type !== 'search') return;
    if(typeof el.oninput !== 'function') return;
    e.stopImmediatePropagation();
    clearTimeout(timers.get(el));
    const fn = el.oninput;
    timers.set(el, setTimeout(() => fn.call(el, e), DEBOUNCE_MS));
  }, true); // fase de captura: se adelanta al manejador inline del elemento
})();

function showMenu(group){
  clearTimeout(group._hideTimer);
  let dropdown = group._dropdownEl;
  if(!dropdown){
    dropdown = group.querySelector('.nav-dropdown');
    if(!dropdown) return;
    group._dropdownEl = dropdown;
    // Como el desplegable se saca de .nav-group hacia <body> (ver más abajo), deja
    // de contar como "parte" del grupo para efectos de mouseenter/mouseleave — sin
    // esto, en cuanto el mouse entra al desplegable el navegador interpreta que
    // "saliste" del grupo y lo cierra antes de poder hacer clic en algo. Se maneja
    // el hover del propio desplegable por separado, una sola vez.
    dropdown.addEventListener('mouseenter', () => clearTimeout(group._hideTimer));
    dropdown.addEventListener('mouseleave', () => hideMenu(group));
  }
  // La barra de menú (.nav-tabs) puede desplazarse horizontalmente cuando no caben
  // todos los grupos — si el desplegable se quedara anidado ahí, el navegador lo
  // recortaría junto con el resto del contenido que se sale del contenedor. Para
  // evitarlo, se saca a <body> con posición fija, calculada a partir de en dónde
  // está el botón en ese momento.
...
function hideMenu(group){
  group._hideTimer = setTimeout(()=>{
    const dropdown = group._dropdownEl;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(navTabs){
    navTabs.addEventListener('scroll', () => {
      document.querySelectorAll('.nav-dropdown').forEach(dd => dd.style.display = '');
    });
  }
});
...
function switchModule(mod, btn){
  ...
  const groupMap = {
    ...
    wh:'ng-rrhh', rates:'ng-rrhh', report:'ng-costo', multirpt:'ng-costo', fx:'ng-costo',
    admin:'ng-config'
  };
...
function jobOpen(jobNum){
  ...
  document.getElementById('je-closing').value=j.closing_date||'';
...
function jStab(id,btn){
  ...
  if(id==='doc'&&jobCurrentJob){jLoadFiles(jobCurrentJob.job_number);jLoadFilesCierre(jobCurrentJob.job_number);}
}
...
async function jobSave(){
  ...
  const data={
    ...
    ship_date:document.getElementById('je-ship').value,closing_date:document.getElementById('je-closing').value,
    approval_fc:document.getElementById('je-fc').value,
...
// ── Documentos de Cierre (subcarpeta CIERRE) ──
async function jLoadFilesCierre(jobNum){
  const fl=document.getElementById('j-fl-cierre'); if(!fl) return;
  fl.innerHTML='<div class="es" style="padding:16px 0"><div class="spinner"></div></div>';
  try{
...
async function jUploadFilesCierre(files){
  if(!jobCurrentJob||!files.length)return;
  const fd=new FormData();Array.from(files).forEach(f=>fd.append('files',f));
  try{
    const r=await fetch('/api/files-cierre/'+jobCurrentJob.job_number,{method:'POST',body:fd});
    const d=await r.json();if(d.error){toast(d.error,'er');return;}
    toast(d.saved.length+' archivo(s) guardados','ok');jLoadFilesCierre(jobCurrentJob.job_number);
  }catch{toast('Error al subir','er');}
}
function jDropFilesCierre(e){e.preventDefault();document.getElementById('j-dz-cierre').classList.remove('dg');jUploadFilesCierre(e.dataTransfer.files);}

async function jDelFileCierre(jobNum,name){
  if(!confirm('¿Eliminar "'+name+'"?'))return;
  try{const r=await apiCall('DELETE','/files-cierre/'+jobNum+'/'+encodeURIComponent(name));if(r.error){toast(r.error,'er');return;}toast('Archivo eliminado','ok');jLoadFilesCierre(jobNum);}catch{toast('Error','er');}
}
...
async function loadAreas(){
  try{
    ...
    if(Array.isArray(d)){
      areasList=d;
      areasLoadError=d.length===0?'El servidor respondió HTTP 200 pero con 0 áreas registradas.':null;
    }else{
      areasList=[];
...
function areaRenderList(){
  const c=document.getElementById('areas-list'); if(!c) return;
  if(!areasList.length){ c.innerHTML='<div class="es"><div class="ei">🏢</div><p>Sin áreas registradas.</p></div>'; return; }
  c.innerHTML=areasList.map((a,idx)=>`<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:10px 14px">
    <div style="display:flex;align-items:center;justify-content:space-between" id="area-view-${idx}">
      <div>
...
function areaEditStart(idx){
  const a=areasList[idx];
  const opts='<option value="">— Nivel superior (sin jefe directo) —</option>'
    +areasList.filter((x,i)=>i!==idx).map(x=>`<option value="${esc(x.nombre)}" ${x.nombre===a.reportaA?'selected':''}>${esc(x.nombre)}</option>`).join('');
  document.getElementById('area-view-'+idx).outerHTML=`
    <div id="area-view-${idx}" style="display:flex;flex-direction:column;gap:8px">
      <input type="text" id="area-edit-nombre-${idx}" value="${esc(a.nombre)}" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;padding:6px 8px;font-size:12px;color:var(--text);outline:none">
      <select id="area-edit-reporta-${idx}" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;padding:6px 8px;font-size:12px;color:var(--text);outline:none">${opts}</select>
      <div style="display:flex;gap:6px;justify-content:flex-end">
        <button class="btn btn-s" style="width:auto;padding:5px 12px;font-size:11px" onclick="areaRenderList()">Cancelar</button>
        <button class="btn btn-p" style="width:auto;padding:5px 12px;font-size:11px" onclick="areaEditSave(${idx})">Guardar</button>
      </div>
    </div>`;
}
async function areaEditSave(idx){
  const nombre=document.getElementById('area-edit-nombre-'+idx).value.trim();
  const reportaA=document.getElementById('area-edit-reporta-'+idx).value||null;
  if(!nombre){toast('El nombre es obligatorio','er');return;}
  try{
    const r=await(await fetch('/api/areas/'+idx,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre,reportaA})})).json();
    if(r.error){toast(r.error,'er');return;}
...
async function areaCreate(){
  const inp=document.getElementById('area-new-input'); const nombre=inp.value.trim();
  const reportaA=document.getElementById('area-new-reporta')?.value||null;
  if(!nombre){toast('Escribe un nombre de área','er');return;}
  try{
    const r=await(await fetch('/api/areas',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre,reportaA})})).json();
    if(r.error){toast(r.error,'er');return;}
    inp.value=''; const rs=document.getElementById('area-new-reporta'); if(rs) rs.value='';
    toast('Área agregada ✓','ok'); await loadAreas();
  }catch(e){toast('Error: '+e,'er');}
}
async function areaDelete(idx){
  if(!confirm('¿Eliminar el área "'+areasList[idx].nombre+'"?\n\nLas áreas que reportaban a esta pasarán a nivel superior.'))return;
  try{ await fetch('/api/areas/'+idx,{method:'DELETE'}); toast('Área eliminada','if'); await loadAreas(); }catch(e){toast('Error: '+e,'er');}
}
function areaPopulateSelects(){
  const opts='<option value="">— Seleccionar —</option>'+areasList.map(a=>`<option value="${esc(a.nombre)}">${esc(a.nombre)}</option>`).join('');
  const optsAll='<option value="">Todas</option>'+areasList.map(a=>`<option value="${esc(a.nombre)}">${esc(a.nombre)}</option>`).join('');
  ['tn-area','te-area'].forEach(id=>{const el=document.getElementById(id);if(el){const cur=el.value;el.innerHTML=opts;el.value=cur;}});
  const flt=document.getElementById('personal-area-flt'); if(flt){const cur=flt.value;flt.innerHTML=optsAll;flt.value=cur;}
  const rs=document.getElementById('area-new-reporta');
...
function areaRenderOrgChart(){
  ...
  const childrenOf={};
  areasList.forEach(a=>{
    const parent=(a.reportaA && names.has(a.reportaA)) ? a.reportaA : null;
    (childrenOf[parent]=childrenOf[parent]||[]).push(a.nombre);
  });
...
function perfilRenderList(){
  const c=document.getElementById('perfiles-list'); if(!c) return;
  if(!perfilesList.length){ c.innerHTML='<div class="es"><div class="ei">🪪</div><p>Sin perfiles registrados.</p></div>'; return; }
  c.innerHTML=perfilesList.map((p,idx)=>`<div style="background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:10px 14px">
    <div style="display:flex;align-items:center;justify-content:space-between" id="perfil-view-${idx}">
      <div>
...
function perfilEditStart(idx){
  const p=perfilesList[idx];
  const opts='<option value="">— Sin área —</option>'+areasList.map(a=>`<option value="${esc(a.nombre)}" ${a.nombre===p.area?'selected':''}>${esc(a.nombre)}</option>`).join('');
  document.getElementById('perfil-view-'+idx).outerHTML=`
...
async function perfilDelete(idx){
  if(!confirm('¿Eliminar el perfil "'+perfilesList[idx].nombre+'"? Esto también borrará sus documentos adjuntos.'))return;
  try{ await fetch('/api/perfiles/'+idx,{method:'DELETE'}); toast('Perfil eliminado','if'); await loadPerfiles(); }catch(e){toast('Error: '+e,'er');}
}
function perfilPopulateSelects(){
  const opts='<option value="">— Seleccionar —</option>'+perfilesList.map(p=>`<option value="${esc(p.nombre)}">${esc(p.nombre)}</option>`).join('');
  const optsAll='<option value="">Todos</option>'+perfilesList.map(p=>`<option value="${esc(p.nombre)}">${esc(p.nombre)}</option>`).join('');
  ['tn-puesto','te-puesto'].forEach(id=>{const el=document.getElementById(id);if(el){const cur=el.value;el.innerHTML=opts;el.value=cur;}});
...
function pfOpenDocs(idx){
  const p=perfilesList[idx]; if(!p) return;
  pfCurrentIdx=idx;
  document.getElementById('pf-nombre').textContent=p.nombre||'—';
  openPanel('pf-panel');
  pfLoadFiles(p.pid);
}
async function pfLoadFiles(pid){
  const fl=document.getElementById('pf-fl');
  document.getElementById('pf-dp').textContent=pid||'—';
  if(!pid){ fl.innerHTML='<div class="es"><div class="ei">⚠</div><p>Este perfil aún no tiene ID de documentos — guárdalo de nuevo (editar → Guardar) para generarlo.</p></div>'; return; }
  fl.innerHTML='<div class="es"><div class="spinner"></div></div>';
  try{
...
async function pfUploadFiles(fileList){
  const p=perfilesList[pfCurrentIdx]; if(!p||!p.pid){ toast('Este perfil no tiene ID de documentos — guárdalo primero','er'); return; }
  const fd=new FormData(); Array.from(fileList).forEach(f=>fd.append('files',f));
  try{
    const res=await fetch('/api/perfiles/upload/'+p.pid,{method:'POST',body:fd});
    const data=await res.json(); if(data.error)throw new Error(data.error);
    toast(data.saved.length+' archivo(s) guardados','ok'); await pfLoadFiles(p.pid);
  }catch(e){toast('Error al subir: '+e,'er');}
  document.querySelector('#pf-dz input').value='';
}
function pfDropFiles(e){e.preventDefault();document.getElementById('pf-dz').classList.remove('dg');pfUploadFiles(e.dataTransfer.files);}
async function pfDelFile(pid,filename){
  if(!confirm('¿Eliminar "'+filename+'"?'))return;
  try{const r=await(await fetch('/api/perfiles/files/'+pid+'/'+encodeURIComponent(filename),{method:'DELETE'})).json();if(r.error){toast(r.error,'er');return;}toast(filename+' eliminado','if');await pfLoadFiles(pid);}catch(e){toast('Error: '+e,'er');}
}

// ── Listado de Trabajadores ──
let personalRecords=[], personalCurrentRow=null, personalVisibleCount=100, tImpFile=null, personalLoadError=null;
...
async function loadPersonal(){
  try{
    ...
    if(!Array.isArray(d)){
      personalRecords=[];
      personalLoadError=`HTTP ${resp.status} — ${(d&&d.error)?d.error:'respuesta inesperada del servidor'}`;
      document.getElementById('personal-dot').className='conn-dot er';
      document.getElementById('personal-lbl').textContent=personalLoadError;
      personalRender();
      return;
    }
    personalLoadError = d.length===0 ? `El servidor respondió HTTP 200 pero con 0 trabajadores registrados en esta base de datos.` : null;
    personalRecords=d;
    document.getElementById('personal-dot').className=d.length?'conn-dot ok':'conn-dot er';
    document.getElementById('personal-lbl').textContent=personalRecords.length+' registros';
    personalVisibleCount=100;
    personalRender();
  }catch(e){
    personalRecords=[];
    personalLoadError='Error de red/conexión al pedir /api/personal: '+e;
    document.getElementById('personal-dot').className='conn-dot er';
    document.getElementById('personal-lbl').textContent='Error de conexión';
    toast('Error al cargar Listado de Trabajadores: '+e,'er');
  }
}

function personalJefeOptions(excludeTid){
  return '<option value="">— Sin jefe directo asignado —</option>'
    + personalRecords.filter(p=>p.tid!==excludeTid).sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||''))
      .map(p=>`<option value="${esc(p.tid)}">${esc(p.nombre)}${p.puesto?' — '+esc(p.puesto):''}</option>`).join('');
}
function personalJefeName(tid){
  if(!tid) return null;
  const j=personalRecords.find(x=>x.tid===tid);
  return j?j.nombre:null;
}

function personalAntiguedad(fecha){
  if(!fecha) return '—';
  const d=new Date(fecha+'T00:00:00'); if(isNaN(d)) return '—';
  const now=new Date();
  ...
  if(years<0) return '—';
  const yTxt=years>0?years+(years===1?' año':' años'):'';
  const mTxt=months>0?months+(months===1?' mes':' meses'):'';
  return [yTxt,mTxt].filter(Boolean).join(', ')||'< 1 mes';
}

function personalFiltered(){
  const gs=(document.getElementById('personal-gs')?.value||'').toLowerCase();
  const area=document.getElementById('personal-area-flt')?.value||'';
  const puesto=document.getElementById('personal-puesto-flt')?.value||'';
  const estado=document.getElementById('personal-estado-flt')?.value||'';
  return personalRecords.filter(r=>{
    if(area && r.area!==area) return false;
    if(puesto && r.puesto!==puesto) return false;
    if(estado && (r.estado||'Activo')!==estado) return false;
    if(gs && !((r.nombre||'')+' '+(r.puesto||'')+' '+(r.area||'')).toLowerCase().includes(gs)) return false;
    return true;
  }).sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||''));
}

function personalRenderReset(){ personalVisibleCount=100; personalRender(); }
function personalLoadMore(){ personalVisibleCount+=100; personalRender(); }

function personalEstadoBadge(estado){
  const e=estado||'Activo';
  if(e==='Baja') return '<span class="badge b-toappr">Baja</span>';
  if(e==='Transferencia') return '<span class="badge b-wip">Transferencia</span>';
  return '<span class="badge b-yes">Activo</span>';
}

function personalRender(){
  const rows=personalFiltered();
  const tb=document.getElementById('personal-tb');
  document.getElementById('personal-tot').textContent=personalRecords.length;
  document.getElementById('personal-areas-n').textContent=new Set(personalRecords.map(r=>r.area).filter(Boolean)).size;

  if(!rows.length){
    tb.innerHTML='<tr><td colspan="10"><div class="es"><span class="ei">👥</span><br>Sin registros</div></td></tr>';
    return;
  }
  ...
  tb.innerHTML=display.map(r=>{
    ...
    const recBadge=rec
      ? `<span class="badge b-toappr" title="${esc(rec)}" style="max-width:220px;overflow:hidden;text-overflow:ellipsis;display:inline-block;vertical-align:middle">⚠ ${esc(rec)}</span>`
      : `<span class="badge b-yes">Sin incidentes</span>`;
    const jefeNombre=personalJefeName(r.jefe_directo);
    return`<tr class="${r.row===personalCurrentRow?'sel':''}" onclick="personalOpen(${r.row})">
      <td style="font-weight:600;color:var(--text)">${esc(r.nombre||'—')}</td>
      <td>${esc(r.puesto||'—')}</td>
      <td>${esc(r.area||'—')}</td>
      <td style="font-size:11px;color:var(--muted2)">${jefeNombre?esc(jefeNombre):'—'}</td>
      <td style="font-size:11px;color:var(--muted2)">${esc(r.tipo_actividad||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${r.fecha_ingreso||'—'}</td>
      <td style="font-size:12px;color:var(--gold);font-weight:600">${personalAntiguedad(r.fecha_ingreso)}</td>
      <td>${personalEstadoBadge(r.estado)}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${r.fecha_baja||'—'}</td>
      <td>${recBadge}</td>
    </tr>`;
  }).join('')+moreRow;
}

function personalOpen(row){
  const r=personalRecords.find(x=>x.row===row); if(!r) return;
  personalCurrentRow=row;
  ...
  document.getElementById('te-nombre').value=r.nombre||'';
  document.getElementById('te-puesto').value=r.puesto||'';
  document.getElementById('te-area').value=r.area||'';
  document.getElementById('te-tipo').value=r.tipo_actividad||'';
  document.getElementById('te-ingreso').value=r.fecha_ingreso||'';
  ...
  tEstadoChange();
  document.getElementById('te-rec').value=r.record_disciplinario||'';
  tStab('det',document.querySelector('#t-panel .ptab'));
  openPanel('t-panel');
  personalRender();
}

function tEstadoChange(){
  const estado=document.getElementById('te-estado').value;
  const fb=document.getElementById('te-fecha-baja');
  fb.disabled=(estado!=='Baja');
  if(estado!=='Baja') fb.value='';
}

function tStab(name,btn){
  document.querySelectorAll('#t-panel .ptab').forEach(b=>b.classList.remove('on'));
  document.querySelectorAll('#t-panel .tc2').forEach(c=>c.classList.remove('on'));
  if(btn) btn.classList.add('on');
  else document.querySelectorAll('#t-panel .ptab').forEach(b=>{if(b.getAttribute('onclick').includes("'"+name+"'"))b.classList.add('on');});
  document.getElementById('ttab-'+name).classList.add('on');
  if(name==='doc'&&personalCurrentRow!==null){const r=personalRecords.find(x=>x.row===personalCurrentRow);if(r)tLoadFiles(r.tid);}
}

async function personalSave(){
  const r=personalRecords.find(x=>x.row===personalCurrentRow); if(!r) return;
  const payload={
    nombre:document.getElementById('te-nombre').value.trim(),
    puesto:document.getElementById('te-puesto').value||null,
    area:document.getElementById('te-area').value||null,
    tipo_actividad:document.getElementById('te-tipo').value.trim()||null,
    fecha_ingreso:document.getElementById('te-ingreso').value||null,
    jefe_directo:document.getElementById('te-jefe').value||null,
    estado:document.getElementById('te-estado').value||'Activo',
    fecha_baja:document.getElementById('te-fecha-baja').value||null,
    record_disciplinario:document.getElementById('te-rec').value.trim()||null,
  };
  if(!payload.nombre){toast('El nombre es obligatorio','er');return;}
  try{
    const res=await apiCall('PUT','/personal/'+personalCurrentRow,payload); if(res.error)throw new Error(res.error);
    Object.assign(r,payload);
    document.getElementById('tp-nombre').textContent=r.nombre;
    document.getElementById('tp-puesto').textContent=[r.puesto,r.area].filter(Boolean).join(' · ')||'Sin puesto asignado';
    document.getElementById('te-antiguedad').textContent=personalAntiguedad(r.fecha_ingreso);
...
async function personalDelete(){
  const r=personalRecords.find(x=>x.row===personalCurrentRow); if(!r) return;
  if(!confirm('¿Eliminar a '+r.nombre+'? Esto también borrará sus documentos adjuntos.'))return;
  try{
    await apiCall('DELETE','/personal/'+personalCurrentRow);
    closePanel(); toast(r.nombre+' eliminado','if'); await loadPersonal();
  }catch(e){toast('Error: '+e,'er');}
}

function personalOpenNew(){
  ['tn-nombre','tn-tipo','tn-ingreso','tn-rec'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('tn-puesto').value=''; document.getElementById('tn-area').value='';
  document.getElementById('tn-jefe').innerHTML=personalJefeOptions(null);
  document.getElementById('tn-jefe').value='';
  document.getElementById('mo-tnew').classList.add('on');
  document.getElementById('tn-nombre').focus();
}

async function personalCreate(){
  const nombre=document.getElementById('tn-nombre').value.trim();
  if(!nombre){toast('El nombre del trabajador es obligatorio','er');return;}
  const btn=document.getElementById('btn-tcreate'); btn.disabled=true; btn.textContent='Creando…';
  const payload={
    nombre,
    puesto:document.getElementById('tn-puesto').value||null,
    area:document.getElementById('tn-area').value||null,
    tipo_actividad:document.getElementById('tn-tipo').value.trim()||null,
    fecha_ingreso:document.getElementById('tn-ingreso').value||null,
    jefe_directo:document.getElementById('tn-jefe').value||null,
    record_disciplinario:document.getElementById('tn-rec').value.trim()||null,
  };
  try{
    const res=await apiCall('POST','/personal',payload); if(res.error)throw new Error(res.error);
    closeMo('mo-tnew'); toast(nombre+' creado ✓','ok');
    await loadPersonal(); setTimeout(()=>{const nr=personalRecords.find(x=>x.tid===res.tid); if(nr) personalOpen(nr.row);},300);
  }catch(e){toast('Error al crear: '+e,'er');}
  finally{btn.disabled=false; btn.textContent='Crear trabajador';}
}

// ── Documentos por trabajador ──
async function tLoadFiles(tid){
  const fl=document.getElementById('t-fl');
  document.getElementById('t-dp').textContent=tid;
  fl.innerHTML='<div class="es"><div class="spinner"></div></div>';
  try{
...
async function tUploadFiles(fileList){
  ...
  const fd=new FormData(); Array.from(fileList).forEach(f=>fd.append('files',f));
  try{
    const res=await fetch('/api/personal/upload/'+r.tid,{method:'POST',body:fd});
    const data=await res.json(); if(data.error)throw new Error(data.error);
    toast(data.saved.length+' archivo(s) guardados','ok'); await tLoadFiles(r.tid);
  }catch(e){toast('Error al subir: '+e,'er');}
  document.querySelector('#t-dz input').value='';
}
function tDropFiles(e){e.preventDefault();document.getElementById('t-dz').classList.remove('dg');tUploadFiles(e.dataTransfer.files);}
async function tDelFile(tid,filename){
  if(!confirm('¿Eliminar "'+filename+'"?'))return;
  try{const r=await(await fetch('/api/personal/files/'+tid+'/'+encodeURIComponent(filename),{method:'DELETE'})).json();if(r.error){toast(r.error,'er');return;}toast(filename+' eliminado','if');await tLoadFiles(tid);}catch(e){toast('Error: '+e,'er');}
}

// ── Importar Excel ──
function personalExport(){ window.open('/api/personal/export','_blank'); }

function personalOpenImport(){
  tImpFile=null;
  document.getElementById('timp-file').value='';
  document.getElementById('timp-fname').textContent='—';
  document.getElementById('timp-results').style.display='none';
  document.getElementById('btn-timp-run').disabled=true;
  document.getElementById('mo-timp').classList.add('on');
}
function onTimpFile(inp){
  if(inp.files.length){
    tImpFile=inp.files[0];
    document.getElementById('timp-fname').textContent=tImpFile.name;
    document.getElementById('btn-timp-run').disabled=false;
  }
}
function tDropImport(e){
  e.preventDefault();
  document.getElementById('tdz-imp').classList.remove('dg');
  const f=e.dataTransfer.files[0];
  if(f){tImpFile=f;document.getElementById('timp-fname').textContent=f.name;document.getElementById('btn-timp-run').disabled=false;}
}
async function personalRunImport(){
  if(!tImpFile)return;
  const btn=document.getElementById('btn-timp-run');
  btn.disabled=true; btn.textContent='Importando…';
  document.getElementById('timp-results').style.display='none';
  const fd=new FormData();
  fd.append('file',tImpFile);
  fd.append('mode',document.getElementById('timp-mode').value);
  try{
    const r=await fetch('/api/personal/import',{method:'POST',body:fd});
    const d=await r.json();
    if(d.error){toast(d.error,'er');return;}
    document.getElementById('timp-chips').innerHTML=
      '<div class="r-chip" style="background:rgba(39,174,96,.18);border:1px solid rgba(39,174,96,.5)"><div class="n" style="color:#1f8a4c;font-weight:700">'+d.imported+'</div><div class="l" style="color:#1f8a4c">Importados</div></div>'+
      '<div class="r-chip" style="background:rgba(200,16,46,.16);border:1px solid rgba(200,16,46,.4)"><div class="n" style="color:#a80d24;font-weight:700">'+d.total+'</div><div class="l" style="color:#a80d24">Total registros</div></div>';
    document.getElementById('timp-results').style.display='block';
    await loadPersonal();
    toast(d.imported+' trabajadores importados ✓','ok',5000);
  }catch(e){toast('Error al importar: '+e,'er');}
  finally{btn.disabled=false;btn.textContent='Importar →';}
}

document.addEventListener('DOMContentLoaded',async ()=>{ await loadAreas(); await loadPerfiles(); await loadPersonal(); await loadCapLegend(); await loadCapacidad(); await loadVacaciones(); });

// ════════════════════════════════════════════════════════
//  RECURSOS HUMANOS — VACACIONES
// ════════════════════════════════════════════════════════
let vacaciones={};
let vacCurrentTid=null;

function vacAniosCompletos(fecha){
  if(!fecha) return 0;
  const d=new Date(fecha+'T00:00:00'); if(isNaN(d)) return 0;
  const now=new Date();
  let years=now.getFullYear()-d.getFullYear();
  const beforeAnniversary = (now.getMonth()<d.getMonth()) || (now.getMonth()===d.getMonth() && now.getDate()<d.getDate());
  if(beforeAnniversary) years--;
  return Math.max(0,years);
}

async function loadVacaciones(){
  try{ vacaciones=await(await fetch('/api/vacaciones')).json(); }catch(e){ vacaciones={}; }
  vacRender();
}

function vacRender(){
  const tb=document.getElementById('vac-tb'); if(!tb) return;
  const gs=(document.getElementById('vac-gs')?.value||'').toLowerCase();
  const tids=Object.keys(vacaciones);
  const rows=tids.map(tid=>{
    const p=personalRecords.find(x=>x.tid===tid);
    return {tid, p};
  }).filter(r=>r.p) // solo trabajadores que sigan existiendo en Listado
    .filter(r=>!gs || (r.p.nombre||'').toLowerCase().includes(gs))
    .sort((a,b)=>(a.p.nombre||'').localeCompare(b.p.nombre||''));

  document.getElementById('vac-tot').textContent=rows.length;

  if(!rows.length){
    tb.innerHTML='<tr><td colspan="7"><div class="es"><span class="ei">🏖</span><br>Sin registros — usa "Importar Trabajadores" para empezar.</div></td></tr>';
    document.getElementById('vac-neg').textContent='0';
    return;
  }

  let negCount=0;
  tb.innerHTML=rows.map(({tid,p})=>{
    const esBaja=(p.estado||'Activo')==='Baja';
    const anios=vacAniosCompletos(p.fecha_ingreso);
    const ganados=esBaja?0:diasGanadosAcumulados(anios);
    const gozados=esBaja?0:(vacaciones[tid]?.dias_gozados||0);
    const balance=esBaja?0:(ganados-gozados);
    if(!esBaja && balance<0) negCount++;
    return `<tr>
      <td style="font-weight:600;color:var(--text)">${esc(p.nombre||'—')}${esBaja?' <span class="badge b-toappr" style="margin-left:6px">Baja</span>':''}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${p.fecha_ingreso||'—'}</td>
      <td style="font-size:12px;color:var(--muted2)">${personalAntiguedad(p.fecha_ingreso)}</td>
      <td class="n" style="text-align:right;font-family:'DM Mono',monospace">${ganados}</td>
      <td class="n" style="text-align:right;font-family:'DM Mono',monospace">${gozados}</td>
      <td class="n" style="text-align:right;font-family:'DM Mono',monospace;font-weight:700;color:${balance<0?'var(--red)':'#1f8a4c'}">${balance}</td>
      <td>${esBaja?'—':'<button class="btn-reload" style="font-size:11px;padding:5px 12px" onclick="vacOpenAdd(\''+tid+'\')">+ Días</button>'}</td>
    </tr>`;
  }).join('');
  document.getElementById('vac-neg').textContent=negCount;
}

async function vacImport(){
  try{
    const r=await apiCall('POST','/vacaciones/import',{});
    if(r.error){toast(r.error,'er');return;}
    toast(r.added+' trabajador(es) agregados a Vacaciones ✓','ok');
    await loadVacaciones();
  }catch(e){toast('Error al importar: '+e,'er');}
}

function vacOpenAdd(tid){
  const p=personalRecords.find(x=>x.tid===tid); if(!p) return;
  vacCurrentTid=tid;
  document.getElementById('vac-add-nombre').textContent=p.nombre+' — actualmente '+(vacaciones[tid]?.dias_gozados||0)+' días gozados';
  document.getElementById('vac-add-dias').value='';
  document.getElementById('vac-add-nota').value='';
  document.getElementById('mo-vac-add').classList.add('on');
}

async function vacAddSave(){
  if(!vacCurrentTid) return;
  const dias=parseFloat(document.getElementById('vac-add-dias').value);
  if(!dias){ toast('Ingresa un número de días distinto de cero','er'); return; }
  const nota=document.getElementById('vac-add-nota').value.trim();
  try{
    const r=await apiCall('PUT','/vacaciones/'+vacCurrentTid,{dias,nota});
    if(r.error){toast(r.error,'er');return;}
    closeMo('mo-vac-add');
    toast('Días registrados ✓','ok');
    await loadVacaciones();
  }catch(e){toast('Error: '+e,'er');}
}
...
// ════════════════════════════════════════════════════════
let sueldosMask={}, salUnlockTid=null, salUnlockPassword=null;

async function loadSueldos(){
  const uma=document.getElementById('sal-uma')?.value||113.14;
  const isn_pct=document.getElementById('sal-isn')?.value||3;
  const prima_vac_pct=document.getElementById('sal-prima-vac')?.value||25;
  try{
    const qs=new URLSearchParams({uma,isn_pct,prima_vac_pct});
    sueldosMask=await(await fetch('/api/sueldos?'+qs.toString())).json();
  }catch(e){ sueldosMask={}; }
  salSueldosRender();
}

function salSueldosRender(){
  const tb=document.getElementById('sal-tb2'); if(!tb) return;
  const gs=(document.getElementById('sal-gs2')?.value||'').toLowerCase();
  const rows=Object.keys(sueldosMask).map(tid=>{
    const p=personalRecords.find(x=>x.tid===tid);
    return {tid, p, info:sueldosMask[tid]||{}};
  }).filter(r=>r.p && (!gs || (r.p.nombre||'').toLowerCase().includes(gs)))
    .sort((a,b)=>(a.p.nombre||'').localeCompare(b.p.nombre||''));

  document.getElementById('sal-tot2').textContent=rows.length;
  document.getElementById('sal-conf2').textContent=Object.values(sueldosMask).filter(v=>v&&v.has_salario).length;

  if(!rows.length){
    tb.innerHTML='<tr><td colspan="8"><div class="es"><span class="ei">💵</span><br>Sin registros — usa "Importar Trabajadores" para empezar.</div></td></tr>';
    return;
  }
  tb.innerHTML=rows.map(({tid,p,info})=>{
    const anios=vacAniosCompletos(p.fecha_ingreso);
    const diasVac=diasVacacionesPorLey(anios);
    const costoUsdTxt=info.costo_hora_usd!=null?('$'+info.costo_hora_usd.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})):'—';
    return `<tr>
      <td style="font-weight:600;color:var(--text)">${esc(p.nombre||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${p.fecha_ingreso||'—'}</td>
      <td style="font-size:12px;color:var(--muted2)">${personalAntiguedad(p.fecha_ingreso)}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace">${diasVac}</td>
...
async function salSueldosImport(){
  try{
    const r=await apiCall('POST','/sueldos/import',{});
    if(r.error){toast(r.error,'er');return;}
    toast(r.added+' trabajador(es) agregados ✓','ok');
    await loadSueldos();
  }catch(e){toast('Error al importar: '+e,'er');}
}

function salOpenUnlock(tid){
  salUnlockTid=tid; salUnlockPassword=null;
  const p=personalRecords.find(x=>x.tid===tid);
  document.getElementById('sal-unlock-title').textContent='Verificar Identidad';
  document.getElementById('sal-unlock-nombre').textContent='Para ver/editar el salario de '+(p?p.nombre:'—')+', ingresa tu contraseña de la Suite.';
  document.getElementById('sal-unlock-pass').value='';
  document.getElementById('sal-unlock-error').textContent='';
  document.getElementById('sal-unlock-step-pass').style.display='block';
  document.getElementById('sal-unlock-step-datos').style.display='none';
  const btn=document.getElementById('sal-unlock-btn');
  btn.textContent='Ver Salario →'; btn.onclick=salUnlockSubmit;
  document.getElementById('mo-sal-unlock').classList.add('on');
  setTimeout(()=>document.getElementById('sal-unlock-pass').focus(),200);
}

async function salUnlockSubmit(){
  const password=document.getElementById('sal-unlock-pass').value;
  if(!password){ document.getElementById('sal-unlock-error').textContent='Ingresa tu contraseña'; return; }
  const uma=document.getElementById('sal-uma').value;
  const isn_pct=document.getElementById('sal-isn').value;
  const prima_vac_pct=document.getElementById('sal-prima-vac').value;
  try{
    const r=await apiCall('POST','/sueldos/unlock',{tid:salUnlockTid,password,uma,isn_pct,prima_vac_pct});
    if(r.error){ document.getElementById('sal-unlock-error').textContent=r.error; return; }
    salUnlockPassword=password;
    const p=personalRecords.find(x=>x.tid===salUnlockTid);
    document.getElementById('sal-datos-nombre').textContent=p?p.nombre:'—';
    document.getElementById('sal-datos-monto').value=r.salario_mensual||'';
    document.getElementById('sal-costo-mxn').textContent=r.costo_hora_mxn?('$'+r.costo_hora_mxn.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})):'—';
    document.getElementById('sal-costo-usd').textContent=r.costo_hora_usd?('$'+r.costo_hora_usd.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})):'—';
    document.getElementById('sal-tipo-cambio-txt').textContent=r.tipo_cambio?('Tipo de cambio del día anterior usado: $'+r.tipo_cambio.toFixed(4)+' MXN/USD'):'Sin tipo de cambio disponible para ese día — captúralo en Control de Costo → Tipo de Cambio.';
    document.getElementById('sal-unlock-title').textContent='Salario — '+(p?p.nombre:'—');
    document.getElementById('sal-unlock-step-pass').style.display='none';
    document.getElementById('sal-unlock-step-datos').style.display='block';
    const btn=document.getElementById('sal-unlock-btn');
    btn.textContent='Guardar'; btn.onclick=salUnlockSave;
    salRenderHistorial(r.historial||[]);
  }catch(e){ document.getElementById('sal-unlock-error').textContent='Error: '+e; }
}

function salRenderHistorial(historial){
  const box=document.getElementById('sal-historial-list');
  if(!historial.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin cambios registrados todavía — este será el primer salario capturado.</div>'; return; }
  const fmt=v=>v==null?'— (sin capturar)':('$'+Number(v).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}));
  box.innerHTML=historial.map(h=>`
    <div style="background:var(--sb);border-radius:var(--r);padding:8px 12px;font-size:11px">
      <div style="display:flex;justify-content:space-between;align-items:center">
        <span>${fmt(h.salario_anterior)} → <b style="color:var(--gold)">${fmt(h.salario_nuevo)}</b></span>
        <span style="color:var(--muted);font-size:10px">${new Date(h.fecha).toLocaleString('es-MX')}</span>
      </div>
      <div style="color:var(--muted2);font-size:10px;margin-top:2px">Por ${esc(h.usuario||'—')}${h.origen?' · '+esc(h.origen):''}</div>
    </div>`).join('');
}

async function salUnlockSave(){
  const monto=parseFloat(document.getElementById('sal-datos-monto').value);
  if(!monto||monto<=0){ toast('Ingresa un salario válido','er'); return; }
  try{
    const r=await apiCall('PUT','/sueldos/'+salUnlockTid,{password:salUnlockPassword,salario_mensual:monto});
    if(r.error){ toast(r.error,'er'); return; }
    toast('Salario guardado ✓','ok');
    salUnlockClose();
    await loadSueldos();
  }catch(e){ toast('Error: '+e,'er'); }
}

function salUnlockClose(){
  closeMo('mo-sal-unlock');
  salUnlockTid=null; salUnlockPassword=null;
  document.getElementById('sal-unlock-pass').value='';
  document.getElementById('sal-datos-monto').value='';
  document.getElementById('sal-costo-mxn').textContent='—';
  document.getElementById('sal-costo-usd').textContent='—';
  document.getElementById('sal-historial-list').innerHTML='—';
}

// ── Importar Salarios desde Excel (protegido con contraseña) ──
let salImpFile=null;
function salOpenImportExcel(){
  salImpFile=null;
  document.getElementById('sal-imp-file').value='';
  document.getElementById('sal-imp-fname').textContent='—';
  document.getElementById('sal-imp-pass').value='';
  document.getElementById('sal-imp-error').textContent='';
  document.getElementById('sal-imp-results').style.display='none';
  document.getElementById('sal-imp-btn').disabled=true;
  document.getElementById('mo-sal-import-excel').classList.add('on');
}
function salImportExcelFile(inp){
  if(inp.files.length){
    salImpFile=inp.files[0];
    document.getElementById('sal-imp-fname').textContent=salImpFile.name;
    document.getElementById('sal-imp-btn').disabled=false;
  }
}
function salImportExcelDrop(e){
  e.preventDefault();
  document.getElementById('sal-imp-dz').classList.remove('dg');
  const f=e.dataTransfer.files[0];
  if(f){ salImpFile=f; document.getElementById('sal-imp-fname').textContent=f.name; document.getElementById('sal-imp-btn').disabled=false; }
}
async function salImportExcelSubmit(){
  const password=document.getElementById('sal-imp-pass').value;
  if(!salImpFile){ document.getElementById('sal-imp-error').textContent='Selecciona un archivo primero'; return; }
  if(!password){ document.getElementById('sal-imp-error').textContent='Ingresa tu contraseña'; return; }
  const btn=document.getElementById('sal-imp-btn'); btn.disabled=true; btn.textContent='Importando…';
  document.getElementById('sal-imp-error').textContent='';
  const fd=new FormData();
  fd.append('file',salImpFile);
  fd.append('password',password);
  try{
    const r=await fetch('/api/sueldos/import-excel',{method:'POST',body:fd});
    const d=await r.json();
    if(d.error){ document.getElementById('sal-imp-error').textContent=d.error; return; }
    let chips=`<div class="r-chip" style="background:rgba(39,174,96,.18);border:1px solid rgba(39,174,96,.5)"><div class="n" style="color:#1f8a4c;font-weight:700">${d.updated}</div><div class="l" style="color:#1f8a4c">Actualizados</div></div>`;
    if(d.no_encontrados && d.no_encontrados.length) chips+=`<div class="r-chip" style="background:rgba(200,16,46,.16);border:1px solid rgba(200,16,46,.4)"><div class="n" style="color:#a80d24;font-weight:700">${d.no_encontrados.length}</div><div class="l" style="color:#a80d24">No encontrados</div></div>`;
    if(d.invalidos && d.invalidos.length) chips+=`<div class="r-chip" style="background:rgba(243,156,18,.18);border:1px solid rgba(243,156,18,.4)"><div class="n" style="color:#a8650a;font-weight:700">${d.invalidos.length}</div><div class="l" style="color:#a8650a">Salario inválido</div></div>`;
    document.getElementById('sal-imp-chips').innerHTML=chips;
    document.getElementById('sal-imp-results').style.display='block';
    if((d.no_encontrados&&d.no_encontrados.length)||(d.invalidos&&d.invalidos.length)){
      document.getElementById('sal-imp-error').textContent=
        [d.no_encontrados&&d.no_encontrados.length?'No encontrados: '+d.no_encontrados.join(', '):'',
         d.invalidos&&d.invalidos.length?'Salario inválido: '+d.invalidos.join(', '):''].filter(Boolean).join(' — ');
      document.getElementById('sal-imp-error').style.color='var(--gold)';
    }
    toast(d.updated+' salario(s) actualizados ✓','ok',6000);
    await loadSueldos();
  }catch(e){ document.getElementById('sal-imp-error').textContent='Error: '+e; }
  finally{ btn.disabled=false; btn.textContent='Importar →'; }
}

// ── Exportar Listado a Excel (protegido con contraseña) ──
function salOpenExportExcel(){
  document.getElementById('sal-exp-pass').value='';
  document.getElementById('sal-exp-error').textContent='';
  document.getElementById('mo-sal-export-excel').classList.add('on');
  setTimeout(()=>document.getElementById('sal-exp-pass').focus(),200);
}
async function salExportExcelSubmit(){
  const password=document.getElementById('sal-exp-pass').value;
  if(!password){ document.getElementById('sal-exp-error').textContent='Ingresa tu contraseña'; return; }
  const uma=document.getElementById('sal-uma')?.value;
  const isn_pct=document.getElementById('sal-isn')?.value;
  const prima_vac_pct=document.getElementById('sal-prima-vac')?.value;
  const btn=document.getElementById('sal-exp-btn'); btn.disabled=true; btn.textContent='Generando…';
  try{
    const r=await fetch('/api/sueldos/export-excel',{
      method:'POST', headers:{'Content-Type':'application/json'},
      body:JSON.stringify({password,uma,isn_pct,prima_vac_pct})
    });
    if(!r.ok){
      const d=await r.json().catch(()=>({error:'Error al generar el archivo'}));
      document.getElementById('sal-exp-error').textContent=d.error||'Error al generar el archivo';
      return;
    }
    const blob=await r.blob();
    const url=window.URL.createObjectURL(blob);
    const a=document.createElement('a');
    a.href=url; a.download=`Sueldos_y_Salarios_${new Date().toISOString().slice(0,10)}.xlsx`;
    document.body.appendChild(a); a.click(); a.remove();
    window.URL.revokeObjectURL(url);
    closeMo('mo-sal-export-excel');
    toast('Excel descargado ✓','ok');
  }catch(e){ document.getElementById('sal-exp-error').textContent='Error: '+e; }
  finally{ btn.disabled=false; btn.textContent='Descargar →'; }
}

function salSwitchTab(tab){
  ['listado','isr'].forEach(t=>{
    const btn=document.getElementById('sal-tab-'+t);
    const on=t===tab;
    btn.style.background=on?'var(--red)':'rgba(0,0,0,.055)';
    btn.style.color=on?'#fff':'var(--muted)';
    document.getElementById('sal-content-'+t).style.display=on?'':'none';
  });
  if(tab==='isr') loadIsrTablas();
}

// ── Tablas de ISR (Quincenal / Mensual) ──
function isrPct(p){ return (p*100).toFixed(2)+'%'; }
function isrMoney(v){ return v==null?'—':'$'+Number(v).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}); }

async function loadIsrTablas(){
  try{
    const d=await(await fetch('/api/isr-tablas')).json();
    isrRenderTabla('isr-tb-quincenal', d.quincenal||[]);
    isrRenderTabla('isr-tb-mensual', d.mensual||[]);
    document.getElementById('isr-meta').textContent=d.actualizado_en
      ? `Última actualización: ${new Date(d.actualizado_en).toLocaleString('es-MX')} por ${d.actualizado_por||'—'}`
      : 'Sin tablas cargadas todavía.';
  }catch(e){ toast('Error al cargar tablas ISR: '+e,'er'); }
}

function isrRenderTabla(tbodyId, rows){
  const tb=document.getElementById(tbodyId);
  if(!rows.length){ tb.innerHTML='<tr><td colspan="4"><div class="es">Sin tabla cargada todavía.</div></td></tr>'; return; }
  tb.innerHTML=rows.map(r=>`<tr>
    <td style="font-family:'DM Mono',monospace">${isrMoney(r.limite_inferior)}</td>
    <td style="font-family:'DM Mono',monospace">${r.limite_superior==null?'En adelante':isrMoney(r.limite_superior)}</td>
    <td style="font-family:'DM Mono',monospace">${isrMoney(r.cuota_fija)}</td>
    <td style="font-family:'DM Mono',monospace">${isrPct(r.porcentaje)}</td>
  </tr>`).join('');
}

let isrImpFile=null;
function isrOpenImport(){
  isrImpFile=null;
  document.getElementById('isr-imp-file').value='';
  document.getElementById('isr-imp-fname').textContent='—';
  document.getElementById('isr-imp-error').textContent='';
  document.getElementById('isr-imp-btn').disabled=true;
  document.getElementById('mo-isr-import').classList.add('on');
}
function isrImportFile(inp){
  if(inp.files.length){
    isrImpFile=inp.files[0];
    document.getElementById('isr-imp-fname').textContent=isrImpFile.name;
    document.getElementById('isr-imp-btn').disabled=false;
  }
}
function isrImportDrop(e){
  e.preventDefault();
  document.getElementById('isr-imp-dz').classList.remove('dg');
  const f=e.dataTransfer.files[0];
  if(f){ isrImpFile=f; document.getElementById('isr-imp-fname').textContent=f.name; document.getElementById('isr-imp-btn').disabled=false; }
}
async function isrImportSubmit(){
  if(!isrImpFile){ document.getElementById('isr-imp-error').textContent='Selecciona un archivo primero'; return; }
  const btn=document.getElementById('isr-imp-btn'); btn.disabled=true; btn.textContent='Subiendo…';
  document.getElementById('isr-imp-error').textContent='';
  const fd=new FormData(); fd.append('file',isrImpFile);
  try{
    const r=await fetch('/api/isr-tablas/import',{method:'POST',body:fd});
    const d=await r.json();
    if(d.error){ document.getElementById('isr-imp-error').textContent=d.error; return; }
    toast(`Tablas actualizadas ✓ (Quincenal: ${d.quincenal} filas, Mensual: ${d.mensual} filas)`,'ok',6000);
    closeMo('mo-isr-import');
    await loadIsrTablas();
  }catch(e){ document.getElementById('isr-imp-error').textContent='Error: '+e; }
  finally{ btn.disabled=false; btn.textContent='Subir →'; }
}

document.addEventListener('DOMContentLoaded',()=>{ loadSueldos(); });

// ════════════════════════════════════════════════════════
//  RECURSOS HUMANOS — NÓMINA
// ════════════════════════════════════════════════════════
let npPeriodos=[], npCurrentPeriodo=null, npRecibos=[], npCurrentRecibo=null, npOtrasTemp=[];

async function loadNominaPeriodos(){
  try{ npPeriodos=await(await fetch('/api/nomina/periodos')).json(); }catch(e){ npPeriodos=[]; }
  npRenderPeriodosList();
  if(npCurrentPeriodo){
    const stillExists=npPeriodos.find(p=>p.id===npCurrentPeriodo.id);
    if(stillExists) npSelectPeriodo(stillExists.id); else { npCurrentPeriodo=null; document.getElementById('np-con-periodo').style.display='none'; document.getElementById('np-sin-periodo').style.display=''; }
  }
}

function npRenderPeriodosList(){
  const box=document.getElementById('np-periodos-list'); if(!box) return;
  if(!npPeriodos.length){ box.innerHTML='<div class="es" style="padding:16px 8px"><span style="font-size:11px;color:var(--muted)">Sin periodos todavía.</span></div>'; return; }
  box.innerHTML=npPeriodos.map(p=>`
    <div onclick="npSelectPeriodo('${p.id}')" style="cursor:pointer;padding:10px 12px;border-radius:var(--r);border:1px solid ${npCurrentPeriodo&&npCurrentPeriodo.id===p.id?'var(--red)':'var(--border)'};background:${npCurrentPeriodo&&npCurrentPeriodo.id===p.id?'rgba(200,16,46,.06)':'var(--card)'}">
      <div style="font-size:12px;font-weight:700;text-transform:capitalize">${p.tipo}</div>
      <div style="font-size:11px;color:var(--muted2);font-family:'DM Mono',monospace">${p.fecha_inicio} → ${p.fecha_fin}</div>
    </div>`).join('');
}

function npOpenNewPeriodo(){
  document.getElementById('np-tipo').value='quincenal';
  document.getElementById('np-fecha-inicio').value='';
  document.getElementById('np-fecha-fin').value='';
  document.getElementById('np-periodo-error').textContent='';
  document.getElementById('mo-np-periodo').classList.add('on');
}
async function npCreatePeriodo(){
  const tipo=document.getElementById('np-tipo').value;
  const fecha_inicio=document.getElementById('np-fecha-inicio').value;
  const fecha_fin=document.getElementById('np-fecha-fin').value;
  if(!fecha_inicio||!fecha_fin){ document.getElementById('np-periodo-error').textContent='Completa ambas fechas'; return; }
  try{
    const r=await apiCall('POST','/nomina/periodos',{tipo,fecha_inicio,fecha_fin});
    if(r.error){ document.getElementById('np-periodo-error').textContent=r.error; return; }
    closeMo('mo-np-periodo');
    toast('Periodo creado ✓','ok');
    await loadNominaPeriodos();
    npSelectPeriodo(r.id);
  }catch(e){ document.getElementById('np-periodo-error').textContent='Error: '+e; }
}
async function npDeletePeriodo(){
  if(!npCurrentPeriodo) return;
  if(!confirm(`¿Eliminar el periodo ${npCurrentPeriodo.fecha_inicio} → ${npCurrentPeriodo.fecha_fin}? Se borrarán todos sus recibos.`))return;
  try{
    await apiCall('DELETE','/nomina/periodos/'+npCurrentPeriodo.id);
    toast('Periodo eliminado','if');
    npCurrentPeriodo=null;
    document.getElementById('np-con-periodo').style.display='none';
    document.getElementById('np-sin-periodo').style.display='';
    await loadNominaPeriodos();
  }catch(e){ toast('Error: '+e,'er'); }
}

async function npSelectPeriodo(pid){
  npCurrentPeriodo=npPeriodos.find(p=>p.id===pid);
  if(!npCurrentPeriodo) return;
  npRenderPeriodosList();
  document.getElementById('np-sin-periodo').style.display='none';
  document.getElementById('np-con-periodo').style.display='';
  document.getElementById('np-periodo-titulo').textContent=`Periodo ${npCurrentPeriodo.tipo} — ${npCurrentPeriodo.fecha_inicio} → ${npCurrentPeriodo.fecha_fin}`;
  document.getElementById('np-periodo-meta').textContent=`Creado por ${npCurrentPeriodo.creado_por||'—'} el ${new Date(npCurrentPeriodo.creado_en).toLocaleString('es-MX')}`;
  await npLoadRecibos();
}

async function npLoadRecibos(){
  const tb=document.getElementById('np-recibos-tb');
  tb.innerHTML='<tr><td colspan="12"><div class="es"><div class="spinner"></div></div></td></tr>';
  try{
    npRecibos=await(await fetch('/api/nomina/periodos/'+npCurrentPeriodo.id+'/recibos')).json();
  }catch(e){ npRecibos=[]; }
  npRenderRecibos();
}

function npEstatusBadge(estatus){
  return estatus==='Timbrado' ? '<span class="badge b-yes">Timbrado</span>' : '<span class="badge b-wip">Borrador</span>';
}

function npRenderRecibos(){
  const tb=document.getElementById('np-recibos-tb');
  document.getElementById('np-kpi-tot').textContent=npRecibos.length;
  const totPerc=npRecibos.reduce((s,r)=>s+(r.total_percepciones||0),0);
  const totDed=npRecibos.reduce((s,r)=>s+(r.total_deducciones||0),0);
  const totNeto=npRecibos.reduce((s,r)=>s+(r.neto||0),0);
  document.getElementById('np-kpi-perc').textContent=clMoney(totPerc);
  document.getElementById('np-kpi-ded').textContent=clMoney(totDed);
  document.getElementById('np-kpi-neto').textContent=clMoney(totNeto);

  if(!npRecibos.length){
    tb.innerHTML='<tr><td colspan="12"><div class="es"><span class="ei">🧾</span><br>Sin recibos todavía — usa "🧮 Generar / Recalcular".</div></td></tr>';
    return;
  }
  tb.innerHTML=npRecibos.map(r=>{
    const otrasTotal=(r.otras_percepciones||[]).reduce((s,o)=>s+(o.monto||0),0);
    const vacTotal=(r.pago_vacaciones||0)+(r.prima_vacacional||0);
    const esTimbrado=r.estatus==='Timbrado';
    return `<tr>
      <td style="font-weight:600;color:var(--text)">${esc(r.nombre)}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${r.dias_trabajados}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${r.dias_vacaciones}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${clMoney(r.sueldo_normal)}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${clMoney(vacTotal)}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${clMoney(otrasTotal)}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace">${clMoney(r.horas_extra_monto)}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace;color:var(--red)">${clMoney(r.isr)}</td>
      <td style="text-align:right;font-family:'DM Mono',monospace;color:var(--red)">${clMoney(r.imss_obrero)}</td>
...
function npOpenGenerar(){
  document.getElementById('np-gen-pass').value='';
  document.getElementById('np-gen-error').textContent='';
  document.getElementById('mo-np-generar').classList.add('on');
}
async function npGenerarSubmit(){
  const password=document.getElementById('np-gen-pass').value;
  if(!password){ document.getElementById('np-gen-error').textContent='Ingresa tu contraseña'; return; }
  const uma=document.getElementById('np-gen-uma').value;
  const prima_vac_pct=document.getElementById('np-gen-prima-vac').value;
  const btn=document.getElementById('np-gen-btn'); btn.disabled=true; btn.textContent='Generando…';
  try{
    const r=await apiCall('POST','/nomina/periodos/'+npCurrentPeriodo.id+'/generar',{password,uma,prima_vac_pct});
    if(r.error){ document.getElementById('np-gen-error').textContent=r.error; return; }
    closeMo('mo-np-generar');
    let msg=`✅ ${r.generados} recibo(s) generados/actualizados`;
    if(r.omitidos_timbrados&&r.omitidos_timbrados.length) msg+=` — ${r.omitidos_timbrados.length} ya timbrado(s), sin tocar`;
    toast(msg,'ok',6000);
    await npLoadRecibos();
  }catch(e){ document.getElementById('np-gen-error').textContent='Error: '+e; }
  finally{ btn.disabled=false; btn.textContent='Generar →'; }
}

function npExportExcel(){
  if(!npCurrentPeriodo) return;
  window.open('/api/nomina/periodos/'+npCurrentPeriodo.id+'/export','_blank');
}

async function npTimbrarRecibo(rid){
  if(!confirm('¿Timbrar este recibo? Ya no podrás editarlo (solo un administrador podría revertirlo).'))return;
  try{
    const r=await apiCall('POST','/nomina/recibos/'+rid+'/timbrar',{});
    if(r.error){ toast(r.error,'er'); return; }
    toast('Recibo timbrado ✓','ok');
    await npLoadRecibos();
  }catch(e){ toast('Error: '+e,'er'); }
}
async function npRevertirRecibo(rid){
  if(!confirm('¿Revertir este recibo a Borrador?'))return;
  try{
    const r=await apiCall('POST','/nomina/recibos/'+rid+'/revertir',{});
    if(r.error){ toast(r.error,'er'); return; }
    toast('Recibo revertido a Borrador','if');
    await npLoadRecibos();
  }catch(e){ toast('Error: '+e,'er'); }
}

function npOpenRecibo(rid){
  const r=npRecibos.find(x=>x.id===rid); if(!r) return;
  npCurrentRecibo=r;
  npOtrasTemp=JSON.parse(JSON.stringify(r.otras_percepciones||[]));
  document.getElementById('np-rc-titulo').textContent='Recibo — '+r.nombre;
  document.getElementById('np-rc-horas-extra').value=r.horas_extra_monto||0;
  document.getElementById('np-rc-new-concepto').value='';
  document.getElementById('np-rc-new-monto').value='';
  npRenderOtrasList();
  npUpdateRecicoPreview();
  document.getElementById('mo-np-recibo').classList.add('on');
}
function npRenderOtrasList(){
  const box=document.getElementById('np-rc-otras-list');
  if(!npOtrasTemp.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin otras percepciones capturadas.</div>'; return; }
  box.innerHTML=npOtrasTemp.map((o,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:8px 12px">
...
function npAddOtraPercepcion(){
  const concepto=document.getElementById('np-rc-new-concepto').value.trim();
  const monto=parseFloat(document.getElementById('np-rc-new-monto').value);
  const gravable=document.getElementById('np-rc-new-gravable').value==='1';
  if(!concepto||!monto||monto<=0){ toast('Completa concepto y monto','er'); return; }
  npOtrasTemp.push({concepto,monto,gravable});
  document.getElementById('np-rc-new-concepto').value='';
  document.getElementById('np-rc-new-monto').value='';
  npRenderOtrasList();
  npUpdateRecicoPreview();
}
function npRemoveOtraPercepcion(i){
  npOtrasTemp.splice(i,1);
  npRenderOtrasList();
  npUpdateRecicoPreview();
}
function npUpdateRecicoPreview(){
  // Vista previa simple (no recalcula ISR exacto hasta guardar en servidor)
  const r=npCurrentRecibo;
  const horasExtra=parseFloat(document.getElementById('np-rc-horas-extra').value)||0;
  const otrasTotal=npOtrasTemp.reduce((s,o)=>s+(o.monto||0),0);
  const totalPerc=(r.sueldo_normal||0)+(r.pago_vacaciones||0)+(r.prima_vacacional||0)+horasExtra+otrasTotal;
  document.getElementById('np-rc-perc').textContent=clMoney(totalPerc);
  document.getElementById('np-rc-ded').textContent=clMoney(r.total_deducciones)+' (aprox.)';
  document.getElementById('np-rc-neto').textContent=clMoney(totalPerc-(r.total_deducciones||0))+' (aprox.)';
}
async function npSaveRecibo(){
  if(!npCurrentRecibo) return;
  const horas_extra_monto=parseFloat(document.getElementById('np-rc-horas-extra').value)||0;
  try{
    const r=await apiCall('PUT','/nomina/recibos/'+npCurrentRecibo.id,{horas_extra_monto,otras_percepciones:npOtrasTemp});
    if(r.error){ toast(r.error,'er'); return; }
    closeMo('mo-np-recibo');
    toast('Recibo actualizado ✓','ok');
    await npLoadRecibos();
  }catch(e){ toast('Error: '+e,'er'); }
}
document.addEventListener('input',(e)=>{
  if(e.target && e.target.id==='np-rc-horas-extra') npUpdateRecicoPreview();
});

document.addEventListener('DOMContentLoaded',()=>{ loadNominaPeriodos(); });

// ════════════════════════════════════════════════════════
//  RECURSOS HUMANOS — PERMISOS (solicitud + aprobación doble)
// ════════════════════════════════════════════════════════
let permisos=[], pmCurrentId=null;
...
function pmEstatusBadge(estatus){
  if(estatus==='Aprobado') return '<span class="badge b-yes">Aprobado</span>';
  if(estatus==='Pendiente Jefe Directo') return '<span class="badge b-wip">Pend. Jefe Directo</span>';
  if(estatus==='Pendiente RRHH') return '<span class="badge b-wip">Pend. RRHH</span>';
  return '<span class="badge b-toappr">'+esc(estatus)+'</span>'; // rechazos
}

function pmPeriodoTxt(pm){
  if(pm.modalidad==='horas') return `${pm.fecha} · ${pm.hora_inicio}–${pm.hora_fin}`;
  return `${pm.fecha_inicio} → ${pm.fecha_fin}`;
}
function pmCantidadTxt(pm){
  if(pm.modalidad==='horas') return pm.horas+(pm.horas===1?' hora':' horas');
  return pm.dias+(pm.dias===1?' día':' días');
}

function pmRender(){
  const tb=document.getElementById('pm-tb'); if(!tb) return;
  const gs=(document.getElementById('pm-gs')?.value||'').toLowerCase();
  const tipoF=document.getElementById('pm-tipo-flt')?.value||'';
  const estF=document.getElementById('pm-estatus-flt')?.value||'';

  const rows=permisos.map(pm=>{
    const p=personalRecords.find(x=>x.tid===pm.tid);
    return {pm, nombre:p?p.nombre:'(trabajador no encontrado)'};
  }).filter(r=>{
    if(tipoF && r.pm.tipo!==tipoF) return false;
    if(estF && r.pm.estatus!==estF) return false;
    if(gs && !r.nombre.toLowerCase().includes(gs)) return false;
    return true;
  }).sort((a,b)=>(b.pm.creado||'').localeCompare(a.pm.creado||''));

  document.getElementById('pm-tot').textContent=permisos.length;
  document.getElementById('pm-pend').textContent=permisos.filter(p=>p.estatus.startsWith('Pendiente')).length;

  if(!rows.length){
    tb.innerHTML='<tr><td colspan="7"><div class="es"><span class="ei">📝</span><br>Sin solicitudes de permiso todavía.</div></td></tr>';
    return;
  }
  tb.innerHTML=rows.map(({pm,nombre})=>{
    const jefeNombre=personalJefeName(personalRecords.find(x=>x.tid===pm.tid)?.jefe_directo);
    const origenTxt=pm.origen==='kiosco'?'📱 Kiosco':'💻 Suite';
    return `<tr class="${pm.id===pmCurrentId?'sel':''}" onclick="pmOpen('${pm.id}')">
      <td style="font-weight:600;color:var(--text)">${esc(nombre)}</td>
      <td style="font-size:11px;color:var(--muted2)">${esc(pm.tipo)}</td>
      <td style="font-family:'DM Mono',monospace;font-size:11px">${pmPeriodoTxt(pm)}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace;font-size:11px">${pmCantidadTxt(pm)}</td>
      <td>${pmEstatusBadge(pm.estatus)}</td>
      <td style="font-size:11px;color:var(--muted2)">${jefeNombre?esc(jefeNombre):'—'}</td>
      <td style="font-size:11px;color:var(--muted2)">${origenTxt}</td>
    </tr>`;
  }).join('');
}

function pmnSetModalidad(mod){
  document.getElementById('pmn-mod-dias').classList.toggle('on',mod==='dias');
  document.getElementById('pmn-mod-horas').classList.toggle('on',mod==='horas');
  document.getElementById('pmn-fields-dias').style.display=mod==='dias'?'flex':'none';
  document.getElementById('pmn-fields-horas').style.display=mod==='horas'?'flex':'none';
}

function pmOpenNew(){
  const sel=document.getElementById('pmn-tid');
  sel.innerHTML='<option value="">— Seleccionar —</option>'+personalRecords
    .filter(p=>(p.estado||'Activo')!=='Baja')
    .sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||''))
    .map(p=>`<option value="${p.tid}">${esc(p.nombre)}${p.puesto?' — '+esc(p.puesto):''}</option>`).join('');
  document.getElementById('pmn-tipo').value='Vacaciones';
  pmnSetModalidad('dias');
  document.getElementById('pmn-inicio').value='';
  document.getElementById('pmn-fin').value='';
  document.getElementById('pmn-fecha-h').value='';
  document.getElementById('pmn-hora-inicio').value='';
  document.getElementById('pmn-hora-fin').value='';
  document.getElementById('pmn-motivo').value='';
  document.getElementById('mo-pm-new').classList.add('on');
}

async function pmCreate(){
  const tid=document.getElementById('pmn-tid').value;
  const tipo=document.getElementById('pmn-tipo').value;
  const modalidad=document.getElementById('pmn-mod-horas').classList.contains('on')?'horas':'dias';
  const motivo=document.getElementById('pmn-motivo').value.trim();
  if(!tid){ toast('Elige un trabajador','er'); return; }

  const payload={tid,tipo,modalidad,motivo};
  if(modalidad==='dias'){
    payload.fecha_inicio=document.getElementById('pmn-inicio').value;
    payload.fecha_fin=document.getElementById('pmn-fin').value;
    if(!payload.fecha_inicio||!payload.fecha_fin){ toast('Completa las fechas','er'); return; }
  }else{
    payload.fecha=document.getElementById('pmn-fecha-h').value;
    payload.hora_inicio=document.getElementById('pmn-hora-inicio').value;
    payload.hora_fin=document.getElementById('pmn-hora-fin').value;
    if(!payload.fecha||!payload.hora_inicio||!payload.hora_fin){ toast('Completa la fecha y el rango de horas','er'); return; }
  }

  const btn=document.getElementById('btn-pmn-create'); btn.disabled=true; btn.textContent='Solicitando…';
  try{
    const r=await apiCall('POST','/permisos',payload);
    if(r.error){ toast(r.error,'er'); return; }
    closeMo('mo-pm-new');
    toast('Solicitud creada ✓','ok');
    await loadPermisos();
    setTimeout(()=>pmOpen(r.id),300);
  }catch(e){ toast('Error: '+e,'er'); }
  finally{ btn.disabled=false; btn.textContent='Solicitar'; }
}

function pmOpen(id){
  const pm=permisos.find(x=>x.id===id); if(!pm) return;
  pmCurrentId=id;
  const p=personalRecords.find(x=>x.tid===pm.tid);
  document.getElementById('pm-nombre').textContent=p?p.nombre:'(trabajador no encontrado)';
  document.getElementById('pm-id-txt').textContent=pm.id+' · '+pm.tipo;
  document.getElementById('pme-origen').textContent=(pm.origen==='kiosco'?'📱 Solicitado desde el Kiosco de Asistencia':'💻 Capturado en la Suite');

  document.getElementById('pme-tipo').value=pm.tipo;
  const esHoras=pm.modalidad==='horas';
  document.getElementById('pme-fields-dias').style.display=esHoras?'none':'flex';
  document.getElementById('pme-fields-horas').style.display=esHoras?'flex':'none';
  if(esHoras){
    document.getElementById('pme-fecha-h').value=pm.fecha||'';
    document.getElementById('pme-hora-inicio').value=pm.hora_inicio||'';
  ...
  document.getElementById('pme-motivo').value=pm.motivo||'';
  document.getElementById('pme-solicitante').textContent=pm.solicitado_por||'—';

  const soloLectura=pm.estatus!=='Pendiente Jefe Directo';
  ['pme-tipo','pme-inicio','pme-fin','pme-fecha-h','pme-hora-inicio','pme-hora-fin','pme-motivo'].forEach(id=>document.getElementById(id).disabled=soloLectura);

  document.getElementById('pm-estatus-badge').innerHTML=pmEstatusBadge(pm.estatus);
  document.getElementById('pm-jefe-nombre-ref').textContent='Jefe Directo: '+(personalJefeName(p?.jefe_directo)||'no asignado');

  const jefeF=pm.jefe_aprobacion;
  document.getElementById('pm-jefe-estado').innerHTML=jefeF
    ? (jefeF.decision==='aprobado'?'✅ Aprobado':'✕ Rechazado')+` por ${esc(jefeF.por)} · ${new Date(jefeF.fecha).toLocaleString('es-MX')}`+(jefeF.comentario?'<br><span style="font-size:11px;color:var(--muted2)">'+esc(jefeF.comentario)+'</span>':'')
    : 'Sin decisión todavía';
  document.getElementById('pm-jefe-botones').style.display=(pm.estatus==='Pendiente Jefe Directo')?'flex':'none';

  const rrhhF=pm.rrhh_aprobacion;
  document.getElementById('pm-rrhh-estado').innerHTML=rrhhF
    ? (rrhhF.decision==='aprobado'?'✅ Aprobado':'✕ Rechazado')+` por ${esc(rrhhF.por)} · ${new Date(rrhhF.fecha).toLocaleString('es-MX')}`+(rrhhF.comentario?'<br><span style="font-size:11px;color:var(--muted2)">'+esc(rrhhF.comentario)+'</span>':'')
    : (pm.estatus==='Pendiente Jefe Directo' ? 'Esperando aprobación del Jefe Directo' : 'Sin decisión todavía');
  document.getElementById('pm-rrhh-botones').style.display=(pm.estatus==='Pendiente RRHH')?'flex':'none';

  pmStab('det',document.querySelector('#pm-panel .ptab'));
  openPanel('pm-panel');
  pmRender();
}

function pmStab(name,btn){
  document.querySelectorAll('#pm-panel .ptab').forEach(b=>b.classList.remove('on'));
  document.querySelectorAll('#pm-panel .tc2').forEach(c=>c.classList.remove('on'));
  if(btn) btn.classList.add('on');
  else document.querySelectorAll('#pm-panel .ptab').forEach(b=>{if(b.getAttribute('onclick').includes("'"+name+"'"))b.classList.add('on');});
  document.getElementById('pmtab-'+name).classList.add('on');
  if(name==='doc'&&pmCurrentId) pmLoadFiles(pmCurrentId);
}

async function pmSave(){
  if(!pmCurrentId) return;
  const pm=permisos.find(x=>x.id===pmCurrentId); if(!pm) return;
  const esHoras=pm.modalidad==='horas';
  const payload={
    tipo:document.getElementById('pme-tipo').value,
    modalidad:pm.modalidad,
    motivo:document.getElementById('pme-motivo').value.trim(),
  };
  if(esHoras){
    payload.fecha=document.getElementById('pme-fecha-h').value;
    payload.hora_inicio=document.getElementById('pme-hora-inicio').value;
    payload.hora_fin=document.getElementById('pme-hora-fin').value;
  }else{
    payload.fecha_inicio=document.getElementById('pme-inicio').value;
    payload.fecha_fin=document.getElementById('pme-fin').value;
  }
  try{
    const r=await apiCall('PUT','/permisos/'+pmCurrentId,payload);
    if(r.error){ toast(r.error,'er'); return; }
    toast('Guardado ✓','ok');
    await loadPermisos();
    pmOpen(pmCurrentId);
...
async function pmDelete(){
  if(!pmCurrentId) return;
  const pm=permisos.find(x=>x.id===pmCurrentId); if(!pm) return;
  if(!confirm('¿Eliminar esta solicitud de permiso? Esta acción no se puede deshacer.'))return;
  try{
    await apiCall('DELETE','/permisos/'+pmCurrentId);
    closePanel();
    toast('Solicitud eliminada','if');
    await loadPermisos();
  }catch(e){ toast('Error: '+e,'er'); }
}

async function pmDecidir(etapa,decision){
  if(!pmCurrentId) return;
  let comentario='';
  ...
  try{
    const r=await apiCall('POST','/permisos/'+pmCurrentId+'/decidir',{etapa,decision,comentario});
    if(r.error){ toast(r.error,'er'); return; }
    toast(decision==='aprobado'?'Aprobado ✓':'Rechazado','ok');
    await loadPermisos();
    pmOpen(pmCurrentId);
  }catch(e){ toast('Error: '+e,'er'); }
}
...
async function pmLoadFiles(pid){
  const fl=document.getElementById('pm-fl');
  document.getElementById('pm-dp').textContent=pid;
  fl.innerHTML='<div class="es"><div class="spinner"></div></div>';
  try{
...
async function pmUploadFiles(fileList){
  ...
  const fd=new FormData(); Array.from(fileList).forEach(f=>fd.append('files',f));
  try{
    const res=await fetch('/api/permisos/upload/'+pmCurrentId,{method:'POST',body:fd});
    const data=await res.json(); if(data.error)throw new Error(data.error);
    toast(data.saved.length+' archivo(s) guardados','ok'); await pmLoadFiles(pmCurrentId);
  }catch(e){toast('Error al subir: '+e,'er');}
  document.querySelector('#pm-dz input').value='';
}
function pmDropFiles(e){e.preventDefault();document.getElementById('pm-dz').classList.remove('dg');pmUploadFiles(e.dataTransfer.files);}
async function pmDelFile(pid,filename){
  if(!confirm('¿Eliminar "'+filename+'"?'))return;
  try{const r=await(await fetch('/api/permisos/files/'+pid+'/'+encodeURIComponent(filename),{method:'DELETE'})).json();if(r.error){toast(r.error,'er');return;}toast(filename+' eliminado','if');await pmLoadFiles(pid);}catch(e){toast('Error: '+e,'er');}
}
...
async function asisCheckStatus(){
  const dot=document.getElementById('asis-dot'), txt=document.getElementById('asis-status-txt');
  if(!dot) return;
  try{
    const s=await(await fetch('/api/asistencia/status')).json();
    if(!s.configured){ dot.className='conn-dot er'; txt.textContent='No configurado (falta ATTENDANCE_URL en el servidor)'; return; }
    if(s.connected){ dot.className='conn-dot ok'; txt.textContent='Conectado al kiosco de asistencia'; }
    else { dot.className='conn-dot er'; txt.textContent='Sin conexión con el kiosco de asistencia'; }
  }catch(e){ dot.className='conn-dot er'; txt.textContent='Error al verificar conexión'; }
}

function asisDefaultDates(){
  const to=document.getElementById('asis-to'), from=document.getElementById('asis-from');
  if(to && !to.value){ const d=new Date(); to.value=d.toISOString().slice(0,10); }
  if(from && !from.value){ const d=new Date(); d.setDate(d.getDate()-7); from.value=d.toISOString().slice(0,10); }
}

async function asisLoadRecords(){
  const tb=document.getElementById('asis-tb'); if(!tb) return;
  asisDefaultDates();
  tb.innerHTML='<tr><td colspan="8"><div class="es"><div class="spinner"></div></div></td></tr>';
  const from=document.getElementById('asis-from').value, to=document.getElementById('asis-to').value;
  try{
    const qs=new URLSearchParams(); if(from)qs.set('from',from); if(to)qs.set('to',to);
    const r=await fetch('/api/asistencia/records?'+qs.toString());
    const d=await r.json();
    if(d.error){ tb.innerHTML='<tr><td colspan="8"><div class="es"><span class="ei">⚠</span><br>'+esc(d.error)+'</div></td></tr>'; asisRecords=[]; return; }
    asisRecords=d;
    asisRenderTable();
  }catch(e){
    tb.innerHTML='<tr><td colspan="8"><div class="es"><span class="ei">⚠</span><br>Error al cargar registros</div></td></tr>';
  }
}

function asisRenderTable(){
  const tb=document.getElementById('asis-tb'); if(!tb) return;
  const gs=(document.getElementById('asis-gs')?.value||'').toLowerCase();
  const rows=asisRecords.filter(r=>!gs || (r.nombre||'').toLowerCase().includes(gs));
  document.getElementById('asis-tot').textContent=asisRecords.length;
  document.getElementById('asis-sinvincular').textContent=asisRecords.filter(r=>!r.vinculado).length;

  if(!rows.length){
    tb.innerHTML='<tr><td colspan="8"><div class="es"><span class="ei">🗓</span><br>Sin registros en este rango de fechas</div></td></tr>';
    return;
  }
  tb.innerHTML=rows.map(r=>{
    ...
    const tipoBadge=r.tipo==='entrada'?'<span class="badge b-yes">Entrada</span>':'<span class="badge b-open">Salida</span>';
    const geoBadge=r.en_geocerca?'<span class="badge b-yes">✓ Dentro</span>':'<span class="badge b-toappr">Fuera</span>';
    const nombreTxt=esc(r.nombre)+(r.vinculado?'':' <span class="badge b-wip" title="No se encontró en Control de Personal">sin vincular</span>');
    return `<tr>
      <td style="font-weight:600;color:var(--text)">${nombreTxt}</td>
      <td style="font-size:11px;color:var(--muted2)">${esc(r.puesto||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${fecha}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${hora}</td>
      <td>${tipoBadge}</td>
      <td style="font-size:11px;color:var(--muted2)">${esc(r.ubicacion||'—')}</td>
      <td>${geoBadge}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${r.horas_jornada||'—'}</td>
    </tr>`;
  }).join('');
}

async function asisSyncWorkers(){
  try{
    const r=await apiCall('POST','/asistencia/sync-workers',{});
    if(r.error){toast(r.error,'er');return;}
    toast(`Sincronizado ✓ — ${r.created} nuevos, ${r.updated} actualizados, ${r.deactivated} dados de baja`,'ok',6000);
  }catch(e){toast('Error al sincronizar: '+e,'er');}
}

// ── Vincular manualmente trabajadores que ya existían en el kiosco ──
async function asisOpenLink(){
  document.getElementById('mo-asis-link').classList.add('on');
  const box=document.getElementById('asis-link-list');
  box.innerHTML='<div class="es"><div class="spinner"></div></div>';
  try{
    const kw=await(await fetch('/api/asistencia/workers')).json();
    if(kw.error){ box.innerHTML='<div class="es"><span class="ei">⚠</span><br>'+esc(kw.error)+'</div>'; return; }
    asisRenderLinkList(kw);
  }catch(e){ box.innerHTML='<div class="es"><span class="ei">⚠</span><br>Error al cargar</div>'; }
}

function asisRenderLinkList(kioskWorkers){
  const box=document.getElementById('asis-link-list');
  const unlinked=kioskWorkers.filter(w=>!w.externalId);
  if(!unlinked.length){ box.innerHTML='<div class="es"><span class="ei">✅</span><br>Todos los trabajadores del kiosco ya están vinculados.</div>'; return; }
  const activePersonal=personalRecords.filter(p=>(p.estado||'Activo')!=='Baja').sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||''));
  const opts='<option value="">— Elegir trabajador —</option>'+activePersonal.map(p=>`<option value="${p.tid}">${esc(p.nombre)}${p.puesto?' — '+esc(p.puesto):''}</option>`).join('');
  box.innerHTML=unlinked.map(w=>`<div style="display:flex;align-items:center;gap:8px;padding:9px 0;border-bottom:1px solid var(--border)">
    <div style="flex:1;font-size:12px"><b>${esc(w.name)}</b><br><span style="color:var(--muted);font-size:10px">${esc(w.position||'—')}</span></div>
    <select id="asis-link-sel-${w.id}" style="flex:1;background:var(--inp);border:1px solid var(--border);border-radius:4px;padding:6px 8px;font-size:11px;color:var(--text)">${opts}</select>
    <button class="btn-reload" style="font-size:11px;padding:6px 12px;flex-shrink:0" onclick="asisLinkSave('${w.id}')">Vincular</button>
  </div>`).join('');
}

async function asisLinkSave(kioskId){
  const sel=document.getElementById('asis-link-sel-'+kioskId);
  const tid=sel.value;
  if(!tid){ toast('Elige un trabajador','er'); return; }
  try{
    const r=await apiCall('POST','/asistencia/link-worker',{kiosk_worker_id:kioskId, tid});
    if(r.error){ toast(r.error,'er'); return; }
    toast('Vinculado ✓','ok');
    await asisOpenLink();
    await asisLoadRecords();
  }catch(e){ toast('Error: '+e,'er'); }
}

function asisSwitchTab(tab){
  ['reg','ctrl'].forEach(t=>{
    const btn=document.getElementById('asis-tab-'+t);
    const on=t===tab;
    btn.style.background=on?'var(--red)':'rgba(0,0,0,.055)';
    btn.style.color=on?'#fff':'var(--muted)';
    document.getElementById('asis-content-'+t).style.display=on?'':'none';
  });
  if(tab==='ctrl') ctrlInit();
}

// ════════════════════════════════════════════════════════
//  CONTROL DE HORAS (distribución de horas por Job)
// ════════════════════════════════════════════════════════
let ctrlMode='week', ctrlReportRows=[], ctrlReportKey='', ctrlFirmas={}, ctrlExportado=null;
...
function ctrlInit(){
  const wsel=document.getElementById('ctrl-worker');
  if(wsel){
    const cur=wsel.value;
    wsel.innerHTML='<option value="">Todos</option>'+personalRecords
      .filter(p=>(p.estado||'Activo')!=='Baja')
      .sort((a,b)=>(a.nombre||'').localeCompare(b.nombre||''))
      .map(p=>`<option value="${p.tid}">${esc(p.nombre)}</option>`).join('');
    wsel.value=cur;
  }
  if(ctrlInited) return;
  ctrlInited=true;
  const now=new Date();
  document.getElementById('ctrl-week-year').value=now.getFullYear();
  document.getElementById('ctrl-month-year').value=now.getFullYear();
  document.getElementById('ctrl-year-val').value=now.getFullYear();
  const curWeek=getISOWeek(now);
  document.getElementById('ctrl-week-from').value=curWeek;
  document.getElementById('ctrl-week-to').value=curWeek;
  document.getElementById('ctrl-month').value=now.getMonth()+1;
}

function ctrlSetMode(mode){
  ctrlMode=mode;
  ['week','month','year'].forEach(m=>{
    document.getElementById('ctrl-tab-'+m).classList.toggle('on',m===mode);
    document.getElementById('ctrl-fields-'+m).style.display=m===mode?'flex':'none';
  });
  document.getElementById('ctrl-report-wrap').style.display='none';
}
...
function ctrlDateRange(){
  ...
  let from,to,label;
  if(ctrlMode==='week'){
    const wf=parseInt(document.getElementById('ctrl-week-from').value);
    const wt=parseInt(document.getElementById('ctrl-week-to').value);
    const year=parseInt(document.getElementById('ctrl-week-year').value)||now.getFullYear();
    ...
    from=monFrom.toISOString().slice(0,10); to=sunTo.toISOString().slice(0,10);
    label=wf===wt?`Semana ${wf} / ${year}`:`Semanas ${Math.min(wf,wt)}–${Math.max(wf,wt)} / ${year}`;
  }else if(ctrlMode==='month'){
    const month=parseInt(document.getElementById('ctrl-month').value);
    const year=parseInt(document.getElementById('ctrl-month-year').value)||now.getFullYear();
    ...
    const year=parseInt(document.getElementById('ctrl-year-val').value)||now.getFullYear();
    from=`${year}-01-01`; to=`${year}-12-31`; label=`Año ${year}`;
  }
  return {from,to,label};
}

async function ctrlLoadReport(){
  const range=ctrlDateRange();
  if(!range){ toast('Completa los filtros de fecha','er'); return; }
  const tid=document.getElementById('ctrl-worker').value;
  const job=document.getElementById('ctrl-job').value.trim().toUpperCase();
  ...
  try{
    const r=await fetch('/api/asistencia/control-horas?'+qs.toString());
    const d=await r.json();
    if(d.error){ toast(d.error,'er'); return; }
    ctrlReportRows=d.rows; ctrlReportKey=d.reportKey; ctrlFirmas=d.firmas||{}; ctrlExportado=d.exportado||null;

    const workerLabel=tid?(personalRecords.find(p=>p.tid===tid)?.nombre||''):'Todos';
    const jobLabel=job?` · Job: ${job}`:'';
    document.getElementById('ctrl-title').textContent=`${range.label} — ${workerLabel}${jobLabel}`;

    const totalHoras=Math.round(ctrlReportRows.reduce((s,r)=>s+(r.horas||0),0)*100)/100;
    document.getElementById('ctrl-total-horas').textContent=totalHoras;
    document.getElementById('ctrl-total-jobs').textContent=new Set(ctrlReportRows.map(r=>r.job)).size;
    document.getElementById('ctrl-total-trab').textContent=new Set(ctrlReportRows.map(r=>r.nombre)).size;

    const tb=document.getElementById('ctrl-tb');
    tb.innerHTML=ctrlReportRows.length?ctrlReportRows.map(row=>`<tr>
      <td style="font-weight:600">${esc(row.nombre)}</td>
      <td style="font-family:'DM Mono',monospace;font-size:11px">${row.timestamp?row.timestamp.slice(0,10):'—'}</td>
      <td><span class="badge b-wip">${esc(row.job)}</span></td>
      <td class="n" style="text-align:right;font-family:'DM Mono',monospace;font-weight:600">${row.horas}</td>
      <td style="font-size:11px;color:var(--muted2)">${esc(row.area||'—')}</td>
    </tr>`).join(''):'<tr><td colspan="5"><div class="es">Sin registros con distribución de jobs en el período</div></td></tr>';

    const tf=document.getElementById('ctrl-tf');
    if(ctrlReportRows.length){
      const jobTotals={};
      ctrlReportRows.forEach(r=>{ jobTotals[r.job]=Math.round(((jobTotals[r.job]||0)+r.horas)*100)/100; });
      const summary=Object.entries(jobTotals).sort((a,b)=>b[1]-a[1]).map(([j,h])=>`<span class="badge b-wip" style="margin:2px">${j}: ${h}h</span>`).join('');
      tf.innerHTML=`<tr><td colspan="3" style="font-weight:700;text-align:right">TOTAL</td><td class="n" style="text-align:right;font-weight:800;font-size:15px">${totalHoras} h</td><td></td></tr>
        <tr><td colspan="5" style="padding:8px 10px">${summary}</td></tr>`;
    } else tf.innerHTML='';

    ctrlUpdateSigBoxes();

    const sigSection=document.getElementById('ctrl-sig-section');
    const noAuthNote=document.getElementById('ctrl-noauth-note');
    const badge=document.getElementById('ctrl-auth-badge');
    const exportSection=document.getElementById('ctrl-export-section');
    const exportReady=document.getElementById('ctrl-export-ready');
    const exportDone=document.getElementById('ctrl-export-done');
    let isAuth=false;
    if(ctrlMode==='week'){
      sigSection.style.display=''; noAuthNote.style.display='none';
      isAuth=!!(ctrlFirmas.gerencia && ctrlFirmas.rrhh);
      const isPartial=!isAuth && !!(ctrlFirmas.gerencia || ctrlFirmas.rrhh);
      badge.innerHTML=isAuth
        ? '<span class="badge b-yes">✅ Autorizado</span>'
        : isPartial
          ? '<span class="badge b-wip">⚠ Parcial</span>'
          : '<span class="badge b-toappr">✗ Sin autorizar</span>';
    }else{
      sigSection.style.display='none'; noAuthNote.style.display='';
      badge.innerHTML='<span class="badge b-nometa">No aplica</span>';
    }

    if(ctrlMode==='week' && isAuth){
      exportSection.style.display='';
      if(ctrlExportado){
        exportReady.style.display='none'; exportDone.style.display='';
        document.getElementById('ctrl-export-done-txt').textContent=
          `✅ Ya exportada a Control de Costo el ${new Date(ctrlExportado.fecha).toLocaleString('es-MX')} por ${ctrlExportado.exportado_por} (${ctrlExportado.registros} registros).`;
        document.getElementById('ctrl-btn-revocar-export').style.display=(USER_PERMS&&USER_PERMS.is_admin)?'':'none';
      }else{
        exportReady.style.display=''; exportDone.style.display='none';
      }
    }else{
      exportSection.style.display='none';
    }

    document.getElementById('ctrl-report-wrap').style.display='';
    toast(`${ctrlReportRows.length} registros cargados ✓`,'ok');
  }catch(e){ toast('Error al generar reporte: '+e,'er'); }
}

function ctrlOpenExport(){
  if(!ctrlReportKey){ toast('Genera el reporte primero','er'); return; }
  const range=ctrlDateRange();
  const totalHoras=Math.round(ctrlReportRows.reduce((s,r)=>s+(r.horas||0),0)*100)/100;
  document.getElementById('ctrl-export-confirm-txt').textContent=
    `Vas a exportar ${ctrlReportRows.length} registro(s), ${totalHoras} horas totales, de "${range?range.label:ctrlReportKey}" hacia Mano de Obra (Work Hours).`;
  document.getElementById('ctrl-export-pass').value='';
  document.getElementById('ctrl-export-error').textContent='';
  document.getElementById('mo-ctrl-export').classList.add('on');
  setTimeout(()=>document.getElementById('ctrl-export-pass').focus(),200);
}

async function ctrlConfirmExport(){
  const password=document.getElementById('ctrl-export-pass').value;
  if(!password){ document.getElementById('ctrl-export-error').textContent='Ingresa tu contraseña'; return; }
  const range=ctrlDateRange();
  const tid=document.getElementById('ctrl-worker').value;
  const job=document.getElementById('ctrl-job').value.trim().toUpperCase();
  const uma=document.getElementById('sal-uma')?.value;
  ...
  const btn=document.getElementById('ctrl-export-btn'); btn.disabled=true; btn.textContent='Exportando…';
  try{
    const r=await apiCall('POST','/asistencia/control-horas/exportar-mano-obra',{
      reportKey:ctrlReportKey, from:range.from, to:range.to, tid:tid||null, job:job||null,
      password, uma, isn_pct, prima_vac_pct,
    });
    if(r.error){ document.getElementById('ctrl-export-error').textContent=r.error; return; }
    closeMo('mo-ctrl-export');
    let msg=`✅ ${r.added} registro(s) exportados a Mano de Obra (USD, tipo de cambio $${(r.tipo_cambio||0).toFixed(4)})`;
    if(r.sin_salario && r.sin_salario.length) msg+=` — ⚠ sin salario capturado: ${r.sin_salario.join(', ')} (costo $0)`;
    toast(msg,'ok',7000);
    await ctrlLoadReport();
  }catch(e){ document.getElementById('ctrl-export-error').textContent='Error: '+e; }
  finally{ btn.disabled=false; btn.textContent='OK, Exportar'; }
}

async function ctrlRevocarExport(){
  if(!ctrlReportKey) return;
  if(!confirm('¿Revocar esta exportación?\n\nSe eliminarán de Mano de Obra (Work Hours) los registros que se crearon con esta exportación, y la semana quedará disponible para volver a exportarse.\n\nEsta acción no se puede deshacer.'))return;
  try{
    const r=await apiCall('POST','/asistencia/control-horas/revocar-exportacion',{reportKey:ctrlReportKey});
    if(r.error){ toast(r.error,'er'); return; }
    toast(`Exportación revocada — ${r.removed} registro(s) eliminados de Mano de Obra`,'ok',6000);
    await ctrlLoadReport();
  }catch(e){ toast('Error: '+e,'er'); }
}

function ctrlUpdateSigBoxes(){
  ['gerencia','rrhh'].forEach(tipo=>{
    const f=ctrlFirmas[tipo];
    const txt=document.getElementById('ctrl-sig-'+tipo+'-txt');
    const btnF=document.getElementById('ctrl-btn-firmar-'+tipo);
    const btnR=document.getElementById('ctrl-btn-revocar-'+tipo);
    if(f){
      txt.innerHTML=`✅ ${esc(f.firmante)}<br><span style="font-size:10px;color:var(--muted);font-weight:400">${new Date(f.fecha).toLocaleString('es-MX')}</span>`;
      btnF.style.display='none'; btnR.style.display='';
    }else{
      txt.textContent='Sin firmar';
      btnF.style.display=''; btnR.style.display='none';
    }
  });
}

async function ctrlFirmar(tipo){
  if(!ctrlReportKey){ toast('Genera el reporte primero','er'); return; }
  try{
    const r=await apiCall('POST','/asistencia/control-horas/firmar',{reportKey:ctrlReportKey, tipo});
    if(r.error){ toast(r.error,'er'); return; }
    ctrlFirmas=r.firmas ? {...ctrlFirmas, [tipo]:r.firmas[tipo]} : ctrlFirmas;
    ctrlFirmas[tipo]=r.firmas[tipo];
    ctrlUpdateSigBoxes();
    toast('Firmado ✓','ok');
  }catch(e){ toast('Error: '+e,'er'); }
}

async function ctrlRevocar(tipo){
  if(!confirm('¿Revocar esta firma?'))return;
  try{
    await apiCall('POST','/asistencia/control-horas/revocar',{reportKey:ctrlReportKey, tipo});
    delete ctrlFirmas[tipo];
    ctrlUpdateSigBoxes();
    toast('Firma revocada','if');
  }catch(e){ toast('Error: '+e,'er'); }
}

function ctrlExport(){
  const range=ctrlDateRange(); if(!range) return;
  const tid=document.getElementById('ctrl-worker').value;
  const job=document.getElementById('ctrl-job').value.trim().toUpperCase();
  const qs=new URLSearchParams({from:range.from, to:range.to});
  if(tid) qs.set('tid',tid);
  if(job) qs.set('job',job);
  window.open('/api/asistencia/control-horas/export?'+qs.toString(),'_blank');
}

document.addEventListener('DOMContentLoaded',()=>{ asisCheckStatus(); asisLoadRecords(); });

// ════════════════════════════════════════════════════════
//  OPERACIONES — CAPACIDAD (planeación de mano de obra)
// ════════════════════════════════════════════════════════
let capacidadData={};
...
function capFmtShort(d){ const meses=['ene','feb','mar','abr','may','jun','jul','ago','sep','oct','nov','dic']; return d.getDate()+' '+meses[d.getMonth()]+' '+d.getFullYear(); }

async function loadCapacidad(){
  try{
    const d=await(await fetch('/api/capacidad')).json();
    capacidadData=(d && !d.error && typeof d==='object')?d:{};
    if(d && d.error){
      document.getElementById('cap-dot').className='conn-dot er';
      document.getElementById('cap-lbl').textContent=d.error;
      capRender();
      return;
    }
    ...
    capacidadData={};
    document.getElementById('cap-dot').className='conn-dot er';
    document.getElementById('cap-lbl').textContent='Error de conexión';
    toast('Error al cargar Capacidad: '+e,'er');
  }
}

function permCanCreate(mod){
  if(!USER_PERMS) return true; // aún no cargan los permisos (o no aplica): no bloquear de más
  if(USER_PERMS.is_admin) return true;
  const lvl=(USER_PERMS.permissions||{})[mod]||'none';
  return lvl==='create'||lvl==='full';
}

function capRenderAreaTable(groupName, workers, days){
  const dayHdr=days.map(d=>`<th class="cap-th-day${d.isWeekend?' cap-weekend':''}${d.isWeekStart?' cap-sep':''}">${d.label}</th>`).join('');
  const dateHdr=days.map(d=>`<th class="cap-th-date${d.isWeekend?' cap-weekend':''}${d.isWeekStart?' cap-sep':''}">${d.shortDate}</th>`).join('');
  const rows=workers.map(w=>{
    const cells=days.map(d=>{
      const val=(capacidadData[w.tid]&&capacidadData[w.tid][d.str])||'';
      return `<td class="${d.isWeekend?'cap-weekend':''}${d.isWeekStart?' cap-sep':''}"><input class="cap-input" value="${esc(val)}" onchange="capSaveCell('${w.tid}','${d.str}',this.value,this)" onpaste="capOnPaste(event)"></td>`;
    }).join('');
    return `<tr><td class="cap-td-name">${esc(w.nombre)}</td>${cells}</tr>`;
  }).join('');
  return `<div class="cap-area-block">
    <div class="cap-area-title">🏢 ${esc(groupName)} <span class="n">(${workers.length} trabajador${workers.length===1?'':'es'})</span></div>
    <div class="cap-scroll">
      <table class="cap-table">
        <thead>
          <tr><th class="cap-th-name">Trabajador</th>${dayHdr}</tr>
          <tr><th class="cap-th-name"></th>${dateHdr}</tr>
        </thead>
        <tbody>${rows}</tbody>
      </table>
    </div>
...
function capRender(){
  const wrap=document.getElementById('cap-wrap'); if(!wrap) return;
  const days=capDays();
  document.getElementById('cap-range').textContent=capFmtShort(days[0].date)+' → '+capFmtShort(days[days.length-1].date)+' (6 semanas)';

  const groups=[];
  areasList.forEach(a=>{
    const workers=personalRecords.filter(w=>w.area===a.nombre).sort((x,y)=>(x.nombre||'').localeCompare(y.nombre||''));
    if(workers.length) groups.push([a.nombre, workers]);
  });
  const sinArea=personalRecords.filter(w=>!w.area||!areasList.some(a=>a.nombre===w.area)).sort((x,y)=>(x.nombre||'').localeCompare(y.nombre||''));
  if(sinArea.length) groups.push(['Sin área asignada', sinArea]);

  if(!groups.length){
    const diag=`
      <div style="text-align:left;background:var(--sb);border-radius:8px;padding:12px 16px;margin-top:14px;font-family:'DM Mono',monospace;font-size:11px;max-width:520px">
        <div><b>/api/personal</b> → ${esc(personalLoadError||'OK, '+personalRecords.length+' registro(s)')}</div>
        <div style="margin-top:4px"><b>/api/areas</b> → ${esc(areasLoadError||'OK, '+areasList.length+' registro(s)')}</div>
      </div>`;
    wrap.innerHTML=`<div class="es"><div class="ei">${personalLoadError?'🔒':'👥'}</div>
      <p>${personalLoadError?'No se pudo cargar el listado de trabajadores.<br>Puede que tu usuario no tenga acceso a Control de Personal — contacta al administrador.':'No hay trabajadores registrados todavía.<br>Ve a Recursos Humanos → Control de Personal → Listado de Trabajadores.'}</p>
      ${diag}
    </div>`;
    return;
  }
  wrap.innerHTML=groups.map(([name,workers])=>capRenderAreaTable(name,workers,days)).join('');
  capApplyColors();
...
let capLegendRows=[], capOsListCache=[];

async function loadCapLegend(){
  try{
    const d=await(await fetch('/api/capacidad/codigos')).json();
    capLegendRows=Array.isArray(d)?d:[];
  }catch(e){ capLegendRows=[]; }
  try{
    const osd=await(await fetch('/api/ordenes-servicio')).json();
    capOsListCache=Array.isArray(osd)?osd:[];
  }catch(e){ capOsListCache=[]; }
  capLegendRenderRows();
  capApplyColors();
}

const CAP_INT_BORDER = 'inset 0 0 0 2px #4263a0';

function capLegendRenderRows(){
  const tb=document.querySelector('#cap-legend-tb tbody'); if(!tb) return;
  const padded=capLegendRows.slice(0,10);
  while(padded.length<10) padded.push({codigo:'',descripcion:'',esExterno:false,osId:null});
  const osOptions=osId=>'<option value="">— Ninguna —</option>'+capOsListCache.map(os=>
    `<option value="${esc(os.id)}" ${os.id===osId?'selected':''}>${esc(os.id)} · ${esc(os.cliente||'')}</option>`).join('');
  tb.innerHTML=padded.map(row=>`<tr>
    <td style="padding:4px 8px!important;text-align:left!important"><input type="text" maxlength="4" value="${esc(row.codigo)}" placeholder="PMX" style="width:70px;background:var(--inp);border:1px solid var(--border);border-radius:4px;padding:4px 6px;font-size:11px;font-family:'DM Mono',monospace;text-transform:uppercase;color:var(--text);outline:none" onchange="capLegendSave()"></td>
...
async function capLegendSave(){
  const tb=document.querySelector('#cap-legend-tb tbody'); if(!tb) return;
  const rows=Array.from(tb.children).map(tr=>{
    const inps=tr.querySelectorAll('input');
    const sel=tr.querySelector('select');
    return {codigo:inps[0].value.trim().toUpperCase(), descripcion:inps[1].value.trim(), esExterno:inps[2].checked, osId:sel?sel.value:''};
  });
  try{
    const res=await apiCall('PUT','/capacidad/codigos',{codigos:rows});
    if(res.error){toast(res.error,'er');return;}
    capLegendRows=res.codigos;
    capLegendRenderRows();
    capApplyColors();
  }catch(e){toast('Error al guardar leyenda: '+e,'er');}
}

function capShowOsDetalle(osId){
  if(!osId) return;
  const os=capOsListCache.find(o=>o.id===osId);
  if(!os){ toast('No se encontró esa Orden de Servicio','er'); return; }
  document.getElementById('cap-os-det-title').textContent='Orden de Servicio '+os.id;
  document.getElementById('cap-os-det-body').innerHTML=`
    <div><b>Cliente:</b> ${esc(os.cliente||'—')}</div>
    <div><b>Ubicación de Planta:</b> ${esc(os.ubicacion_planta||'—')}</div>
    <div><b>Fecha de Inicio:</b> ${esc(os.fecha_inicio||'—')}</div>
    <div><b>Fecha de Fin:</b> ${esc(os.fecha_fin||'—')}</div>
    <div><b>Estatus:</b> ${esc(os.estatus||'—')}</div>`;
  document.getElementById('mo-cap-os-detalle').classList.add('on');
}

function capLegendMap(){
  const map={};
  capLegendRows.forEach(r=>{ if(r.codigo && r.esExterno) map[r.codigo.trim().toUpperCase()]=true; });
  return map;
}

function capApplyColors(){
  const map=capLegendMap();
  document.querySelectorAll('#cap-wrap .cap-input').forEach(inp=>{
    const v=inp.value.trim().toUpperCase();
    const esExterno=!!map[v];
    inp.style.color=''; inp.style.backgroundColor='';
    const td=inp.closest('td');
    if(td) td.style.boxShadow=esExterno?CAP_INT_BORDER:'';
  });
}

async function capSaveCell(tid,fecha,value,el){
  const codigo=value.trim().toUpperCase();
  if(el){
    el.style.color=''; el.style.backgroundColor='';
    const esExterno=!!capLegendMap()[codigo];
    const td=el.closest('td');
    if(td) td.style.boxShadow=esExterno?CAP_INT_BORDER:'';
  }
  try{
    const res=await apiCall('PUT','/capacidad/'+tid,{fecha,codigo});
    if(res.error){toast(res.error,'er');return;}
    capacidadData[tid]=capacidadData[tid]||{};
    if(codigo) capacidadData[tid][fecha]=codigo; else delete capacidadData[tid][fecha];
  }catch(e){toast('Error al guardar: '+e,'er');}
}

// Pegar un bloque copiado de Excel/Google Sheets: distribuye filas (\n) y columnas (\t)
// a partir de la celda donde se pegó, dentro de la misma tabla de área.
function capOnPaste(e){
  const text=(e.clipboardData||window.clipboardData).getData('text');
  if(!text || (!text.includes('\t') && !text.includes('\n'))) return; // celda única: dejar el pegado nativo
  e.preventDefault();
  ...
  const colIndex=allTds.indexOf(td);
  ...
  rows.forEach((rowText,rOffset)=>{
    const targetRow=allRows[rowIndex+rOffset];
    if(!targetRow) return; // no pasar filas más allá de esta tabla de área
    const targetTds=Array.from(targetRow.children);
    rowText.split('\t').forEach((val,cOffset)=>{
      const targetTd=targetTds[colIndex+cOffset];
      if(!targetTd) return; // no pasar más allá de las 42 columnas visibles
      const targetInput=targetTd.querySelector('input.cap-input');
  ...
  if(pasted) toast(pasted+' celda(s) pegadas ✓','ok');
}

// ════════════════════════════════════════════════════════
//  RECURSOS HUMANOS — CALCULAR SALARIO (Costo Laboral MX)
// ════════════════════════════════════════════════════════
function clMoney(n){ return '$'+(n||0).toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2}); }

function diasVacacionesPorLey(anios){
  anios=Math.max(0,Math.floor(anios));
  ...
  return 28 + Math.ceil((anios-25)/5)*2;
}

// Días ganados ACUMULADOS: suma de los días correspondientes a cada año cumplido (1, 2, 3...)
function diasGanadosAcumulados(aniosCompletos){
  let total=0;
  for(let y=1;y<=aniosCompletos;y++){
    total+=diasVacacionesPorLey(y);
  }
  return total;
}

function clSwitchTipo(tipo){
  document.getElementById('cl-tab-diario').classList.toggle('on',tipo==='diario');
  document.getElementById('cl-tab-mensual').classList.toggle('on',tipo==='mensual');
}

function clFromDiario(){
  const diasMes=parseFloat(document.getElementById('cl-dias-mes').value)||30.4;
  const d=parseFloat(document.getElementById('cl-diario').value)||0;
  document.getElementById('cl-mensual').value=d?(d*diasMes).toFixed(2):'';
  clCalcular();
}
function clFromMensual(){
  const diasMes=parseFloat(document.getElementById('cl-dias-mes').value)||30.4;
  const m=parseFloat(document.getElementById('cl-mensual').value)||0;
  document.getElementById('cl-diario').value=m?(m/diasMes).toFixed(2):'';
  clCalcular();
}
function clRecalcAll(){
  // el factor de días por mes cambió: recalcula el par diario/mensual manteniendo el diario como fuente
  const d=parseFloat(document.getElementById('cl-diario').value)||0;
  if(d) clFromDiario(); else clCalcular();
}
function clAntiguedadChange(){
  const anios=parseFloat(document.getElementById('cl-antiguedad').value)||0;
  document.getElementById('cl-dias-vac').value=diasVacacionesPorLey(anios);
  clCalcular();
}
...
function clCalcular(){
  ...
  const diasMes=parseFloat(document.getElementById('cl-dias-mes').value)||30.4;
  const diario=parseFloat(document.getElementById('cl-diario').value)||0;
  const mensual=parseFloat(document.getElementById('cl-mensual').value)||(diario*diasMes);
  const diasAguinaldo=parseFloat(document.getElementById('cl-dias-aguinaldo').value)||15;
  const diasVac=parseFloat(document.getElementById('cl-dias-vac').value)||12;
  const primaVacPct=(parseFloat(document.getElementById('cl-prima-vac-pct').value)||25)/100;
  const uma=parseFloat(document.getElementById('cl-uma').value)||0;
  const primaRiesgoPct=(parseFloat(document.getElementById('cl-prima-riesgo-pct').value)||0)/100;
  const cvPct=(parseFloat(document.getElementById('cl-cv-pct').value)||0)/100;
  const aplicarTope=document.getElementById('cl-tope-uma').checked;
  const isnPct=(parseFloat(document.getElementById('cl-isn-pct').value)||0)/100;
  const jornadaSemanal=parseFloat(document.getElementById('cl-jornada').value)||48;

  if(!diario||diario<=0){
    results.innerHTML='<div class="es"><span class="ei">🧮</span><p>Captura el salario diario o mensual para ver el desglose del costo laboral.</p></div>';
    clLastData=null;
    return;
  }

  // ── Salario Diario Integrado (SDI) y SBC ──
  const factorIntegracion=1+(diasAguinaldo/365)+((diasVac*primaVacPct)/365);
  let sdi=diario*factorIntegracion;
  let sbc=sdi;
  let topeAplicado=false;
  if(aplicarTope && uma>0){
    const tope=uma*25;
    if(sbc>tope){ sbc=tope; topeAplicado=true; }
  }

  // ── Prestaciones (anuales → mensualizadas) ──
  const aguinaldoAnual=diario*diasAguinaldo;
  const vacacionesAnual=diario*diasVac;
  const primaVacAnual=vacacionesAnual*primaVacPct;
  const aguinaldoMes=aguinaldoAnual/12, vacacionesMes=vacacionesAnual/12, primaVacMes=primaVacAnual/12;
  const prestacionesMes=aguinaldoMes+vacacionesMes+primaVacMes;

  // ── IMSS Patronal (mensualizado sobre SBC, cuotas diarias × días/mes) ──
  const cuotaFijaMes       = uma>0 ? 0.2040*uma*diasMes : 0;
  const excedenteBase      = Math.max(0, sbc-(uma*3));
  const excedenteMes       = uma>0 ? 0.0110*excedenteBase*diasMes : 0;
  const prestDineroMes     = 0.0070*sbc*diasMes;
  const gastosMedPensMes   = 0.0105*sbc*diasMes;
  const invalidezVidaMes   = 0.0175*sbc*diasMes;
  const guarderiasMes      = 0.0100*sbc*diasMes;
  const riesgoTrabajoMes   = primaRiesgoPct*sbc*diasMes;
  const retiroMes          = 0.0200*sbc*diasMes;
  const cyvMes             = cvPct*sbc*diasMes;
  const imssTotalMes = cuotaFijaMes+excedenteMes+prestDineroMes+gastosMedPensMes+invalidezVidaMes+guarderiasMes+riesgoTrabajoMes+retiroMes+cyvMes;

  const infonavitMes = 0.0500*sbc*diasMes;
  const cargasSocialesMes = imssTotalMes+infonavitMes;

  const baseISN = mensual+prestacionesMes;
  const isnMes = baseISN*isnPct;

  const costoMensualTotal = mensual+prestacionesMes+cargasSocialesMes+isnMes;
  const costoAnualTotal = costoMensualTotal*12;
  const costoPorHora = costoMensualTotal/(4*jornadaSemanal);

  // ── Construir desglose ──
  const rows=[
    {grp:'Sueldo Base'},
    {label:'Sueldo Mensual', val:mensual, emph:true},
    {grp:'Prestaciones de Ley'},
    {label:'Aguinaldo (prorrateado)', val:aguinaldoMes},
    {label:'Vacaciones (prorrateado)', val:vacacionesMes},
    {label:'Prima Vacacional (prorrateado)', val:primaVacMes},
    {label:'Subtotal Prestaciones', val:prestacionesMes, sub:true},
    {grp:'Cuotas Patronales IMSS'},
    {label:'Enfermedad y Maternidad — Cuota Fija', val:cuotaFijaMes},
    {label:'Enfermedad y Maternidad — Excedente', val:excedenteMes},
    {label:'Enfermedad y Maternidad — Prestaciones en Dinero', val:prestDineroMes},
    {label:'Gastos Médicos Pensionados', val:gastosMedPensMes},
    {label:'Invalidez y Vida', val:invalidezVidaMes},
    {label:'Guarderías y Prestaciones Sociales', val:guarderiasMes},
    {label:'Riesgo de Trabajo', val:riesgoTrabajoMes},
    {label:'Retiro (SAR)', val:retiroMes},
    {label:'Cesantía y Vejez', val:cyvMes},
    {label:'Subtotal IMSS Patronal', val:imssTotalMes, sub:true},
    {grp:'Otras Aportaciones'},
    {label:'INFONAVIT', val:infonavitMes},
    {label:'Impuesto Sobre Nómina', val:isnMes},
    {label:'Costo Mensual Total', val:costoMensualTotal, tot:true},
  ];

  const bodyRows=rows.map(r=>{
    if(r.grp) return `<tr class="grp"><td colspan="3">${esc(r.grp)}</td></tr>`;
    const pct=costoMensualTotal>0?(r.val/costoMensualTotal*100):0;
    const cls=r.tot?'tot':(r.emph?'emph':(r.sub?'sub':''));
    return `<tr class="${cls}">
      <td>${esc(r.label)}</td>
      <td class="n">${clMoney(r.val)}</td>
      <td><div class="cl-bar-wrap"><div class="cl-bar"><i style="width:${Math.min(100,pct).toFixed(1)}%"></i></div><span class="cl-bar-pct">${pct.toFixed(1)}%</span></div></td>
    </tr>`;
  }).join('');

  clLastData={diario,mensual,diasAguinaldo,diasVac,primaVacPct,uma,primaRiesgoPct,cvPct,aplicarTope,isnPct,
    factorIntegracion,sdi,sbc,topeAplicado,prestacionesMes,imssTotalMes,infonavitMes,cargasSocialesMes,isnMes,
    costoMensualTotal,costoAnualTotal,jornadaSemanal,costoPorHora,rows};
  ...
  results.innerHTML=`
    ...
    <div class="cl-kpis">
      ...
      <div class="cl-kpi"><div class="l">Prestaciones Mensuales</div><div class="v">${clMoney(prestacionesMes)}</div></div>
      <div class="cl-kpi red"><div class="l">Cargas Sociales Mensuales</div><div class="v">${clMoney(cargasSocialesMes)}</div></div>
      <div class="cl-kpi red"><div class="l">Imp. Sobre Nómina Mensual</div><div class="v">${clMoney(isnMes)}</div></div>
    </div>
    <div class="cl-kpis" style="grid-template-columns:1fr 1fr 1fr">
      <div class="cl-kpi hl emph"><div class="l">Costo Mensual Total para la Empresa</div><div class="v">${clMoney(costoMensualTotal)}</div></div>
      <div class="cl-kpi hl"><div class="l">Costo Anual Total para la Empresa</div><div class="v">${clMoney(costoAnualTotal)}</div></div>
      <div class="cl-kpi hl"><div class="l">Costo por Hora (Jornada ${jornadaSemanal}h/sem)</div><div class="v">${clMoney(costoPorHora)}</div></div>
    </div>
    <div class="cl-card" style="margin-top:4px">
      ...
      <div style="font-size:11px;color:var(--muted2);margin-bottom:10px">
        SDI: <b style="color:var(--text)">${clMoney(sdi)}</b> · SBC: <b style="color:var(--text)">${clMoney(sbc)}</b>${topeAplicado?' <span style="color:var(--gold)">(tope de 25 UMA aplicado)</span>':''} · Factor de Integración: <b style="color:var(--text)">${factorIntegracion.toFixed(4)}</b>
      </div>
      <table class="cl-tbl">
        <thead><tr><th>Concepto</th><th style="text-align:right">Mensual</th><th>% del Costo Total</th></tr></thead>
        <tbody>${bodyRows}</tbody>
      </table>
    </div>
    <div class="cl-hint" style="margin-top:6px">Estimación con fines de planeación. Verifica UMA, prima de riesgo, tasa de Cesantía y Vejez e Impuesto Sobre Nómina vigentes antes de usar esta cifra para fines fiscales u oficiales.</div>
  `;
}

let clLastData=null;
function clExportPDF(){
  if(!clLastData){ toast('Primero realiza un cálculo','er'); return; }
  const d=clLastData;
  const now=new Date().toLocaleDateString('es-MX',{year:'numeric',month:'long',day:'numeric'});
  const rowsHtml=d.rows.map(r=>{
    if(r.grp) return `<tr class="grp"><td colspan="2">${esc(r.grp)}</td></tr>`;
    const cls=r.tot?'tot':(r.emph?'emph':(r.sub?'sub':''));
    return `<tr class="${cls}"><td>${esc(r.label)}</td><td class="n">${clMoney(r.val)}</td></tr>`;
  }).join('');

  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Costo Laboral</title><style>
    *{box-sizing:border-box}
    body{font-family:Arial,sans-serif;font-size:12px;color:#222;padding:28px 34px}
    .hd{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #C8102E;padding-bottom:12px;margin-bottom:18px}
    .hd h1{font-size:19px;margin:0;color:#C8102E;letter-spacing:.5px}
    .hd .meta{text-align:right;font-size:11px;color:#666}
    .cards{display:flex;gap:10px;margin-bottom:18px;flex-wrap:wrap}
    .card{flex:1;min-width:140px;border:1px solid #ddd;border-radius:8px;padding:10px 12px}
    .card .l{font-size:9px;text-transform:uppercase;letter-spacing:.5px;color:#888;font-weight:700}
    .card .v{font-size:16px;font-weight:700;margin-top:3px}
    .card.hl{background:#1f3864;border-color:#1f3864}
    .card.hl .l{color:rgba(255,255,255,.7)}
    .card.hl .v{color:#fff;font-size:18px}
    table{width:100%;border-collapse:collapse;margin-bottom:16px}
    th{text-align:left;font-size:9px;text-transform:uppercase;letter-spacing:.5px;color:#888;border-bottom:2px solid #C8102E;padding:6px 8px}
    td{padding:6px 8px;border-bottom:1px solid #eee;font-size:11.5px}
    td.n{text-align:right;font-family:'Courier New',monospace}
    tr.grp td{background:#f2f2f2;font-weight:700;text-transform:uppercase;font-size:9.5px;color:#555}
    tr.sub td{background:#fafafa;font-weight:700}
    tr.emph td{font-weight:800;font-size:13px}
    tr.tot td{font-weight:800;font-size:13px;background:#fdecee;color:#C8102E;border-top:2px solid #C8102E}
    h2{font-size:13px;color:#C8102E;border-bottom:1px solid #ddd;padding-bottom:5px;margin:20px 0 10px}
    .note{font-size:10px;color:#888;margin-top:10px;line-height:1.5}
    .footer{margin-top:26px;padding-top:10px;border-top:1px solid #ddd;font-size:9.5px;color:#999;display:flex;justify-content:space-between}
  </style></head><body>
    <div class="hd">
      <h1>🧮 Calculadora de Costo Laboral</h1>
      <div class="meta">Persico Suite · Recursos Humanos<br>Generado el ${now}</div>
    </div>
    <div class="cards">
      <div class="card"><div class="l">Sueldo Base Mensual</div><div class="v">${clMoney(d.mensual)}</div></div>
      <div class="card"><div class="l">Prestaciones Mensuales</div><div class="v">${clMoney(d.prestacionesMes)}</div></div>
      <div class="card"><div class="l">Cargas Sociales Mensuales</div><div class="v">${clMoney(d.cargasSocialesMes)}</div></div>
      <div class="card hl"><div class="l">Costo Mensual Total</div><div class="v">${clMoney(d.costoMensualTotal)}</div></div>
      <div class="card hl"><div class="l">Costo Anual Total</div><div class="v">${clMoney(d.costoAnualTotal)}</div></div>
      <div class="card hl"><div class="l">Costo por Hora (Jornada ${d.jornadaSemanal}h/sem)</div><div class="v">${clMoney(d.costoPorHora)}</div></div>
    </div>
    <div style="font-size:11px;color:#666;margin-bottom:6px">SDI: <b>${clMoney(d.sdi)}</b> · SBC: <b>${clMoney(d.sbc)}</b>${d.topeAplicado?' (tope de 25 UMA aplicado)':''} · Factor de Integración: <b>${d.factorIntegracion.toFixed(4)}</b></div>
    <h2>Desglose por Concepto (Costo Patronal)</h2>
    <table><thead><tr><th>Concepto</th><th style="text-align:right">Mensual (MXN)</th></tr></thead><tbody>${rowsHtml}</tbody></table>
    <div class="note">Estimación con fines de planeación interna. Verifica UMA, prima de riesgo de trabajo, tasa de Cesantía y Vejez e Impuesto Sobre Nómina vigentes antes de usar esta cifra para fines fiscales u oficiales.</div>
    <div class="footer"><span>Persico Suite · Calculadora de Costo Laboral</span><span>${now}</span></div>
...
async function poShowAllYears(){
  ...
  }catch(e){ toast('Error al cargar todo: '+e,'er'); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}
function poShowAll(){ poVisibleCount=Infinity; poRender(); }
...
// Si el registro trae un costo por hora fijo (ej. exportado desde Control de Horas,
// calculado con Sueldos y Salarios), se respeta ese valor en vez de Hourly Rate.
function whEffectiveRate(r){
  return (r.cost_per_hour!=null) ? r.cost_per_hour : whGetRate(r.employee||'');
}
...
function whFiltered(){
  ...
  return whRecords.filter(r=>{
    ...
    if(key==='cost'){
      const ca=(a.hours||0)*whEffectiveRate(a);
      const cb=(b.hours||0)*whEffectiveRate(b);
      return dir*(ca-cb);
    }
    let av=a[key]??'', bv=b[key]??'';
    if(typeof av==='number') return dir*(av-bv);
...
async function whShowAllYears(){
  ...
  try{
    ...
    whRender(); whUpdateStats();
    toast(whRecords.length.toLocaleString()+' registros cargados (todos los años) ✓','ok',5000);
  }catch(e){ toast('Error al cargar todo: '+e,'er'); }
  finally{ btn.disabled=false; btn.textContent=orig; }
}
...
function whRender(){
  ...
  const isAdmin=!!(USER_PERMS && USER_PERMS.is_admin);
  document.getElementById('wh-actions-hdr').style.display=isAdmin?'':'none';
  const totalHrs =rows.reduce((s,r)=>s+(r.hours||0),0);
  const totalCost=rows.reduce((s,r)=>s+(r.hours||0)*whEffectiveRate(r),0);
  ...
  if(!rows.length){
    tb.innerHTML=`<tr><td colspan="${colspan}"><div class="es"><span class="ei">⏱</span><br>Sin registros</div></td></tr>`;
    return;
  }
  ...
  tb.innerHTML=display.map((r,i)=>{
    ...
    const isFixed=r.cost_per_hour!=null;
    return`<tr style="${i%2===0?'':'background:rgba(0,0,0,.02)'}">
      ...
      <td style="text-align:right;font-family:'DM Mono',monospace;font-size:12px;color:${isFixed?'var(--blue)':(hasRate?'var(--green)':'var(--muted)')}" title="${isFixed?'Costo fijo desde Sueldos y Salarios: $'+rate.toFixed(2)+'/hr USD'+(r.cost_per_hour_mxn?' ($'+r.cost_per_hour_mxn.toFixed(2)+'/hr MXN, TC $'+(r.tipo_cambio||0).toFixed(4)+')':''):(hasRate?'$'+rate.toFixed(2)+'/hr':'Sin tarifa en Hourly Rates')}">
...
async function whDeleteRecord(id){
  if(!confirm('¿Eliminar este registro de Work Hours? Esta acción no se puede deshacer.'))return;
  try{
    const r=await apiCall('DELETE','/wh/'+whActiveYear+'/'+id);
    if(r.error){ toast(r.error,'er'); return; }
    toast('Registro eliminado','if');
    await loadWH();
  }catch(e){ toast('Error: '+e,'er'); }
}
...
function whUpdateStats(){
  ...
  const totalCost=whRecords.reduce((s,r)=>s+(r.hours||0)*whEffectiveRate(r),0);
...
async function rptRenderOperativo(d) {
  ...
  const presLabel = presDisp !== null ? 'Internal Target' : '⚠ Sin config. (usando Revenue)';
  ...
  setCard('rpt-card-gm', 'Gross Margin Operativo', rptFmtMoney(grossOp),
    `${gmSign} ${gmPctOp.toFixed(1)}%  ·  vs Internal Target`, gmClass);

  // Update cost bar relative to Internal Target
  const rev = base || 1;
...
async function mrptRenderOperativo(reports) {
  mrptRestoreLayout();
  // Load Internal Target (presupuesto_disponible) for all jobs — case-insensitive, trimmed
  ...
  mcard('mrpt-card-rev', 'Internal Target', fmt(totBase), configNote, 'rpt-card-blue');
...
async function fxLoadAutoStatus(){
  const box=document.getElementById('fx-auto-status'); if(!box) return;
  try{
    const s=await(await fetch('/api/fx/auto-status')).json();
    const hoy=new Date().toISOString().slice(0,10);
    let html='';
    if(!s.token_configured){
      html='⚠️ <b style="color:var(--red)">Falta configurar BANXICO_TOKEN</b> en el servidor — la actualización automática probablemente está fallando por esto.';
    }else if(s.last_run_date===hoy){
      html=`✅ Actualización automática al corriente (hoy, ${s.last_run_date}).`;
    }else if(s.last_error){
      html=`⚠️ Último intento automático falló: <span style="color:var(--red)">${esc(s.last_error)}</span>`;
    }else if(s.last_check){
      html=`⏳ Aún no corre hoy — último chequeo: ${new Date(s.last_check).toLocaleString('es-MX')}.`;
    }else{
      html='⏳ El actualizador automático todavía no ha corrido desde que arrancó el servidor.';
    }
    box.innerHTML=html;
  }catch(e){ box.textContent='No se pudo consultar el estatus del actualizador automático.'; }
}

document.addEventListener('DOMContentLoaded',()=>{ fxLoadAutoStatus(); });
...
const MODULE_LABELS = {
  ...
  'ops-op':'Operaciones — Órdenes de Producción', 'ops-os':'Operaciones — Órdenes de Servicio'
};
...
function adminSelectUser(uname) {
  ...
  if(!panel) return;
  const puedeVerSalarios=!!info.puede_ver_salarios;
  panel.innerHTML = `
...
async function adminSetPuedeVerSalarios(uname, checked){
  try{
    const r=await fetch('/api/admin/users/'+uname, {
      method:'PUT', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({puede_ver_salarios: checked})
    }).then(r=>r.json());
    if(r.error){ toast(r.error,'er'); return; }
    if(_adminUsersData?.users?.[uname]) _adminUsersData.users[uname].puede_ver_salarios = checked;
    toast(`${uname} → ${checked?'puede':'ya no puede'} ver Salarios`,'ok',2500);
...
async function initPerms() {
  try {
    ...
    applyPermsToDom(d);
    // Refuerzo: la cuadrícula de Capacidad y su leyenda pueden haberse renderizado
    // antes de que los permisos terminaran de cargar (módulos con carga propia en paralelo).
    if(!d.is_admin && !permCanCreate('ops-capacidad')){
      document.querySelectorAll('#cap-wrap .cap-input, #cap-legend-tb input').forEach(inp=>inp.disabled=true);
    }
...
function applyPermsToDom(d) {
  ...
  const navMap = {
    ...
    'fx':            ["switchMenu('fx'"],
    'fin-recepciones':    ["switchMenu('fin-recepciones'"],
    'fin-procesarcompra': ["switchMenu('fin-procesarcompra'"],
    'fin-cpp':            ["switchMenu('fin-cpp'"],
    'fin-pagos':          ["switchMenu('fin-pagos'"],
    'fin-esquemas':       ["switchMenu('fin-esquemas'"],
    'rrhh-asistencia':    ["switchMenu('rrhh-asistencia'"],
    'rrhh-vacaciones':    ["switchMenu('rrhh-vacaciones'"],
    'rrhh-permisos':      ["switchMenu('rrhh-permisos'"],
    ...
    'ops-op':             ["switchMenu('ops-op'"],
    'ops-os':             ["switchMenu('ops-os'"],
  };
  ...
  const btnPatterns = [
    ...
    { pat:'purOpenWizard(',       mod:'fin-procesarcompra', need:'create' },
    { pat:'cppOpenPagoWizard(',   mod:'fin-pagos',          need:'create' },
    { pat:'pagoConfirmar(',       mod:'fin-pagos',          need:'create' },
    { pat:'esqOpenNew(',          mod:'fin-esquemas',       need:'create' },
    { pat:"getElementById('esq-import-file')", mod:'fin-esquemas', need:'create' },
    { pat:'esqOpenEdit(',         mod:'fin-esquemas',       need:'create' },
    { pat:'recOpenEdit(',         mod:'fin-recepciones',    need:'create' },
    ...
    // Control de Personal
    { pat:'personalOpenNew(',    mod:'personal-listado', need:'create' },
    { pat:'personalOpenImport(', mod:'personal-listado', need:'full' },
    { pat:'personalDelete(',     mod:'personal-listado', need:'full' },
    { pat:'personalSave(',       mod:'personal-listado', need:'create' },
    { pat:'areaCreate(',         mod:'personal-areas',   need:'create' },
    ...
    { pat:'vacOpenAdd(',         mod:'rrhh-vacaciones',  need:'create' },
    { pat:'asisSyncWorkers(',    mod:'rrhh-asistencia',  need:'create' },
    { pat:'asisOpenLink(',       mod:'rrhh-asistencia',  need:'create' },
    { pat:'asisLinkSave(',       mod:'rrhh-asistencia',  need:'create' },
    { pat:'ctrlFirmar(',         mod:'rrhh-asistencia',  need:'create' },
    { pat:'ctrlRevocar(',        mod:'rrhh-asistencia',  need:'full' },
    { pat:'ctrlOpenExport(',     mod:'rrhh-asistencia',  need:'full' },
    { pat:'pmOpenNew(',          mod:'rrhh-permisos',    need:'create' },
    { pat:'pmSave(',             mod:'rrhh-permisos',    need:'create' },
    { pat:'pmDecidir(',          mod:'rrhh-permisos',    need:'create' },
    { pat:'pmDelete(',           mod:'rrhh-permisos',    need:'full' },
    { pat:'salSueldosImport(',   mod:'rrhh-sueldos',     need:'create' },
    { pat:'salOpenUnlock(',      mod:'rrhh-sueldos',     need:'create' },
    { pat:'salOpenImportExcel(', mod:'rrhh-sueldos',     need:'create' },
    { pat:'salOpenExportExcel(', mod:'rrhh-sueldos',     need:'create' },
    { pat:'isrOpenImport(',      mod:'rrhh-sueldos',     need:'create' },
    { pat:'npOpenNewPeriodo(',   mod:'rrhh-nomina',      need:'create' },
    { pat:'npOpenGenerar(',      mod:'rrhh-nomina',      need:'create' },
    { pat:'npDeletePeriodo(',    mod:'rrhh-nomina',      need:'full' },
    { pat:'npOpenRecibo(',       mod:'rrhh-nomina',      need:'create' },
    { pat:'npTimbrarRecibo(',    mod:'rrhh-nomina',      need:'full' },
    { pat:'cpcOpenNew(',         mod:'fin-cpc',          need:'create' },
    { pat:'cpcQuickDelete(',     mod:'fin-cpc',          need:'full' },
    { pat:'cpcDelete(',          mod:'fin-cpc',          need:'full' },
    { pat:'osOpenNew(',          mod:'ops-os',           need:'create' },
    { pat:'osQuickDelete(',      mod:'ops-os',           need:'full' },
    { pat:'taOpenNew(',          mod:'ops-ot',           need:'create' },
    { pat:'taQuickDelete(',      mod:'ops-ot',           need:'full' },
    { pat:'taDelete(',           mod:'ops-ot',           need:'full' },
    { pat:'osDelete(',           mod:'ops-os',           need:'full' },
    { pat:'npRevertirRecibo(',   mod:'rrhh-nomina',      need:'full' },
  ];
...
async function dbLoadEstatus(){
  const box=document.getElementById('db-estatus-box'); if(!box) return;
  try{
    const d=await(await fetch('/api/admin/db-estatus')).json();
    if(d.error){ box.innerHTML=`⚠ ${esc(d.error)}`; return; }
    if(!d.db_enabled){
      box.innerHTML='⚠ <b style="color:var(--red)">DATABASE_URL no está configurada</b> — Ventas sigue usando archivos JSON. Agrega la variable de entorno en Railway para poder migrar.';
      return;
    }
    const jc=d.en_archivos_json, dbc=d.en_base_de_datos;
    const cposJsonTotal=Object.values(jc.cpos_por_anio||{}).reduce((a,b)=>a+b,0);
    box.innerHTML=`
      <div>✅ Base de datos conectada.</div>
      <div style="margin-top:6px"><b>Cotizaciones</b> — en JSON: ${jc.quotes} · en base de datos: ${dbc.quotes}</div>
      <div><b>CPOs</b> — en JSON: ${cposJsonTotal} (${Object.keys(jc.cpos_por_anio||{}).length} año/os) · en base de datos: ${dbc.cpos}</div>
    `;
  }catch(e){ box.textContent='Error al consultar el estatus: '+e; }
}

async function dbMigrarVentas(){
  const statusEl=document.getElementById('db-migrar-status');
  if(!confirm('¿Copiar Cotizaciones y CPOs de los archivos JSON hacia PostgreSQL? Es seguro correrlo más de una vez.'))return;
  statusEl.textContent='Migrando…'; statusEl.style.color='var(--muted)';
  try{
    const r=await apiCall('POST','/admin/migrar-ventas-db',{});
    if(r.error){ statusEl.textContent='⚠ '+r.error; statusEl.style.color='var(--red)'; return; }
    const cposTotal=Object.values(r.resultado.cpos_por_anio||{}).reduce((a,b)=>a+b,0);
    statusEl.textContent=`✅ Migrado: ${r.resultado.quotes} cotización(es), ${cposTotal} CPO(s)`;
    statusEl.style.color='#1f8a4c';
    toast('Migración completada ✓','ok');
    dbLoadEstatus();
  }catch(e){ statusEl.textContent='Error: '+e; statusEl.style.color='var(--red)'; }
}

async function dbLoadEstatusGeneral(){
  const box=document.getElementById('db-estatus-general-box'); if(!box) return;
  try{
    const d=await(await fetch('/api/admin/migrar-db-estatus')).json();
    if(!d.db_enabled){
      box.innerHTML='⚠ <b style="color:var(--red)">DATABASE_URL no está configurada.</b>';
      return;
    }
    const filas=Object.entries(d.conteos).map(([nombre,n])=>
      `<div style="display:flex;justify-content:space-between;padding:2px 0"><span>${esc(nombre)}</span><span style="font-family:'DM Mono',monospace;color:${n===null?'var(--muted)':'var(--gold)'}">${n===null?'— (sin migrar)':n}</span></div>`
    ).join('');
    box.innerHTML=`<div style="margin-bottom:6px">✅ Base de datos conectada.</div>${filas}`;
  }catch(e){ box.textContent='Error al consultar el estatus: '+e; }
}

async function dbMigrarTodo(){
  const statusEl=document.getElementById('db-migrar-todo-status');
  if(!confirm('¿Migrar TODA la Suite (44 colecciones) hacia PostgreSQL? Se hace un respaldo completo de los archivos antes de empezar. Es seguro correrlo más de una vez.'))return;
  statusEl.textContent='Migrando… esto puede tardar uno o dos minutos'; statusEl.style.color='var(--muted)';
  try{
    const r=await apiCall('POST','/admin/migrar-todo-db',{});
    if(r.error){ statusEl.textContent='⚠ '+r.error; statusEl.style.color='var(--red)'; return; }
    const colecciones=Object.values(r.resumen);
    const totIns=colecciones.reduce((s,c)=>s+c.insertados,0);
    const totUpd=colecciones.reduce((s,c)=>s+c.actualizados,0);
    const conError=Object.entries(r.resumen).filter(([_,c])=>c.errores.length);
    if(conError.length){
      statusEl.innerHTML=`⚠ Migración con errores en: ${conError.map(([n])=>esc(n)).join(', ')} — revisa la consola del servidor`;
      statusEl.style.color='var(--red)';
    }else{
      statusEl.textContent=`✅ Migración completa: ${totIns} insertado(s), ${totUpd} actualizado(s), sin errores`;
      statusEl.style.color='#1f8a4c';
      toast('Migración completa terminada ✓','ok',6000);
    }
    dbLoadEstatusGeneral();
  }catch(e){ statusEl.textContent='Error: '+e; statusEl.style.color='var(--red)'; }
}

async function dbRepararEsquema(){
  const statusEl=document.getElementById('db-reparar-status');
  if(!confirm('¿Reparar el esquema de la base de datos? Ajusta columnas de tablas que ya existían (nunca borra filas ni el contenido de tus datos). Es seguro correrlo más de una vez.'))return;
  statusEl.textContent='Reparando…'; statusEl.style.color='var(--muted)';
  try{
    const r=await apiCall('POST','/admin/reparar-esquema-db',{});
    if(r.error){ statusEl.textContent='⚠ '+r.error; statusEl.style.color='var(--red)'; return; }
    statusEl.innerHTML=r.log.map(l=>esc(l)).join('<br>');
    statusEl.style.color= r.log.some(l=>l.startsWith('⚠')) ? 'var(--red)' : '#1f8a4c';
    toast('Reparación de esquema completada ✓','ok',6000);
    dbLoadEstatusGeneral();
  }catch(e){ statusEl.textContent='Error: '+e; statusEl.style.color='var(--red)'; }
}
...
// ════════════════════════════════════════════════════════
//  COMPRAS — REQUISICIÓN DE COMPRA (BOM por Job)
// ════════════════════════════════════════════════════════
let reqCurrentJob = null;
...
function reqOpenUpload(){
  document.getElementById('req-up-tipo').value = reqCurrentTipo || 'electrico';
  document.getElementById('req-up-job').value = reqCurrentJob || '';
  document.getElementById('req-up-file').value = '';
  document.getElementById('req-up-status').innerHTML = '';
  document.getElementById('mo-req-upload').classList.add('on');
}

function reqDownloadTemplate(){
  window.open('/api/requisiciones/template', '_blank');
}

async function reqUploadFile(file){
  if(!file) return;
  const job = document.getElementById('req-up-job').value;
  const tipo = document.getElementById('req-up-tipo').value;
  const statusEl = document.getElementById('req-up-status');
  if(!job){ statusEl.innerHTML = '<span style="color:var(--red)">Selecciona el Job antes de subir el archivo.</span>'; return; }
...
async function reqDeleteItem(itemId){
  if(!confirm('¿Eliminar este renglón del BOM?')) return;
  try{
    await apiCall('DELETE','/requisiciones/'+itemId);
    toast('Renglón eliminado','if');
    reqRenderTab();
  }catch(e){ toast('Error: '+e,'er'); }
}

async function reqBuscarStock(){
  if(!reqItems.length){ toast('No hay renglones que buscar','er'); return; }
...
document.addEventListener('DOMContentLoaded', () => {
  const origSwitchReq = window.switchMenu;
  if(typeof origSwitchReq === 'function' && !window._reqHooked) {
    window._reqHooked = true;
    window.switchMenu = function(mod, groupId) {
      origSwitchReq(mod, groupId);
      if(mod === 'requisicion') reqInitSelectors();
    };
  }
});

let stockVisibleCount = 100;
function stockRenderReset(){ stockVisibleCount=100; stockRender(); }
function stockLoadMore(){ stockVisibleCount+=100; stockRender(); }
function stockShowAll(){ stockVisibleCount=Infinity; stockRender(); }
...
function stockRender() {
  ...
  document.getElementById('stk-count').textContent=`${rows.length} materiales${rows.length>stockVisibleCount?` (mostrando ${display.length})`:''}`;
  ...
  document.getElementById('pc-jobs-body').innerHTML = jobDetails.map((j,idx) => {
    ...
    <div class="pc-job-card" id="pc-row-${idx}" style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:16px 20px">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:20px;align-items:flex-end;padding-bottom:14px;border-bottom:1px solid var(--border);margin-bottom:14px">
        ...
        <div class="pc-full-only"><div style="${lbl}">Revenue (CPO)</div><div style="font-weight:700;font-family:'DM Mono',monospace">$${revenue.toLocaleString('en-US',{minimumFractionDigits:2})}</div></div>
        <div><div style="${lbl}">F. Inicio</div><input class="pc-blue-field" data-field="fecha_inicio" type="date" value="${saved.fecha_inicio||''}" oninput="pcCalc(${idx})" style="${dateInp}"></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:28px;margin-bottom:16px">
        <div class="pc-full-only" style="display:flex;flex-direction:column;gap:8px;background:rgba(255,193,7,.06);border-radius:8px;padding:8px 10px;margin:-8px -10px">
          <div style="display:flex;justify-content:space-between;align-items:center">
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;padding-top:6px;border-top:1px dashed var(--border)">
            <span style="font-size:12px;font-weight:700">Calculation Cost</span>
        ...
        <div style="display:flex;flex-direction:column;gap:8px">
          <div style="display:flex;justify-content:space-between;align-items:center">
            ...
            <input class="pc-blue-field" data-field="ahorro_pct" type="number" min="0" max="100" step="0.1" value="${ahorroPct}" placeholder="%" oninput="pcCalc(${idx})" style="${pctInp}">
          </div>
          <div style="flex:1"></div>
          <div style="display:flex;justify-content:space-between;align-items:center;padding-top:6px;border-top:1px dashed var(--border)">
            <span style="font-size:12px;font-weight:700">Internal Target</span>
...
function pcApplyProjconfigFieldPerms() {
  ...
  // Azul: fechas y Target de Ahorro — editable desde "Crear", solo lectura en "Ver"
  if(level === 'view') {
    document.querySelectorAll('.pc-blue-field').forEach(el => {
...
function pcCalc(idx) {
  ...
  if(deltaEl) {
    ...
    deltaEl.title = deltaMatch ? '✓ Suma de estimados igual al Internal Target' : (delta<0 ? 'Los estimados exceden el Internal Target' : 'Queda margen sin asignar');
  }
  pcUpdateTotals();
}
...
// Mantiene sincronizado el filtro de Estatus del sidebar con el filtro embebido en la columna de la tabla
document.getElementById('sal-filter-status')?.addEventListener('change',function(){const col=document.getElementById('sal-filter-status-col');if(col)col.value=this.value;});
function salSyncStatusFilter(value,sourceId){
  const otherId = sourceId==='sal-filter-status-col' ? 'sal-filter-status' : 'sal-filter-status-col';
  const other = document.getElementById(otherId);
  if(other) other.value = value;
  salidaRender();
}
...
function pcRenderGantt(rows, endDateMap, groupRanges) {
  ...
  if(todayX >= 0 && todayX <= W) {
    svgRows += `<line x1="${LABEL+todayX}" y1="${HEAD}" x2="${LABEL+todayX}" y2="${bodyBottom}" stroke="#c8102e" stroke-width="1.5" stroke-dasharray="4,2"/>
    <text x="${LABEL+todayX+3}" y="${HEAD+9}" font-size="8" fill="#c8102e" font-family="Arial">Hoy</text>`;
  }
...
function pcPrintGanttPDF(){
  if(!pcLastGanttSVG){ toast('Genera primero el diagrama de tiempos (captura al menos una actividad con fechas)','er'); return; }
  const job = (typeof jobCurrentJob!=='undefined' && jobCurrentJob) ? jobCurrentJob : {};
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Diagrama de Tiempos ${job.job_number||''}</title><style>
    body{font-family:Arial,sans-serif;padding:26px 30px;color:#222}
    .hd{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:3px solid #C8102E;padding-bottom:10px;margin-bottom:16px}
    .hd h1{font-size:18px;margin:0;color:#C8102E}
    .hd .meta{text-align:right;font-size:11px;color:#666}
    .info{font-size:11.5px;color:#444;margin-bottom:14px}
    .leyenda{display:flex;gap:18px;margin-bottom:14px;font-size:10px;color:#555}
    .leyenda span.sw{display:inline-block;width:9px;height:9px;border-radius:2px;margin-right:5px;vertical-align:middle}
  </style></head><body>
    <div class="hd">
      <h1>📊 Diagrama de Tiempos — Job ${job.job_number||''}</h1>
      <div class="meta">${(job.customer||'').toUpperCase()}<br>Generado ${new Date().toLocaleString('es-MX')}</div>
    </div>
    <div class="info">
      <b>Descripción:</b> ${job.description||'—'} &nbsp; · &nbsp;
      <b>Rango:</b> ${pcLastGanttMeta?pcLastGanttMeta.minD+' → '+pcLastGanttMeta.maxD:'—'} &nbsp; · &nbsp;
      <b>Actividades:</b> ${pcLastGanttMeta?pcLastGanttMeta.total:'—'}
    </div>
    <div class="leyenda">
      <div><span class="sw" style="background:#a8650a"></span>En tiempo</div>
...
function pcDownloadPersonalTemplate() {
  if(!pcPersonalRange) { toast('Configura primero las fechas en la pestaña Timing', 'er'); return; }
  const fmt = d => d.toISOString().slice(0,10);
  const url = '/api/projconfig/plan-personal/template?start=' + fmt(pcPersonalRange.start) + '&end=' + fmt(pcPersonalRange.end);
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  dl2.id = 'pc-timing-activities-list';
  document.body.appendChild(dl2);
});

function pcUpdateActividadPreviaList() {
  const dl = document.getElementById('pc-timing-activities-list');
  const tb = document.getElementById('pc-timing-body');
  if(!dl || !tb) return;
  const names = [...tb.querySelectorAll('[data-field="actividad"]')]
...
// ════════════════════════════════════════════════════════
//  CPC — CUENTAS POR COBRAR (Facturación a Clientes)
// ════════════════════════════════════════════════════════
let cpcRecords=[], cpcYears=[], cpcCurrentId=null, cpcCurrentCFDI=null, cpcCpoSaldos=[];

// ════════════════════════════════════════════════════════
//  OPERACIONES — ÓRDENES DE SERVICIO
// ════════════════════════════════════════════════════════
let osRecords=[], osCurrentId=null, osSvList=[], osPtList=[], osSvJobsDisponibles=[];
let osDraft={};

function osBlankDraft(){
  return {
    sv_number:'', pt_number:'', jobs:[], cliente:'', ubicacion_planta:'', tipo_relacion:'Propio',
    personal:[], alcances:[], materiales:[], pm:'', contacto_cliente:{nombre:'',telefono:'',email:''},
    puntos_abiertos:[], fecha_inicio:'', fecha_fin:'', fecha_viaje_salida:'', fecha_viaje_regreso:'',
    estatus:'Abierto', tipo_servicio:'Pagado',
    necesidades:{traslados:false,renta_auto:false,hospedaje:false,viaticos:false,combustible:false,lavanderia:false},
    envios:[], compras:[],
  };
}
let osTipoNumero='sv'; // 'sv' | 'pt' — cuál de los dos se está usando en el formulario actual
const OS_NECESIDADES_LABELS={traslados:'Traslados',renta_auto:'Renta de Automóvil',hospedaje:'Hospedaje',viaticos:'Viáticos',combustible:'Combustible',lavanderia:'Gastos de Lavandería'};

async function loadOS(){
  try{ osRecords=await(await fetch('/api/ordenes-servicio')).json(); if(!Array.isArray(osRecords))osRecords=[]; }
  catch(e){ osRecords=[]; toast('Error al cargar Órdenes de Servicio: '+e,'er'); }
  osRender();
}

function osEstatusBadge(est){
  if(est==='Cerrado') return '<span class="badge b-yes">Cerrado</span>';
  if(est==='Cancelado') return '<span class="badge" style="background:rgba(0,0,0,.08);color:var(--muted)">Cancelado</span>';
  return '<span class="badge b-wip">Abierto</span>';
}

function osRender(){
  const q=(document.getElementById('os-gs')?.value||'').toLowerCase();
  const estF=document.getElementById('os-estatus-flt')?.value||'';
  let rows=osRecords.slice();
  if(q) rows=rows.filter(r=>(r.id||'').toLowerCase().includes(q)||(r.sv_number||'').toLowerCase().includes(q)||
    (r.pt_number||'').toLowerCase().includes(q)||(r.jobs||[]).join(' ').toLowerCase().includes(q)||(r.cliente||'').toLowerCase().includes(q));
  if(estF) rows=rows.filter(r=>r.estatus===estF);

  document.getElementById('os-tot').textContent=osRecords.length;
  document.getElementById('os-abiertas').textContent=osRecords.filter(r=>r.estatus==='Abierto').length;

  const tb=document.getElementById('os-tb');
  if(!rows.length){ tb.innerHTML='<tr><td colspan="10"><div class="es"><span class="ei">🧰</span><br>Sin órdenes de servicio todavía.</div></td></tr>'; return; }
  tb.innerHTML=rows.map(r=>`
    <tr onclick="osOpenEdit('${r.id}')" style="cursor:pointer;${r.estatus==='Cancelado'?'opacity:.55':''}">
      <td style="font-family:'DM Mono',monospace;color:var(--gold);font-weight:700">${esc(r.id)}</td>
      <td style="font-family:'DM Mono',monospace">${esc(r.sv_number?('SV '+r.sv_number):(r.pt_number?('PT '+r.pt_number):'—'))}</td>
      <td style="font-family:'DM Mono',monospace;color:var(--amber);font-size:11px">${esc((r.jobs||[]).join(', '))}</td>
      <td style="color:var(--muted2)">${esc(r.cliente||'—')}</td>
      <td style="font-size:11px">${esc(r.pm||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-size:11px">${esc(r.fecha_inicio||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-size:11px">${esc(r.fecha_fin||'—')}</td>
...
async function loadSvListForOS(){
  try{
    const d=await fetch('/api/sv').then(r=>r.json());
    osSvList=d.records||[];
    const sel=document.getElementById('os-sv');
    sel.innerHTML='<option value="">— Seleccionar —</option>'+osSvList.map(sv=>
      `<option value="${esc(sv.sv_number)}">${esc(sv.sv_number)} · ${esc(sv.customer||'')}</option>`).join('');
  }catch(e){}
}
async function loadPtListForOS(){
  try{
    const d=await fetch('/api/pt').then(r=>r.json());
    osPtList=d.records||[];
    const sel=document.getElementById('os-pt');
    sel.innerHTML='<option value="">— Seleccionar —</option>'+osPtList.map(pt=>
      `<option value="${esc(pt.pt_number)}">${esc(pt.pt_number)} · ${esc(pt.customer||'')}</option>`).join('');
  }catch(e){}
}

function osSetTipoNumero(tipo){
  osTipoNumero=tipo;
  document.getElementById('os-tipo-num-sv').classList.toggle('on',tipo==='sv');
  document.getElementById('os-tipo-num-pt').classList.toggle('on',tipo==='pt');
  document.getElementById('os-sv-field').style.display=tipo==='sv'?'':'none';
  document.getElementById('os-pt-field').style.display=tipo==='pt'?'':'none';
  if(tipo==='sv'){ document.getElementById('os-pt').value=''; osDraft.pt_number=''; osOnSvSelect(); }
  else{ document.getElementById('os-sv').value=''; osDraft.sv_number=''; osOnPtSelect(); }
}

function osOnSvSelect(){
  const svNum=document.getElementById('os-sv').value;
  const sv=osSvList.find(s=>s.sv_number===svNum);
  osSvJobsDisponibles=sv?(sv.jobs||[]):[];
  if(sv && !document.getElementById('os-cliente').value) document.getElementById('os-cliente').value=sv.customer||'';
  if(sv && !document.getElementById('os-pm').value) document.getElementById('os-pm').value=sv.pm||'';
  osDraft.jobs=osDraft.jobs.filter(j=>osSvJobsDisponibles.includes(j));
  osRenderJobsList();
}
function osOnPtSelect(){
  const ptNum=document.getElementById('os-pt').value;
  const pt=osPtList.find(s=>s.pt_number===ptNum);
  osSvJobsDisponibles=pt?(pt.jobs||[]):[];
  if(pt && !document.getElementById('os-cliente').value) document.getElementById('os-cliente').value=pt.customer||'';
  if(pt && !document.getElementById('os-pm').value) document.getElementById('os-pm').value=pt.pm||'';
  osDraft.jobs=osDraft.jobs.filter(j=>osSvJobsDisponibles.includes(j));
  osRenderJobsList();
}
function osRenderJobsList(){
  const box=document.getElementById('os-jobs-list');
  if(!osSvJobsDisponibles.length){ box.innerHTML=`<span style="font-size:11px;color:var(--muted)">Selecciona primero una ${osTipoNumero==='sv'?'SV':'PT'} para ver sus jobs.</span>`; return; }
  box.innerHTML=osSvJobsDisponibles.map(j=>{
    const checked=osDraft.jobs.includes(j);
    return `<label style="display:flex;align-items:center;gap:5px;background:${checked?'rgba(200,16,46,.12)':'var(--sb)'};border:1px solid ${checked?'var(--red)':'var(--border)'};border-radius:6px;padding:5px 10px;cursor:pointer;font-family:'DM Mono',monospace;font-size:12px">
      <input type="checkbox" ${checked?'checked':''} onchange="osToggleJob('${j}',this.checked)" style="width:14px;height:14px"> ${esc(j)}
    </label>`;
  }).join('');
}
function osToggleJob(job,checked){
  if(checked){ if(!osDraft.jobs.includes(job)) osDraft.jobs.push(job); }
  else{ osDraft.jobs=osDraft.jobs.filter(j=>j!==job); }
}

// ── Personal ──
function osRenderPersonalPicker(){
  const q=(document.getElementById('os-personal-search')?.value||'').toLowerCase();
  const box=document.getElementById('os-personal-picker');
  const activos=personalRecords.filter(p=>(p.estado||'Activo')!=='Baja' && !osDraft.personal.includes(p.tid) &&
...
function osAddPersonal(tid){
  if(!osDraft.personal.includes(tid)) osDraft.personal.push(tid);
  document.getElementById('os-personal-search').value='';
  osRenderPersonalPicker(); osRenderPersonalSelected();
}
function osRemovePersonal(tid){
  osDraft.personal=osDraft.personal.filter(t=>t!==tid);
  osRenderPersonalPicker(); osRenderPersonalSelected();
}
function osRenderPersonalSelected(){
  const box=document.getElementById('os-personal-selected');
  if(!osDraft.personal.length){ box.innerHTML='<span style="font-size:11px;color:var(--muted)">Nadie asignado todavía.</span>'; return; }
  box.innerHTML=osDraft.personal.map(tid=>{
    const p=personalRecords.find(x=>x.tid===tid);
    return `<span class="badge b-wip" style="display:flex;align-items:center;gap:6px">${esc(p?p.nombre:tid)} <span style="cursor:pointer" onclick="osRemovePersonal('${tid}')">✕</span></span>`;
  }).join('');
}

// ── Alcances ──
function osAddAlcance(){
  const txt=document.getElementById('os-new-alcance').value.trim();
  if(!txt) return;
  osDraft.alcances.push({texto:txt,cumplido:false});
  document.getElementById('os-new-alcance').value='';
  osRenderAlcances();
}
function osRenderAlcances(){
  const box=document.getElementById('os-alcances-list');
  if(!osDraft.alcances.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin alcances capturados.</div>'; return; }
  box.innerHTML=osDraft.alcances.map((a,i)=>`
...
// ── Materiales ──
function osAddMaterial(){
  const c=document.getElementById('os-new-mat-concepto').value.trim();
  const cant=parseFloat(document.getElementById('os-new-mat-cantidad').value)||1;
  if(!c) return;
  osDraft.materiales.push({concepto:c,cantidad:cant});
  document.getElementById('os-new-mat-concepto').value=''; document.getElementById('os-new-mat-cantidad').value='';
  osRenderMateriales();
}
function osRenderMateriales(){
  const box=document.getElementById('os-materiales-list');
  if(!osDraft.materiales.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin materiales capturados.</div>'; return; }
  box.innerHTML=osDraft.materiales.map((m,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:7px 10px">
...
function osRenderNecesidades(){
  const box=document.getElementById('os-necesidades-grid');
  box.innerHTML=Object.keys(OS_NECESIDADES_LABELS).map(k=>`
    <label style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:9px 12px;cursor:pointer">
      <input type="checkbox" ${osDraft.necesidades[k]?'checked':''} onchange="osDraft.necesidades['${k}']=this.checked" style="width:16px;height:16px">
      <span style="font-size:12px">${OS_NECESIDADES_LABELS[k]}</span>
    </label>`).join('');
}

// ── Puntos Abiertos ──
function osAddPunto(){
  const txt=document.getElementById('os-new-punto').value.trim();
  if(!txt) return;
  osDraft.puntos_abiertos.push({texto:txt,resuelto:false,fecha:new Date().toISOString(),autor:(USER_PERMS&&USER_PERMS.user)||'Suite'});
  document.getElementById('os-new-punto').value='';
  osRenderPuntos();
}
function osRenderPuntos(){
  const box=document.getElementById('os-puntos-list');
  if(!osDraft.puntos_abiertos.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin puntos abiertos.</div>'; return; }
  box.innerHTML=osDraft.puntos_abiertos.map((p,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:7px 10px">
      <input type="checkbox" ${p.resuelto?'checked':''} onchange="osDraft.puntos_abiertos[${i}].resuelto=this.checked" style="width:16px;height:16px">
      <div style="flex:1">
...
// ── Envíos y Compras ──
function osAddEnvio(){
  const desc=document.getElementById('os-new-envio-desc').value.trim();
  if(!desc) return;
  osDraft.envios.push({descripcion:desc,fecha:document.getElementById('os-new-envio-fecha').value||'',guia:document.getElementById('os-new-envio-guia').value.trim()});
  document.getElementById('os-new-envio-desc').value=''; document.getElementById('os-new-envio-fecha').value=''; document.getElementById('os-new-envio-guia').value='';
  osRenderEnvios();
}
function osRenderEnvios(){
  const box=document.getElementById('os-envios-list');
  if(!osDraft.envios.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin envíos capturados.</div>'; return; }
...
function osAddCompra(){
  const desc=document.getElementById('os-new-compra-desc').value.trim();
  const monto=parseFloat(document.getElementById('os-new-compra-monto').value)||0;
  if(!desc) return;
  osDraft.compras.push({descripcion:desc,monto,fecha:document.getElementById('os-new-compra-fecha').value||''});
  document.getElementById('os-new-compra-desc').value=''; document.getElementById('os-new-compra-monto').value=''; document.getElementById('os-new-compra-fecha').value='';
  osRenderCompras();
}
function osRenderCompras(){
  const box=document.getElementById('os-compras-list');
  if(!osDraft.compras.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin compras capturadas.</div>'; return; }
  box.innerHTML=osDraft.compras.map((c,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:7px 10px">
...
function osSwitchTab(tab){
  ['gen','per','mat','pts','env'].forEach(t=>{
    document.getElementById('os-tab-'+t).classList.toggle('on',t===tab);
    document.getElementById('ostab-'+t).classList.toggle('on',t===tab);
  });
}

function osRenderAllSubLists(){
  osRenderJobsList(); osRenderPersonalPicker(); osRenderPersonalSelected();
  osRenderAlcances(); osRenderMateriales(); osRenderNecesidades();
  osRenderPuntos(); osRenderEnvios(); osRenderCompras();
}

async function osOpenNew(){
  osCurrentId=null; osDraft=osBlankDraft(); osSvJobsDisponibles=[];
  document.getElementById('os-title').textContent='Nueva Orden de Servicio';
  document.getElementById('btn-os-del').style.display='none';
  document.getElementById('os-estatus-field').style.display='none';
  await loadSvListForOS(); await loadPtListForOS();
  document.getElementById('os-sv').value=''; document.getElementById('os-pt').value='';
  osSetTipoNumero('sv');
  ['os-cliente','os-ubicacion','os-pm','os-contacto-nombre','os-contacto-tel','os-contacto-email',
    'os-fecha-inicio','os-fecha-fin','os-viaje-salida','os-viaje-regreso'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('os-tipo-relacion').value='Propio';
  document.getElementById('os-tipo-servicio').value='Pagado';
  osSwitchTab('gen');
  osRenderAllSubLists();
  document.getElementById('mo-os').classList.add('on');
}

async function osOpenEdit(id){
  const r=osRecords.find(x=>x.id===id); if(!r) return;
  osCurrentId=id;
  osDraft={
    sv_number:r.sv_number||'', pt_number:r.pt_number||'', jobs:[...(r.jobs||[])], cliente:r.cliente||'', ubicacion_planta:r.ubicacion_planta||'',
    tipo_relacion:r.tipo_relacion||'Propio', personal:[...(r.personal||[])],
    alcances:JSON.parse(JSON.stringify(r.alcances||[])), materiales:JSON.parse(JSON.stringify(r.materiales||[])),
    pm:r.pm||'', contacto_cliente:{...(r.contacto_cliente||{nombre:'',telefono:'',email:''})},
    puntos_abiertos:JSON.parse(JSON.stringify(r.puntos_abiertos||[])),
    fecha_inicio:r.fecha_inicio||'', fecha_fin:r.fecha_fin||'', fecha_viaje_salida:r.fecha_viaje_salida||'', fecha_viaje_regreso:r.fecha_viaje_regreso||'',
    estatus:r.estatus||'Abierto', tipo_servicio:r.tipo_servicio||'Pagado',
    necesidades:{...osBlankDraft().necesidades, ...(r.necesidades||{})},
    envios:JSON.parse(JSON.stringify(r.envios||[])), compras:JSON.parse(JSON.stringify(r.compras||[])),
  };
  document.getElementById('os-title').textContent='Orden de Servicio '+r.id;
  document.getElementById('btn-os-del').style.display='';
  document.getElementById('os-estatus-field').style.display='';

  await loadSvListForOS(); await loadPtListForOS();
  osTipoNumero = osDraft.pt_number ? 'pt' : 'sv';
  document.getElementById('os-tipo-num-sv').classList.toggle('on', osTipoNumero==='sv');
  document.getElementById('os-tipo-num-pt').classList.toggle('on', osTipoNumero==='pt');
  document.getElementById('os-sv-field').style.display = osTipoNumero==='sv' ? '' : 'none';
  document.getElementById('os-pt-field').style.display = osTipoNumero==='pt' ? '' : 'none';
  if(osDraft.pt_number){
    document.getElementById('os-pt').value=osDraft.pt_number;
    const pt=osPtList.find(s=>s.pt_number===osDraft.pt_number);
    osSvJobsDisponibles=pt?(pt.jobs||[]):[...osDraft.jobs];
  }else{
    document.getElementById('os-sv').value=osDraft.sv_number;
    const sv=osSvList.find(s=>s.sv_number===osDraft.sv_number);
    osSvJobsDisponibles=sv?(sv.jobs||[]):[...osDraft.jobs];
  }

  document.getElementById('os-cliente').value=osDraft.cliente;
  document.getElementById('os-ubicacion').value=osDraft.ubicacion_planta;
  document.getElementById('os-tipo-relacion').value=osDraft.tipo_relacion;
  document.getElementById('os-pm').value=osDraft.pm;
  document.getElementById('os-tipo-servicio').value=osDraft.tipo_servicio;
  document.getElementById('os-contacto-nombre').value=osDraft.contacto_cliente.nombre||'';
  document.getElementById('os-contacto-tel').value=osDraft.contacto_cliente.telefono||'';
  document.getElementById('os-contacto-email').value=osDraft.contacto_cliente.email||'';
  document.getElementById('os-fecha-inicio').value=osDraft.fecha_inicio;
  document.getElementById('os-fecha-fin').value=osDraft.fecha_fin;
  document.getElementById('os-viaje-salida').value=osDraft.fecha_viaje_salida;
  document.getElementById('os-viaje-regreso').value=osDraft.fecha_viaje_regreso;
  document.getElementById('os-estatus').value=osDraft.estatus;
...
function osCollectFromForm(){
  osDraft.cliente=document.getElementById('os-cliente').value.trim();
  osDraft.ubicacion_planta=document.getElementById('os-ubicacion').value.trim();
  osDraft.tipo_relacion=document.getElementById('os-tipo-relacion').value;
  osDraft.pm=document.getElementById('os-pm').value.trim();
  osDraft.tipo_servicio=document.getElementById('os-tipo-servicio').value;
  osDraft.contacto_cliente={
    nombre:document.getElementById('os-contacto-nombre').value.trim(),
    telefono:document.getElementById('os-contacto-tel').value.trim(),
    email:document.getElementById('os-contacto-email').value.trim(),
  };
  osDraft.fecha_inicio=document.getElementById('os-fecha-inicio').value;
  osDraft.fecha_fin=document.getElementById('os-fecha-fin').value;
  osDraft.fecha_viaje_salida=document.getElementById('os-viaje-salida').value;
  osDraft.fecha_viaje_regreso=document.getElementById('os-viaje-regreso').value;
  if(osTipoNumero==='pt'){ osDraft.pt_number=document.getElementById('os-pt').value; osDraft.sv_number=''; }
  else{ osDraft.sv_number=document.getElementById('os-sv').value; osDraft.pt_number=''; }
  if(osCurrentId) osDraft.estatus=document.getElementById('os-estatus').value;
}

async function osSave(){
  osCollectFromForm();
  if((!osDraft.sv_number&&!osDraft.pt_number)||!osDraft.jobs.length){ toast('Selecciona una SV o PT, y al menos un Job','er'); osSwitchTab('gen'); return; }
  if(!osDraft.cliente){ toast('Captura el Cliente','er'); osSwitchTab('gen'); return; }
  try{
    const r=osCurrentId ? await apiCall('PUT','/ordenes-servicio/'+osCurrentId,osDraft) : await apiCall('POST','/ordenes-servicio',osDraft);
    if(r.error){ toast(r.error,'er'); return; }
    closeMo('mo-os');
    toast(osCurrentId?'Orden de servicio actualizada ✓':'Orden de servicio creada ✓','ok');
    await loadOS();
  }catch(e){ toast('Error: '+e,'er'); }
}

async function osDelete(){
  if(!osCurrentId) return;
  if(!confirm('¿Eliminar la orden de servicio '+osCurrentId+'?'))return;
  try{
    await apiCall('DELETE','/ordenes-servicio/'+osCurrentId);
    closeMo('mo-os');
    toast('Orden de servicio eliminada','if');
    await loadOS();
  }catch(e){ toast('Error: '+e,'er'); }
}
async function osQuickDelete(id){
  if(!confirm('¿Eliminar la orden de servicio '+id+'?'))return;
  try{
    await apiCall('DELETE','/ordenes-servicio/'+id);
    toast('Orden de servicio eliminada','if');
    await loadOS();
  }catch(e){ toast('Error: '+e,'er'); }
}
...
document.addEventListener('DOMContentLoaded', () => {
  const origSwitchOS = window.switchMenu;
  if(typeof origSwitchOS === 'function' && !window._osHooked) {
    window._osHooked = true;
    window.switchMenu = function(mod, groupId) {
      origSwitchOS(mod, groupId);
      if(mod === 'ops-os') loadOS();
    };
  }
});

// ════════════════════════════════════════════════════════
//  OPERACIONES — TAREAS ASIGNADAS (antes "Órdenes de Trabajo")
// ════════════════════════════════════════════════════════
let taRecords=[], taCurrentId=null, taDraft={};

function taBlankDraft(){
  return { titulo:'', personal:[], fecha_vencimiento:'', alcances:[], entregables:[], estatus:'Abierto', avance_pct:0, puntos_abiertos:[] };
}

async function loadTareas(){
  try{ taRecords=await(await fetch('/api/tareas')).json(); if(!Array.isArray(taRecords))taRecords=[]; }
  catch(e){ taRecords=[]; toast('Error al cargar Tareas Asignadas: '+e,'er'); }
  taRender();
}

function taEstatusBadge(est){
  if(est==='Cerrado') return '<span class="badge b-yes">Cerrado</span>';
  if(est==='Cancelado') return '<span class="badge" style="background:rgba(0,0,0,.08);color:var(--muted)">Cancelado</span>';
  return '<span class="badge b-wip">Abierto</span>';
}
...
function taRender(){
  const q=(document.getElementById('ta-gs')?.value||'').toLowerCase();
  const estF=document.getElementById('ta-estatus-flt')?.value||'';
  let rows=taRecords.slice();
  if(q) rows=rows.filter(r=>(r.id||'').toLowerCase().includes(q)||(r.titulo||'').toLowerCase().includes(q));
  if(estF) rows=rows.filter(r=>r.estatus===estF);

  document.getElementById('ta-tot').textContent=taRecords.length;
  document.getElementById('ta-abiertas').textContent=taRecords.filter(r=>r.estatus==='Abierto').length;

  const tb=document.getElementById('ta-tb');
  if(!rows.length){ tb.innerHTML='<tr><td colspan="8"><div class="es"><span class="ei">🛠</span><br>Sin tareas asignadas todavía.</div></td></tr>'; return; }
  tb.innerHTML=rows.map(r=>{
    const nombres=(r.personal||[]).map(tid=>{const p=personalRecords.find(x=>x.tid===tid);return p?p.nombre:tid;}).join(', ');
    return `<tr onclick="taOpenEdit('${r.id}')" style="cursor:pointer;${r.estatus==='Cancelado'?'opacity:.55':''}">
      <td style="font-family:'DM Mono',monospace;color:var(--gold);font-weight:700">${esc(r.id)}</td>
      <td style="color:var(--text);font-weight:500">${esc(r.titulo||'—')}</td>
      <td style="font-size:11px;color:var(--muted2)">${esc(nombres||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-size:11px">${r.fecha_creacion?new Date(r.fecha_creacion).toLocaleDateString('es-MX'):'—'}</td>
...
// ── Personal ──
function taRenderPersonalPicker(){
  const q=(document.getElementById('ta-personal-search')?.value||'').toLowerCase();
  const box=document.getElementById('ta-personal-picker');
  const activos=personalRecords.filter(p=>(p.estado||'Activo')!=='Baja' && !taDraft.personal.includes(p.tid) &&
    (!q || (p.nombre||'').toLowerCase().includes(q)));
  if(!activos.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted);padding:6px">Sin resultados.</div>'; return; }
  box.innerHTML=activos.slice(0,30).map(p=>`
...
function taAddPersonal(tid){
  if(!taDraft.personal.includes(tid)) taDraft.personal.push(tid);
  document.getElementById('ta-personal-search').value='';
  taRenderPersonalPicker(); taRenderPersonalSelected();
}
function taRemovePersonal(tid){
  taDraft.personal=taDraft.personal.filter(t=>t!==tid);
  taRenderPersonalPicker(); taRenderPersonalSelected();
}
function taRenderPersonalSelected(){
  const box=document.getElementById('ta-personal-selected');
  if(!taDraft.personal.length){ box.innerHTML='<span style="font-size:11px;color:var(--muted)">Nadie asignado todavía.</span>'; return; }
  box.innerHTML=taDraft.personal.map(tid=>{
    const p=personalRecords.find(x=>x.tid===tid);
    return `<span class="badge b-wip" style="display:flex;align-items:center;gap:6px">${esc(p?p.nombre:tid)} <span style="cursor:pointer" onclick="taRemovePersonal('${tid}')">✕</span></span>`;
  }).join('');
}

// ── Alcances ──
function taAddAlcance(){
  const txt=document.getElementById('ta-new-alcance').value.trim();
  if(!txt) return;
  taDraft.alcances.push({texto:txt,cumplido:false});
  document.getElementById('ta-new-alcance').value='';
  taRenderAlcances();
}
function taRenderAlcances(){
  const box=document.getElementById('ta-alcances-list');
  if(!taDraft.alcances.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin objetivos/alcances capturados.</div>'; return; }
  box.innerHTML=taDraft.alcances.map((a,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:7px 10px">
...
// ── Entregables ──
function taAddEntregable(){
  const txt=document.getElementById('ta-new-entregable').value.trim();
  if(!txt) return;
  taDraft.entregables.push({texto:txt,entregado:false});
  document.getElementById('ta-new-entregable').value='';
  taRenderEntregables();
}
function taRenderEntregables(){
  const box=document.getElementById('ta-entregables-list');
  if(!taDraft.entregables.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin entregables capturados.</div>'; return; }
  box.innerHTML=taDraft.entregables.map((a,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:7px 10px">
...
// ── Puntos Abiertos ──
function taAddPunto(){
  const txt=document.getElementById('ta-new-punto').value.trim();
  if(!txt) return;
  taDraft.puntos_abiertos.push({texto:txt,resuelto:false,fecha:new Date().toISOString(),autor:(USER_PERMS&&USER_PERMS.user)||'Suite'});
  document.getElementById('ta-new-punto').value='';
  taRenderPuntos();
}
function taRenderPuntos(){
  const box=document.getElementById('ta-puntos-list');
  if(!taDraft.puntos_abiertos.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin puntos abiertos.</div>'; return; }
  box.innerHTML=taDraft.puntos_abiertos.map((p,i)=>`
    <div style="display:flex;align-items:center;gap:8px;background:var(--sb);border-radius:var(--r);padding:7px 10px">
      <input type="checkbox" ${p.resuelto?'checked':''} onchange="taDraft.puntos_abiertos[${i}].resuelto=this.checked" style="width:16px;height:16px">
      <div style="flex:1">
        <div style="font-size:12px;${p.resuelto?'text-decoration:line-through;color:var(--muted)':''}">${esc(p.texto)}</div>
        <div style="font-size:10px;color:var(--muted2)">${esc(p.autor||'')} · ${p.fecha?new Date(p.fecha).toLocaleDateString('es-MX'):''}</div>
      </div>
...
function taCalcularAvance(){
  const items=[...taDraft.alcances.map(a=>!!a.cumplido), ...taDraft.entregables.map(e=>!!e.entregado)];
  if(!items.length){ toast('Agrega al menos un alcance o entregable primero','er'); return; }
  const pct=Math.round(items.filter(Boolean).length/items.length*100);
  document.getElementById('ta-avance').value=pct;
  toast('Avance calculado: '+pct+'%','ok');
}

function taRenderAll(){
  taRenderPersonalPicker(); taRenderPersonalSelected();
  taRenderAlcances(); taRenderEntregables(); taRenderPuntos();
}

function taOpenNew(){
  taCurrentId=null; taDraft=taBlankDraft();
  document.getElementById('ta-title').textContent='Nueva Tarea Asignada';
  document.getElementById('btn-ta-del').style.display='none';
  document.getElementById('ta-titulo').value='';
  document.getElementById('ta-fecha-creacion').value='Se asigna al guardar';
  document.getElementById('ta-fecha-vencimiento').value='';
  document.getElementById('ta-estatus').value='Abierto';
  document.getElementById('ta-avance').value=0;
  taRenderAll();
  document.getElementById('mo-ta').classList.add('on');
}

function taOpenEdit(id){
  const r=taRecords.find(x=>x.id===id); if(!r) return;
  taCurrentId=id;
  taDraft={
    titulo:r.titulo||'', personal:[...(r.personal||[])], fecha_vencimiento:r.fecha_vencimiento||'',
    alcances:JSON.parse(JSON.stringify(r.alcances||[])), entregables:JSON.parse(JSON.stringify(r.entregables||[])),
    estatus:r.estatus||'Abierto', avance_pct:r.avance_pct||0,
    puntos_abiertos:JSON.parse(JSON.stringify(r.puntos_abiertos||[])),
  };
  document.getElementById('ta-title').textContent='Tarea '+r.id;
  document.getElementById('btn-ta-del').style.display='';
  document.getElementById('ta-titulo').value=taDraft.titulo;
  document.getElementById('ta-fecha-creacion').value=r.fecha_creacion?new Date(r.fecha_creacion).toLocaleString('es-MX'):'—';
  document.getElementById('ta-fecha-vencimiento').value=taDraft.fecha_vencimiento;
  document.getElementById('ta-estatus').value=taDraft.estatus;
  document.getElementById('ta-avance').value=taDraft.avance_pct;
  taRenderAll();
  document.getElementById('mo-ta').classList.add('on');
}

async function taSave(){
  taDraft.titulo=document.getElementById('ta-titulo').value.trim();
  taDraft.fecha_vencimiento=document.getElementById('ta-fecha-vencimiento').value;
  taDraft.estatus=document.getElementById('ta-estatus').value;
  taDraft.avance_pct=parseInt(document.getElementById('ta-avance').value)||0;
  if(!taDraft.titulo){ toast('Captura el título de la tarea','er'); return; }
  try{
    const r=taCurrentId ? await apiCall('PUT','/tareas/'+taCurrentId,taDraft) : await apiCall('POST','/tareas',taDraft);
    if(r.error){ toast(r.error,'er'); return; }
    closeMo('mo-ta');
    toast(taCurrentId?'Tarea actualizada ✓':'Tarea creada ✓','ok');
    await loadTareas();
  }catch(e){ toast('Error: '+e,'er'); }
}

async function taDelete(){
  if(!taCurrentId) return;
  if(!confirm('¿Eliminar la tarea '+taCurrentId+'?'))return;
  try{
    await apiCall('DELETE','/tareas/'+taCurrentId);
    closeMo('mo-ta');
    toast('Tarea eliminada','if');
    await loadTareas();
  }catch(e){ toast('Error: '+e,'er'); }
}
async function taQuickDelete(id){
  if(!confirm('¿Eliminar la tarea '+id+'?'))return;
  try{
    await apiCall('DELETE','/tareas/'+id);
    toast('Tarea eliminada','if');
    await loadTareas();
  }catch(e){ toast('Error: '+e,'er'); }
}
...
document.addEventListener('DOMContentLoaded', () => {
  const origSwitchTA = window.switchMenu;
  if(typeof origSwitchTA === 'function' && !window._taHooked) {
    window._taHooked = true;
    window.switchMenu = function(mod, groupId) {
      origSwitchTA(mod, groupId);
      if(mod === 'ops-ot') loadTareas();
    };
  }
});

async function loadCPC(){
  const yearSel=document.getElementById('cpc-year');
  const year=parseInt(yearSel.value)||new Date().getFullYear();
  try{
    const d=await fetch('/api/cpc?year='+year).then(r=>r.json());
    cpcRecords=d.records||[];
    cpcYears=d.years&&d.years.length?d.years:[new Date().getFullYear()];
    const cur=yearSel.value;
    yearSel.innerHTML=cpcYears.map(y=>`<option value="${y}">${y}</option>`).join('');
    yearSel.value=cur&&cpcYears.includes(parseInt(cur))?cur:cpcYears[0];
    cpcRender();
  }catch(e){ toast('Error al cargar CPC: '+e,'er'); }
}

function cpcFmt(v){ return (v<0?'-':'')+'$'+Math.abs(Number(v||0)).toLocaleString('en-US',{minimumFractionDigits:2}); }

function cpcEstatusBadge(est){
  if(est==='Pagada') return '<span class="badge b-yes">Pagada</span>';
  if(est==='Vencida') return '<span class="badge b-toappr">Vencida</span>';
  if(est==='Cancelada') return '<span class="badge" style="background:rgba(0,0,0,.08);color:var(--muted)">Cancelada</span>';
  return '<span class="badge b-wip">Pendiente</span>';
}

function cpcRender(){
  const q=(document.getElementById('cpc-gs')?.value||'').toLowerCase();
  const estF=document.getElementById('cpc-estatus-flt')?.value||'';
  let rows=cpcRecords.map(r=>({...r, estatus_efectivo:r.estatus_efectivo||r.estatus}));
  if(q) rows=rows.filter(r=>
    (r.id||'').toLowerCase().includes(q) || (r.job||'').toLowerCase().includes(q) ||
    (r.cpo_id||'').toLowerCase().includes(q) || (r.customer||'').toLowerCase().includes(q) ||
    ((r.cfdi||{}).uuid||'').toLowerCase().includes(q));
  if(estF) rows=rows.filter(r=>r.estatus_efectivo===estF);
  rows.sort((a,b)=>(b.created_at||'').localeCompare(a.created_at||''));

  document.getElementById('cpc-tb').innerHTML=rows.map(r=>`
    <tr onclick="cpcOpenEdit('${r.id}')" style="cursor:pointer;${r.estatus==='Cancelada'?'opacity:.55':''}">
      <td style="font-family:'DM Mono',monospace;color:var(--gold);font-weight:700">${esc(r.id)}</td>
      <td style="font-family:'DM Mono',monospace;color:var(--amber)">${esc(r.job||'—')}</td>
      <td style="font-family:'DM Mono',monospace">${esc(r.cpo_id||'—')}</td>
      <td style="color:var(--muted2);overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${esc(r.customer||'—')}</td>
  ...
  const pendientes=cpcRecords.filter(r=>{
    const ef=r.estatus_efectivo||r.estatus;
    return ef==='Pendiente'||ef==='Vencida';
  });
  const total=pendientes.reduce((s,r)=>s+(r.total||0),0);
  document.getElementById('cpc-total').textContent=cpcFmt(total);
}

// ── Nueva / Editar factura ──
function cpcOpenNew(){
  cpcCurrentId=null; cpcCurrentCFDI=null;
  document.getElementById('cpc-new-title').textContent='Nueva Factura (CPC)';
  document.getElementById('btn-cpc-del').style.display='none';
  document.getElementById('cpc-estatus-field').style.display='none';
  document.getElementById('cpc-fecha-pago-field').style.display='none';
  document.getElementById('cpc-cfdi-file').value='';
  document.getElementById('cpc-cfdi-status').textContent='';
  document.getElementById('cpc-cfdi-content').style.display='none';
  document.getElementById('cpc-usar-cpo').checked=false;
  cpcToggleCpoMode();
  ['cpc-job','cpc-cliente','cpc-fecha-factura','cpc-fecha-venc','cpc-notas'].forEach(id=>document.getElementById(id).value='');
  ['cpc-subtotal','cpc-iva','cpc-total-input'].forEach(id=>document.getElementById(id).value='');
  document.getElementById('cpc-isr-ret').value=0;
  document.getElementById('cpc-iva-ret').value=0;
  document.getElementById('cpc-fecha-factura').value=new Date().toISOString().slice(0,10);
  loadCpoSaldos();
  document.getElementById('mo-cpc-new').classList.add('on');
}

function cpcOpenEdit(id){
  const r=cpcRecords.find(x=>x.id===id); if(!r) return;
  cpcCurrentId=id; cpcCurrentCFDI=r.cfdi||null;
  document.getElementById('cpc-new-title').textContent='Factura '+r.id;
  document.getElementById('btn-cpc-del').style.display='';
  document.getElementById('cpc-estatus-field').style.display='';
  document.getElementById('cpc-cfdi-file').value='';
  if(r.cfdi&&r.cfdi.uuid){ cpcRenderCFDI(r.cfdi); document.getElementById('cpc-cfdi-status').textContent=''; }
  else { document.getElementById('cpc-cfdi-content').style.display='none'; document.getElementById('cpc-cfdi-status').textContent=''; }

  document.getElementById('cpc-usar-cpo').checked=!!r.cpo_id;
  cpcToggleCpoMode();
  loadCpoSaldos(r.cpo_id);

  document.getElementById('cpc-job').value=r.job||'';
  document.getElementById('cpc-cliente').value=r.customer||'';
  document.getElementById('cpc-fecha-factura').value=r.fecha_factura||'';
  document.getElementById('cpc-fecha-venc').value=r.fecha_vencimiento||'';
  document.getElementById('cpc-subtotal').value=r.subtotal||0;
  document.getElementById('cpc-iva').value=r.iva||0;
  document.getElementById('cpc-isr-ret').value=r.isr_retenido||0;
  document.getElementById('cpc-iva-ret').value=r.iva_retenido||0;
  document.getElementById('cpc-total-input').value=r.total||0;
  document.getElementById('cpc-notas').value=r.notas||'';
  document.getElementById('cpc-estatus').value=r.estatus||'Pendiente';
  cpcOnEstatusChange();
  document.getElementById('cpc-fecha-pago').value=r.fecha_pago||'';
  document.getElementById('mo-cpc-new').classList.add('on');
}

function cpcToggleCpoMode(){
  const usar=document.getElementById('cpc-usar-cpo').checked;
  document.getElementById('cpc-cpo-field').style.display=usar?'':'none';
  document.getElementById('cpc-job-field').style.display=usar?'none':'';
}

async function loadCpoSaldos(preselectId){
  try{
    const year=document.getElementById('cpc-fecha-factura').value ? document.getElementById('cpc-fecha-factura').value.slice(0,4) : new Date().getFullYear();
    cpcCpoSaldos=await fetch('/api/cpo-saldos?year='+year).then(r=>r.json());
    const sel=document.getElementById('cpc-cpo-select');
    sel.innerHTML='<option value="">— Seleccionar —</option>'+cpcCpoSaldos.map(c=>
      `<option value="${esc(c.id)}">${esc(c.id)} · ${esc(c.job)} · ${esc(c.customer||'')} (saldo ${cpcFmt(c.saldo)})</option>`).join('');
    if(preselectId){ sel.value=preselectId; cpcOnCpoSelect(); }
  }catch(e){}
}
function cpcOnCpoSelect(){
  const id=document.getElementById('cpc-cpo-select').value;
  const cpo=cpcCpoSaldos.find(c=>c.id===id);
  if(!cpo){ document.getElementById('cpc-cpo-saldo-txt').textContent='—'; return; }
  document.getElementById('cpc-cpo-saldo-txt').innerHTML=`Valor CPO: ${cpcFmt(cpo.value)} · Ya facturado: ${cpcFmt(cpo.facturado)} · <b style="color:${cpo.saldo<0?'var(--red)':'#1f8a4c'}">Saldo por facturar: ${cpcFmt(cpo.saldo)}</b>`;
  document.getElementById('cpc-job').value=cpo.job||'';
  if(!document.getElementById('cpc-cliente').value) document.getElementById('cpc-cliente').value=cpo.customer||'';
}

function cpcRecalcTotal(){
  const sub=parseFloat(document.getElementById('cpc-subtotal').value)||0;
  const iva=parseFloat(document.getElementById('cpc-iva').value)||0;
  const isrR=parseFloat(document.getElementById('cpc-isr-ret').value)||0;
  const ivaR=parseFloat(document.getElementById('cpc-iva-ret').value)||0;
  document.getElementById('cpc-total-input').value=(sub+iva-isrR-ivaR).toFixed(2);
}

function cpcOnEstatusChange(){
  const est=document.getElementById('cpc-estatus').value;
  document.getElementById('cpc-fecha-pago-field').style.display=est==='Pagada'?'':'none';
  if(est==='Pagada' && !document.getElementById('cpc-fecha-pago').value){
    document.getElementById('cpc-fecha-pago').value=new Date().toISOString().slice(0,10);
  }
}

// ── CFDI ──
async function cpcCargarCFDI(file){
  cpcCurrentCFDI=null;
  document.getElementById('cpc-cfdi-content').style.display='none';
  const statusEl=document.getElementById('cpc-cfdi-status');
  if(!file){ statusEl.textContent=''; return; }
  statusEl.textContent='Leyendo CFDI…';
  try{
    const fd=new FormData(); fd.append('file',file);
    const d=await fetch('/api/cpc/parse-cfdi',{method:'POST',body:fd}).then(r=>r.json());
    if(d.error){ statusEl.innerHTML=`<span style="color:var(--red)">⚠ ${esc(d.error)}</span>`; return; }
    cpcCurrentCFDI=d.record;
    cpcRenderCFDI(cpcCurrentCFDI);
  }catch(e){ statusEl.innerHTML='<span style="color:var(--red)">⚠ Error leyendo el archivo</span>'; }
}
function cpcRenderCFDI(cfdi){
  document.getElementById('cpc-cfdi-grid').innerHTML=`
    <div><div style="font-size:9px;color:var(--muted);text-transform:uppercase">UUID</div><div style="font-family:'DM Mono',monospace;font-size:10px;color:var(--gold)">${esc(cfdi.uuid||'—')}</div></div>
    <div><div style="font-size:9px;color:var(--muted);text-transform:uppercase">RFC Receptor</div><div style="font-family:'DM Mono',monospace">${esc(cfdi.rfc_receptor||'—')}</div></div>
    <div><div style="font-size:9px;color:var(--muted);text-transform:uppercase">Receptor</div><div>${esc(cfdi.nombre_receptor||'—')}</div></div>
    <div><div style="font-size:9px;color:var(--muted);text-transform:uppercase">Folio / Serie</div><div>${esc(cfdi.serie||'')}${esc(cfdi.folio||'—')}</div></div>
    <div><div style="font-size:9px;color:var(--muted);text-transform:uppercase">Fecha CFDI</div><div>${esc((cfdi.fecha||'').replace('T',' '))}</div></div>
    <div><div style="font-size:9px;color:var(--muted);text-transform:uppercase">Total CFDI</div><div style="font-weight:700;color:var(--gold)">${cpcFmt(cfdi.total)}</div></div>`;
  document.getElementById('cpc-cfdi-content').style.display='';
  document.getElementById('cpc-cfdi-status').textContent='';
  const matchEl=document.getElementById('cpc-cfdi-match');
  if(cfdi._ya_procesado){
    matchEl.innerHTML=`<span style="color:var(--red)">⚠ Este CFDI ya fue registrado en ${esc(cfdi._folio_previo)} — no se puede volver a usar.</span>`;
  }else{
    matchEl.innerHTML='<span style="color:#1f8a4c">✓ CFDI válido, listo para usarse.</span>';
    document.getElementById('cpc-subtotal').value=cfdi.subtotal||0;
    document.getElementById('cpc-iva').value=cfdi.impuestos_trasladados||0;
    document.getElementById('cpc-iva-ret').value=0;
    document.getElementById('cpc-isr-ret').value=0;
    document.getElementById('cpc-total-input').value=cfdi.total||0;
    if(!document.getElementById('cpc-cliente').value) document.getElementById('cpc-cliente').value=cfdi.nombre_receptor||'';
    if(cfdi.fecha) document.getElementById('cpc-fecha-factura').value=cfdi.fecha.slice(0,10);
  }
}

async function cpcSave(){
  if(cpcCurrentCFDI && cpcCurrentCFDI._ya_procesado){ toast('Este CFDI ya fue registrado antes','er'); return; }
  const usarCpo=document.getElementById('cpc-usar-cpo').checked;
  const cpoId=usarCpo?document.getElementById('cpc-cpo-select').value:null;
  const job=document.getElementById('cpc-job').value.trim();
  const cliente=document.getElementById('cpc-cliente').value.trim();
  const fechaFactura=document.getElementById('cpc-fecha-factura').value;
  if(!job||!cliente||!fechaFactura){ toast('Completa Job, Cliente y Fecha de Factura','er'); return; }
  if(usarCpo && !cpoId){ toast('Selecciona una CPO','er'); return; }

  const payload={
    year:parseInt(fechaFactura.slice(0,4)),
    job, cpo_id:cpoId, customer:cliente,
    fecha_factura:fechaFactura, fecha_vencimiento:document.getElementById('cpc-fecha-venc').value,
    subtotal:document.getElementById('cpc-subtotal').value, iva:document.getElementById('cpc-iva').value,
    isr_retenido:document.getElementById('cpc-isr-ret').value, iva_retenido:document.getElementById('cpc-iva-ret').value,
    total:document.getElementById('cpc-total-input').value,
    notas:document.getElementById('cpc-notas').value,
    cfdi:cpcCurrentCFDI,
  };
  try{
    let r;
    if(cpcCurrentId){
      payload.estatus=document.getElementById('cpc-estatus').value;
      payload.fecha_pago=document.getElementById('cpc-fecha-pago').value||null;
      r=await apiCall('PUT','/cpc/'+cpcCurrentId,payload);
    }else{
      r=await apiCall('POST','/cpc',payload);
    }
    if(r.error){ toast(r.error,'er'); return; }
    closeMo('mo-cpc-new');
    toast(cpcCurrentId?'Factura actualizada ✓':'Factura creada ✓','ok');
    await loadCPC();
  }catch(e){ toast('Error: '+e,'er'); }
}

async function cpcDelete(){
  if(!cpcCurrentId) return;
  const r=cpcRecords.find(x=>x.id===cpcCurrentId);
  if(!confirm('¿Eliminar la factura '+cpcCurrentId+'?'))return;
  try{
    await apiCall('DELETE','/cpc/'+cpcCurrentId+'?year='+(r?r.year:new Date().getFullYear()));
    closeMo('mo-cpc-new');
    toast('Factura eliminada','if');
    await loadCPC();
  }catch(e){ toast('Error: '+e,'er'); }
}
async function cpcQuickDelete(id,year){
  if(!confirm('¿Eliminar la factura '+id+'?'))return;
  try{
    await apiCall('DELETE','/cpc/'+id+'?year='+year);
    toast('Factura eliminada','if');
    await loadCPC();
  }catch(e){ toast('Error: '+e,'er'); }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(typeof origSwitch4 === 'function' && !window._cpcHooked) {
    window._cpcHooked = true;
    window.switchMenu = function(mod, groupId) {
      origSwitch4(mod, groupId);
      if(mod === 'fin-cpc') loadCPC();
    };
  }
});
