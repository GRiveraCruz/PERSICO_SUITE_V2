...
function pcGetTimingData() {
  ...
  return [...tb.rows].map((tr) => {
    // Use querySelectorAll to get all inputs in order
    const inputs = tr.querySelectorAll('input[type="text"], input[type="date"], input[type="number"]');
    const checkboxes = tr.querySelectorAll('input[type="checkbox"]');
    return {
      actividad:             inputs[0]?.value?.trim() || '',
      actividad_previa:      inputs[1]?.value?.trim() || '',
      fecha_inicial:         inputs[2]?.value || '',
      dias_estimados:        parseInt(inputs[3]?.value || 0) || 0,
      fecha_condicionada:    tr.querySelector('.pc-t-cond')?.textContent || '',
      fecha_objetivo:        tr.querySelector('.pc-t-obj')?.textContent || '',
      cumplido:              checkboxes[0]?.checked || false,
      milestone_facturacion: checkboxes[1]?.checked || false,
      pct_facturacion:       parseFloat(inputs[4]?.value) || null,
    };
...
function pcRenderGantt(rows, endDateMap) {
  ...
  rows.forEach((tr) => {
    const inputs    = tr.querySelectorAll('input[type="text"],input[type="date"],input[type="number"]');
    const checkboxes= tr.querySelectorAll('input[type="checkbox"]');
    const act  = inputs[0]?.value?.trim();
    const prev = inputs[1]?.value?.trim();
    const fIni = inputs[2]?.value;
    const dias = parseInt(inputs[3]?.value||0)||0;
    const cumpl= checkboxes[0]?.checked;
    const mile = checkboxes[1]?.checked;
    if(!act) return;
  ...
  let svgRows = entries.map((e,i) => {
    ...
    <text x="${LABEL-4}" y="${y+ROW-8}" font-size="10" fill="#ccc" text-anchor="end" font-family="Arial">${label}${mileIcon}</text>`;
  }).join('');
  ...
  gantt.innerHTML = `<svg width="${LABEL+W+10}" height="${svgH}" xmlns="http://www.w3.org/2000/svg" style="background:rgba(255,255,255,.02);border-radius:6px">
    ${svgRows}
  </svg>`;
}
