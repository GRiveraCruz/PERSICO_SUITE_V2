...
function poRender(){
  ...
  tb.innerHTML=rows.map(r=>{
    ...
    // Show GPO number if available, else clave
    ...
    const delBtn = isAdm && r.gpo_number && r.gpo_pdf
      ? `<button onclick="deleteGPO('${esc(r.gpo_number)}')" class="fi-del" style="font-size:11px;margin-left:4px" title="Eliminar PO">✕</button>`
      : isAdm && !r.gpo_number
      ? `<button onclick="deleteIPO(${poActiveYear},${r.clave})" class="fi-del" style="font-size:11px" title="Eliminar registro">✕</button>`
      : '';
    return`<tr>
      ...
      <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis;font-weight:500;color:#fff" title="${esc(r.nombre)}">${esc(r.nombre||'—')}</td>
      ...
      <td style="white-space:nowrap">${pdfBtn}${delBtn}</td>
    </tr>`;
  }).join('');
}
...
function rptSwitchTab(tab) {
  ...
  Object.entries(tabs).forEach(([k,id])=>{
    ...
    el.style.background = k===tab ? 'var(--red)' : 'rgba(255,255,255,.06)';
...
async function mrptRenderComercialMulti(reports) {
  ...
  const rows = reports.map(r=>{
    ...
    return `<tr>
      <td style="padding:8px 10px;font-family:'DM Mono',monospace;color:var(--gold);font-weight:600">${esc(r.job_number)}</td>
      <td style="padding:8px 10px;color:#fff">${esc(r.customer||'—')}</td>
  ...
  document.getElementById('mrpt-thead').innerHTML=`
    <th style="padding:7px 10px;text-align:left;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Job</th>
    <th style="padding:7px 10px;text-align:left;font-size:9px;color:var(--muted);border-bottom:2px solid var(--red)">Cliente</th>
    <th style="padding:7px 10px;text-align:left;font-size:9px;color:var(--muted);border-bottom:2px solid var(--red)">Descripción</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;color:var(--muted);border-bottom:2px solid var(--red)">Target Compras</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;color:var(--muted);border-bottom:2px solid var(--red)">Purchase Orders</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;color:var(--green);border-bottom:2px solid var(--red)">Ahorro Comercial</th>`;
...
async function rptRenderOperativo(d) {
  ...
  const targetsHtml = hasTargets ? `
    ...
    <div id="rpt-targets-section" style="margin-top:12px;padding:10px 14px;
        background:rgba(255,255,255,.03);border:1px dashed var(--border);border-radius:8px;
...
async function mrptRenderOperativo(reports) {
  ...
  const fmt = v => '$'+Number(v||0).toLocaleString('en-US',{minimumFractionDigits:2});
  let totBase=0, totWH=0, totPur=0, totGM=0, totReas=0, totRecov=0, totConfigured=0;

  const rows = reports.map(r => {
    const key    = (r.job_number||'').trim().toUpperCase();
    const hasPresp = key in presMap;
    const base   = hasPresp ? presMap[key] : r.revenue;
    const grossOp = base - r.amount_wh - r.purchasing_total - (r.reassign_total||0) + (r.recovery_total||0);
    ...
    return `<tr>
      ...
      <td style="padding:8px 10px;text-align:right;font-family:'DM Mono',monospace;color:var(--text)">${fmt(r.amount_wh)}</td>
      <td style="padding:8px 10px;text-align:right;font-family:'DM Mono',monospace;color:var(--text)">${fmt(r.purchasing_total)}</td>
  ...
  const totGMop  = totBase - totWH - totPur - totReas + totRecov;
  ...
  mcard('mrpt-card-pur', 'Purchasings Total', fmt(totPur), '', 'rpt-card-dark');
...
function rptRender(d){
  ...
  const svcSub   = svcTotal>0
    ? `Viáticos: ${rptFmtMoney(d.svc_viaticos||0)} · Viajes: ${rptFmtMoney(d.svc_gastos||0)} · Envíos: ${rptFmtMoney(d.svc_envios||0)}`
    : d.po_items.length+' OC(s)';
  card('rpt-card-pur', svcTotal>0?`Purchasings + Servicios`:'Purchasings Total',
    rptFmtMoney(d.purchasing_total + svcTotal),
    svcSub, 'rpt-card-dark');
  ...
  if(raSection && raTb){
    if(d.reassign_items && d.reassign_items.length){
      ...
      raTb.innerHTML = d.reassign_items.map((i,idx)=>`
        <tr class="rpt-tr" style="${idx%2?'background:rgba(255,255,255,.02)':''}">
          ...
          <td class="rpt-td" style="color:#fff;font-size:11px">${esc(i.manufacturer||'')}</td>
...
function mrptRestoreLayout() {
  // Restore card-pur visibility (hidden by Comercial tab)
  const pur = document.getElementById('mrpt-card-pur');
  if(pur) pur.style.display = '';
  // Restore full thead (overwritten by Comercial tab)
  const thead = document.getElementById('mrpt-thead');
  if(thead) thead.innerHTML = `
    <th style="padding:7px 10px;text-align:left;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Job</th>
    <th style="padding:7px 10px;text-align:left;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Cliente</th>
    <th style="padding:7px 10px;text-align:left;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Descripción</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Revenue</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Hrs</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">WH Cost</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Purchasings</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--red);border-bottom:2px solid var(--red)">Reasign.</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--green);border-bottom:2px solid var(--red)">Recuper.</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">Gross Margin</th>
    <th style="padding:7px 10px;text-align:right;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted);border-bottom:2px solid var(--red)">GM%</th>`;
}
...
function renderAdminUsers(d) {
  ...
  const html = Object.entries(users).map(([uname, info]) => {
    ...
    const rows = MODULE_GROUPS.map(grp => {
      const groupMods = grp.mods.filter(m => modules.includes(m));
      if(!groupMods.length) return '';
      const header = `<tr style="background:rgba(255,255,255,.03)">
        <td colspan="2" style="padding:6px 10px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:1.5px;color:var(--red);border-bottom:1px solid var(--border)">${grp.label}</td>
      </tr>`;
      const modRows = groupMods.map(mod => {
        const level   = isAdmin ? 'full' : (perms[mod] || 'none');
        const options = ['none','view','create','full'].map(l =>
          `<option value="${l}" ${level===l?'selected':''}>${LEVEL_LABELS[l]}</option>`
        ).join('');
        return `<tr style="border-bottom:1px solid rgba(255,255,255,.04)">
          <td style="padding:6px 10px 6px 18px;font-size:11px;color:var(--muted2);white-space:nowrap;width:55%">${MODULE_LABELS[mod]||mod}</td>
          <td style="padding:4px 10px">
            <select data-user="${uname}" data-mod="${mod}"
              onchange="adminSetLevel(this)"
              ${isAdmin?'disabled':''}
              style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:6px;
                     color:${level==='full'?'var(--green)':level==='create'?'var(--gold)':level==='view'?'var(--text)':'var(--muted)'};
                     padding:5px 8px;font-size:11px;outline:none;cursor:${isAdmin?'not-allowed':'pointer'}">
              ${options}
            </select>
          </td>
        </tr>`;
    ...
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:18px;margin-bottom:16px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:14px">
        ...
        <div>
          ...
          <div style="font-size:11px;color:var(--muted)">${isAdmin?'Administrador':'Acceso personalizado'}</div>
        </div>
        <div style="margin-left:auto;display:flex;align-items:center;gap:8px">
          ...
          <select onchange="adminChangeRole('${uname}',this.value)"
            ...
            <option value="viewer" ${role==='viewer'?'selected':''}>Acceso personalizado</option>
      ...
      <div style="overflow-x:auto">
        <table style="width:100%;border-collapse:collapse">
          <thead><tr>
            ...
            <th style="padding:4px 10px;text-align:left;font-size:9px;font-weight:600;text-transform:uppercase;letter-spacing:1px;color:var(--muted)">Nivel de acceso</th>
          </tr></thead>
          <tbody>${rows}</tbody>
        </table>
      </div>
  ...
  document.querySelectorAll('#admin-users-grid').forEach(el => {
    el.innerHTML = html || '<div style="color:var(--muted);text-align:center;padding:40px">No se encontraron usuarios</div>';
  });
}
...
function awardJobRow(idx, customer='', desc='') {
  ...
  const typeSelector = isFirst ? '' : `
    <div style="display:flex;gap:6px;margin-bottom:8px">
      <label style="display:flex;align-items:center;gap:5px;font-size:11px;cursor:pointer;padding:5px 10px;border-radius:6px;border:1px solid var(--border);flex:1;justify-content:center;background:rgba(255,255,255,.03)">
...
function awardUpdateValue() {
  ...
  if(rem) {
    ...
    rem.style.color = remaining < 0 ? '#ff4444' : remaining === 0 ? '#6fcf97' : '#f59e0b';
  }
}
  ...
  try {
    ...
    el.innerHTML = d.users.map(u=>`
      <div style="display:flex;align-items:center;gap:10px;padding:10px 14px;background:var(--card);border:1px solid var(--border);border-radius:8px;margin-bottom:6px">
        <div style="width:32px;height:32px;border-radius:50%;background:${u.role==='admin'?'rgba(200,16,46,.2)':'rgba(255,255,255,.08)'};display:flex;align-items:center;justify-content:center;font-size:14px">${u.role==='admin'?'👑':'👤'}</div>
        <div style="flex:1">
          <div style="font-weight:600;color:${u.active?'#fff':'var(--muted)'}">${esc(u.username)} ${u.is_admin_user?'<span style="font-size:9px;color:var(--red)">[SUPER ADMIN]</span>':''}</div>
...
function stockSearch(q) {
  ...
  res.innerHTML = matches.map(r=>`
    <div onclick="selectStockItem('${r.id}')" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center;transition:background .15s" onmouseenter="this.style.background='rgba(255,255,255,.05)'" onmouseleave="this.style.background=''">
      <div style="flex:1">
...
function raSearch(q) {
  ...
  res.style.display='';
  res.innerHTML=matches.map(r=>`
    <div onclick="raSelectItem('${r.id}')" style="padding:8px 12px;cursor:pointer;border-bottom:1px solid var(--border);display:flex;gap:10px;align-items:center" onmouseenter="this.style.background='rgba(255,255,255,.05)'" onmouseleave="this.style.background=''">
      <div style="flex:1">
        <div style="font-family:'DM Mono',monospace;color:var(--gold);font-size:12px">${esc(r.part_number)}</div>
        <div style="font-size:11px;color:var(--muted2)">${esc(r.manufacturer)} · ${esc(r.description||'')}${r.label_code?' · 🏷 '+esc(r.label_code):''}</div>
      </div>
...
function renderRaItems(){
  ...
  document.getElementById('ra-items-tb').innerHTML=raItems.map((i,idx)=>`
    <div style="display:flex;gap:8px;align-items:center;padding:7px 10px;background:rgba(255,255,255,.03);border-radius:6px;margin-bottom:4px;font-size:11px">
...
function provRender() {
  ...
  document.getElementById('prov-tb').innerHTML = rows.map(r=>`
    <tr class="tr-hover" onclick="provOpenPanel(${r.clave})">
      ...
      <td style="font-weight:600;color:#fff">${esc(r.nombre||'')}</td>
...
async function openNewGPO() {
  ...
  gpoClearSupplier();
  // Reset header fields
  ['gpo-ptsv','gpo-notes','gpo-item-search',
...
function ingresoRender() {
  ...
  const tipoBadge = t => t==='gpo'
    ...
    : '<span style="font-size:9px;background:rgba(255,255,255,.08);color:var(--muted2);padding:2px 6px;border-radius:4px">Manual</span>';
...
function ingresoOpenManual() {
  ...
  document.getElementById('im-recibe').value='';
...
async function imProcesar() {
  ...
  const recibe = document.getElementById('im-recibe').value.trim();
  ...
  document.getElementById('ipo-items-tb').innerHTML = (po.items||[]).map((it,i)=>{
    ...
    return `<tr>
      ...
      <td style="text-align:right">
        <input type="number" min="0" max="${qPend}" step="1" value="${qPend}"
...
function ipoUpdateEstatus() {
  ...
  if(allFull){
    ...
    el.style.background='rgba(255,255,255,.04)'; el.style.color='var(--muted)';
...
function apartadosRender() {
  ...
  document.getElementById('apt-tb').innerHTML = rows.map(r => {
    ...
    <tr id="${detailId}" style="display:none;background:rgba(255,255,255,.025)">
...
function salidaRender() {
  ...
  document.getElementById('sal-tb').innerHTML = rows.flatMap(r=>
    ...
    <tr>
      ${idx===0?`<td rowspan="${r.items.length}" style="vertical-align:top;padding-top:10px;color:var(--muted2);font-size:11px">${r.fecha||''}</td>`:''}
      ${idx===0?`<td rowspan="${r.items.length}" style="vertical-align:top;padding-top:10px;font-family:'DM Mono',monospace;font-weight:700;color:var(--gold)">${esc(r.job||'—')}</td>`:''}
...
async function salidaCargarApartados() {
  ...
  try {
    ...
    const all = d.records||[];
    // Extract items for this specific job
    salidaAptItems = [];
    all.forEach(r=>{
    ...
    if(!salidaAptItems.length){
      document.getElementById('sal-apt-empty').textContent=`No hay materiales apartados para el Job ${job}`;
      document.getElementById('sal-apt-empty').style.display='';
      document.getElementById('sal-apt-content').style.display='none';
      return;
    }
    ...
    salidaRenderItems(salidaAptItems);
  } catch(e){ toast('Error cargando apartados','er'); }
}

function salidaRenderItems(items) {
  ...
  document.getElementById('sal-apt-tb').innerHTML = items.map((it,i)=>`
    <tr id="sal-item-row-${i}">
      ...
      <td style="text-align:right">
        <input type="number" min="1" max="${it.available}" step="1" value="${it.available}"
          id="sal-qty-${i}"
...
async function movHistorialLoad() {
  ...
  try {
    ...
    list.innerHTML = records.map(r=>`
      <div style="border:1px solid var(--border);border-radius:8px;padding:12px 14px;margin-bottom:8px;background:rgba(255,255,255,.03)">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:6px">
          ...
          <span style="font-size:10px;background:rgba(255,255,255,.06);color:var(--muted2);padding:2px 8px;border-radius:4px">
...
function pcAddTimingRow(data={}) {
  const tb = document.getElementById('pc-timing-body');
  if(!tb) { console.error('pcAddTimingRow: tb not found'); return; }
  console.log('Adding row:', data.actividad||'unnamed');

  const tr = document.createElement('tr');
  
  tr.innerHTML = `
    <td><input list="pc-act-list" value="${esc(data.actividad||data.name||'')}"
      oninput="pcUpdateTimingCalcs()"
      style="width:100%;background:var(--inp);border:1px solid rgba(255,193,7,.3);border-radius:4px;color:var(--amber);padding:5px 7px;font-size:11px">
    </td>
    <td><input list="pc-act-list" value="${esc(data.prev||data.actividad_previa||'')}"
      oninput="pcUpdateTimingCalcs()"
      style="width:100%;background:var(--inp);border:1px solid rgba(255,193,7,.3);border-radius:4px;color:var(--muted2);padding:5px 7px;font-size:11px">
    </td>
    <td><input type="date" value="${data.fecha_ini||data.fecha_inicial||''}"
      oninput="pcUpdateTimingCalcs()"
      style="background:var(--inp);border:1px solid rgba(255,193,7,.3);border-radius:4px;color:var(--text);padding:4px 6px;font-size:11px;width:100%">
    </td>
    <td><input type="number" min="0" value="${data.dias||data.dias_estimados||0}"
      oninput="pcUpdateTimingCalcs()"
      style="width:60px;background:var(--inp);border:1px solid rgba(255,193,7,.3);border-radius:4px;color:var(--amber);padding:5px 7px;font-size:11px;text-align:right">
    </td>
    ...
    <td style="text-align:center">
      <input type="checkbox" class="pc-cumplido" ${data.cumplido?'checked':''}
    ...
    <td style="text-align:center">
      <input type="checkbox" class="pc-milestone" ${data.milestone||data.milestone_facturacion?'checked':''}
        style="accent-color:var(--gold);width:16px;height:16px;cursor:pointer"
        onchange="pcUpdateTimingCalcs()">
    </td>
    <td><input type="number" min="0" max="100" step="1" value="${data.pct_fact||data.pct_facturacion||''}"
      placeholder="%"
      style="width:55px;background:var(--inp);border:1px solid rgba(255,193,7,.3);border-radius:4px;color:var(--gold);padding:5px 7px;font-size:11px;text-align:right">
    </td>
...
function pcUpdateTimingCalcs() {
  ...
  // Build map: activity name → calculated end date
  const endDateMap = {};

  rows.forEach((tr, i) => {
    const cells  = tr.cells;
    const activ  = cells[0].querySelector('input')?.value.trim() || '';
    const prev   = cells[1].querySelector('input')?.value.trim() || '';
    const fIni   = cells[2].querySelector('input')?.value || '';
    const dias   = parseInt(cells[3].querySelector('input')?.value||0)||0;
    const cumpl  = cells[6].querySelector('input')?.checked || false;

    // Fecha condicionada = fecha_ini if set, else end date of prev activity
    let fCond = fIni ? new Date(fIni+'T00:00:00') : null;
    if(!fCond && prev && endDateMap[prev]) {
      fCond = new Date(endDateMap[prev]);
      fCond.setDate(fCond.getDate() + 1);
    }

    const condEl = tr.querySelector('.pc-t-cond');
    const objEl  = tr.querySelector('.pc-t-obj');
    const statEl = tr.querySelector('.pc-t-status');

    if(fCond) {
      if(condEl) condEl.textContent = fCond.toLocaleDateString('es-MX',{month:'2-digit',day:'2-digit',year:'numeric'});
      const fObj = new Date(fCond);
      fObj.setDate(fObj.getDate() + dias);
      if(objEl) objEl.textContent = fObj.toLocaleDateString('es-MX',{month:'2-digit',day:'2-digit',year:'numeric'});
      if(activ) endDateMap[activ] = fObj;

      // Status
      if(cumpl) {
        if(statEl) { statEl.innerHTML='<span style="color:var(--green);font-weight:700">✓ CUMPLIDO</span>'; }
      } else if(fCond > today) {
        if(statEl) { statEl.innerHTML='<span style="color:var(--muted)">EN ESPERA</span>'; }
      } else if(fObj >= today) {
        if(statEl) { statEl.innerHTML='<span style="color:var(--amber);font-weight:600">EN TIEMPO</span>'; }
      } else {
        if(statEl) { statEl.innerHTML='<span style="color:var(--red);font-weight:700">⚠ RETRASO</span>'; }
      }
    } else {
...
function pcGetTimingData() {
  ...
  return [...tb.rows].map((tr,i) => {
    const cells = tr.cells;
    return {
      actividad:           cells[0].querySelector('input')?.value.trim()||'',
      actividad_previa:    cells[1].querySelector('input')?.value.trim()||'',
      fecha_inicial:       cells[2].querySelector('input')?.value||'',
      dias_estimados:      parseInt(cells[3].querySelector('input')?.value||0)||0,
      fecha_condicionada:  tr.querySelector('.pc-t-cond')?.textContent||'',
      fecha_objetivo:      tr.querySelector('.pc-t-obj')?.textContent||'',
      cumplido:            cells[6].querySelector('input')?.checked||false,
      milestone_facturacion: cells[8].querySelector('input')?.checked||false,
      pct_facturacion:     parseFloat(cells[9].querySelector('input')?.value)||null,
    };
...
function pcRenderGantt(rows, endDateMap) {
  ...
  // Collect valid date ranges
  const entries = [];
  rows.forEach((tr,i) => {
    const cells = tr.cells;
    const act   = cells[0].querySelector('input')?.value.trim();
    const fIni  = cells[2].querySelector('input')?.value;
    const dias  = parseInt(cells[3].querySelector('input')?.value||0)||0;
    const cumpl = cells[6].querySelector('input')?.checked;
    const mile  = cells[8].querySelector('input')?.checked;
    if(!act || !fIni) return;
    const start = new Date(fIni+'T00:00:00');
    const end   = new Date(start); end.setDate(end.getDate()+dias);
  ...
  const totalDays = Math.max(1, (maxD-minD)/(1000*60*60*24));
  const W = 700;
  const ROW = 22, PAD = 4, LABEL = 160;

  const pct = d => ((d-minD)/(1000*60*60*24)/totalDays*W).toFixed(1);
  const today = new Date(); today.setHours(0,0,0,0);
  ...
  let svgRows = entries.map((e,i)=>{
    ...
    const y  = PAD + i*(ROW+2);
    const color = e.cumpl ? '#48c78e' : (e.end<today ? '#c8102e' : '#f5a623');
    const mileIcon = e.mile ? '★' : '';
    return `
    <rect x="${LABEL+x}" y="${y+3}" width="${w}" height="${ROW-6}" rx="3"
      fill="${color}" opacity=".85"/>
    <text x="${LABEL+x+4}" y="${y+ROW-8}" font-size="9" fill="white" font-family="Arial">
      ${e.dias>0?e.dias+'d':''}</text>
    <text x="${LABEL-4}" y="${y+ROW-8}" font-size="10" fill="#ccc" text-anchor="end"
      font-family="Arial">${esc(e.act.length>22?e.act.slice(0,22)+'…':e.act)} ${mileIcon}</text>`;
  }).join('');

  // Today line
  if(todayX>=0 && todayX<=W){
    svgRows += `<line x1="${LABEL+todayX}" y1="0" x2="${LABEL+todayX}"
      y2="${PAD+(entries.length)*(ROW+2)}" stroke="#c8102e" stroke-width="1.5" stroke-dasharray="4,2"/>
    <text x="${LABEL+todayX+3}" y="10" font-size="8" fill="#c8102e" font-family="Arial">Hoy</text>`;
  }

  const svgH = PAD + entries.length*(ROW+2) + 10;
  gantt.innerHTML = `<svg width="${LABEL+W+10}" height="${svgH}" xmlns="http://www.w3.org/2000/svg"
    style="background:rgba(255,255,255,.02);border-radius:6px">
    <rect width="${LABEL+W+10}" height="${svgH}" fill="none"/>
    ${svgRows}
  </svg>`;
}
...
async function viaSave() {
  ...
  const d = await fetch('/api/viaticos',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({fecha,id_externo:document.getElementById('via-n-id').value.trim(),
      tipo_movimiento:tipo,monto,tipo_cambio:tc,job,notas:document.getElementById('via-n-notas').value.trim()})
  }).then(r=>r.json());
  if(d.error){toast(d.error,'er');return;}
  closeMo('mo-via-new');
...
function viaOpenImport() {
  const sel = document.getElementById('via-imp-job');
  if(sel) sel.innerHTML='<option value="">— Seleccionar Job —</option>'+(jobs||[]).map(j=>`<option value="${esc(j.job_number)}">${esc(j.job_number)}</option>`).join('');
...
async function viaImportFile(file) {
  if(!file) return;
  const job = document.getElementById('via-imp-job').value;
  if(!job){ toast('Selecciona un Job primero','er'); return; }
...
function gvRender() {
  ...
  document.getElementById('gv-tb').innerHTML = rows.map(r=>`
    <tr>
      <td style="color:var(--muted2)">${r.fecha||'—'}</td>
      <td><span style="font-size:10px;background:rgba(255,255,255,.07);padding:2px 8px;border-radius:4px">${esc(r.tipo_gasto||'—')}</span></td>
...
async function gvLoadFX() {
  ...
  const tcEl   = document.getElementById('gv-n-tc');
  if(moneda==='USD'){ tcEl.value='1'; gvCalcUSD(); return; }
  if(!fecha) return;
  try {
    const d = await fetch(`/api/fx/lookup?date=${fecha}`).then(r=>r.json());
    if(d.rate){ tcEl.value=d.rate.toFixed(4); gvCalcUSD(); }
  } catch(e){}
}
...
function gvCalcUSD() {
  ...
  const moneda = document.getElementById('gv-n-moneda').value;
  const tc     = parseFloat(document.getElementById('gv-n-tc').value)||1;
  const usd    = moneda==='MXN' ? costo/tc : costo;
...
async function gvSave() {
  ...
  const d = await fetch('/api/gastos-viaje',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({tipo_gasto:tipo,fecha,moneda,tipo_cambio:tc,costo,job,
...
async function envLoadFX() {
  ...
  const tcEl   = document.getElementById('env-n-tc');
  if(moneda==='USD'){ tcEl.value='1'; envCalcUSD(); return; }
  if(!fecha) return;
  try {
    const d = await fetch(`/api/fx/lookup?date=${fecha}`).then(r=>r.json());
    if(d.rate){ tcEl.value=d.rate.toFixed(4); envCalcUSD(); }
  } catch(e){}
}
...
function envCalcUSD() {
  ...
  const moneda = document.getElementById('env-n-moneda').value;
  const tc     = parseFloat(document.getElementById('env-n-tc').value)||1;
  const usd    = moneda==='MXN' ? costo/tc : costo;
...
async function envSave() {
  ...
  const d = await fetch('/api/envios',{method:'POST',headers:{'Content-Type':'application/json'},
    body:JSON.stringify({fecha,tracking,moneda,tipo_cambio:tc,costo,job,
      notas:document.getElementById('env-n-notas').value.trim()})
  }).then(r=>r.json());
  if(d.error){toast(d.error,'er');return;}
  // Upload POD if attached
