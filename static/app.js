...
function adminSelectUser(uname) {
  ...
  if(pill){ pill.style.background='rgba(200,16,46,.15)'; pill.style.borderColor='var(--red)'; pill.style.color='#fff'; }

  const rows = MODULE_GROUPS.map(grp => {
    const gm = grp.mods.filter(m => modules.includes(m));
    if(!gm.length) return '';
    const hdr = `<tr style="background:rgba(255,255,255,.03)"><td colspan="2"
  ...
  panel.innerHTML = `
    <div style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:18px">
      <div style="display:flex;align-items:center;gap:12px;margin-bottom:16px">
        <div style="width:40px;height:40px;border-radius:50%;background:${isAdmin?'rgba(200,16,46,.2)':'rgba(255,255,255,.08)'};display:flex;align-items:center;justify-content:center;font-size:18px">${isAdmin?'👑':'👤'}</div>
        <div>
...
// ════════════════════════════════════════════════════════
//  SALIDA DE REASIGNACIONES
// ════════════════════════════════════════════════════════
let reasItems = [];

function salidaReasOpenModal() {
  reasItems = [];
  document.getElementById('reas-order-search').value = '';
  document.getElementById('reas-solicitante-display').textContent = USER_PERMS?.user || '—';
  document.getElementById('reas-content').style.display = 'none';
  document.getElementById('reas-empty').style.display = '';
  document.getElementById('reas-empty').textContent = 'Ingresa el número de orden de reasignación para ver los items';
  document.getElementById('btn-reas-save').disabled = true;
  document.getElementById('mo-salida-reas').classList.add('on');
  setTimeout(()=>document.getElementById('reas-order-search').focus(), 100);
}

async function reasCargarItems() {
  const q = document.getElementById('reas-order-search').value.trim();
  if(q.length < 2) return;
  clearTimeout(window._reasTimer);
  window._reasTimer = setTimeout(async () => {
    try {
      const d = await fetch(`/api/reassign?q=${encodeURIComponent(q)}`).then(r=>r.json());
      const orders = (d.records||[]).filter(o=>
        (o.order_number||'').toLowerCase().includes(q.toLowerCase()));
      if(!orders.length){
        document.getElementById('reas-empty').textContent = `No se encontró la orden "${q}"`;
        document.getElementById('reas-empty').style.display = '';
        document.getElementById('reas-content').style.display = 'none';
        document.getElementById('btn-reas-save').disabled = true;
        return;
      }
      // Use first match — aggregate all items
      const allItems = orders.flatMap(o =>
        (o.items||[]).map(it => ({...it, _order: o.order_number}))
      );
      reasItems = allItems.map(it => ({
        part_number:  String(it.part_number||'').trim().toUpperCase(),
        manufacturer: String(it.manufacturer||it.brand||'').trim().toUpperCase(),
        description:  String(it.description||'').trim(),
        job:          String(it.job||'').trim().toUpperCase(),
        unit_cost:    parseFloat(it.unit_cost||0),
        available:    parseFloat(it.quantity||0),
        _order:       it._order,
      })).filter(it => it.available > 0);

      if(!reasItems.length){
        document.getElementById('reas-empty').textContent = 'La orden no tiene items con cantidad disponible';
        document.getElementById('reas-empty').style.display = '';
        document.getElementById('reas-content').style.display = 'none';
        return;
      }
      document.getElementById('reas-empty').style.display = 'none';
      document.getElementById('reas-content').style.display = '';
      reasRenderItems();
    } catch(e){ toast('Error buscando orden: '+e.message,'er'); }
  }, 350);
}

function reasRenderItems() {
  const fmt = v=>'$'+Number(v||0).toFixed(2);
  const inpS='background:var(--inp);border:1px solid rgba(255,193,7,.4);border-radius:4px;color:var(--gold);padding:4px 7px;font-size:12px;text-align:right;font-family:"DM Mono",monospace;font-weight:700';
  document.getElementById('reas-items-tb').innerHTML = reasItems.map((it,i)=>`
    <tr id="reas-row-${i}">
      <td style="text-align:center">
        <input type="checkbox" class="reas-chk" data-idx="${i}"
          onchange="reasUpdateCount()"
          style="accent-color:var(--amber);width:15px;height:15px;cursor:pointer">
      </td>
      <td style="font-family:'DM Mono',monospace;color:var(--gold);font-size:11px">${esc(it.part_number)}</td>
      <td style="font-size:11px">${esc(it.manufacturer||'—')}</td>
      <td style="color:var(--muted2);max-width:200px;overflow:hidden;text-overflow:ellipsis;font-size:11px" title="${esc(it.description)}">${esc(it.description||'—')}</td>
      <td style="font-family:'DM Mono',monospace;font-weight:700;color:var(--amber);font-size:11px">${esc(it.job||'—')}</td>
      <td style="text-align:right;font-weight:700;color:var(--green)">${it.available}</td>
      <td style="text-align:right">
        <input type="number" min="0" max="${it.available}" step="1" value="0"
          id="reas-qty-${i}" oninput="reasUpdateCount()"
          style="${inpS};width:70px">
      </td>
      <td style="text-align:right;font-family:'DM Mono',monospace;font-size:11px">${fmt(it.unit_cost)}</td>
    </tr>`).join('');
  reasUpdateCount();
  document.getElementById('btn-reas-save').disabled = false;
}

function reasFilterItems() {
  const q = (document.getElementById('reas-item-filter')?.value||'').toLowerCase();
  document.querySelectorAll('#reas-items-tb tr').forEach((row,i) => {
    if(!q){ row.style.display=''; return; }
    const it = reasItems[i];
    const match = it && ((it.part_number||'').toLowerCase().includes(q) ||
                         (it.description||'').toLowerCase().includes(q) ||
...
function reasSelectAll(val) {
  document.querySelectorAll('.reas-chk').forEach(cb => {
    const row = document.getElementById(`reas-row-${cb.dataset.idx}`);
    if(row && row.style.display !== 'none') {
      cb.checked = val;
      if(val) {
        const avail = reasItems[cb.dataset.idx]?.available || 0;
        const inp = document.getElementById(`reas-qty-${cb.dataset.idx}`);
        if(inp && parseFloat(inp.value) === 0) inp.value = avail;
      }
    }
  });
  reasUpdateCount();
...
function reasUpdateCount() {
  const sel = [...document.querySelectorAll('.reas-chk:checked')];
  document.getElementById('reas-selected-count').textContent = `${sel.length} item(s) seleccionado(s)`;
}

async function reasGuardar() {
  const solicitante = USER_PERMS?.user || '—';
  const orderNum    = document.getElementById('reas-order-search').value.trim();
  const chks        = [...document.querySelectorAll('.reas-chk:checked')];
  if(!chks.length){ toast('Selecciona al menos un item','er'); return; }

  const salItems = chks.map(cb => {
    const i  = parseInt(cb.dataset.idx);
    const it = reasItems[i];
    const qty = parseFloat(document.getElementById(`reas-qty-${i}`)?.value||0);
    return qty > 0 ? {
      part_number:  it.part_number,
      ...
      quantity:     qty,
      job:          it.job,
      _from_reas:   it._order,
    } : null;
  }).filter(Boolean);

  if(!salItems.length){ toast('Ingresa cantidad mayor a 0 en al menos un item','er'); return; }

  const btn = document.getElementById('btn-reas-save');
  btn.disabled=true; btn.textContent='Registrando…';
  try {
    const job = salItems[0]?.job || '';
    const d = await fetch('/api/salida', {
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({
        job, solicitante, items: salItems,
        tipo: 'reasignacion', reas_order: orderNum,
      })
    }).then(r=>r.json());
    if(d.error){ toast(d.error,'er'); return; }
    closeMo('mo-salida-reas');
    toast(`✓ Salida ${d.record?.id||''} registrada desde Reasignación ${orderNum}`, 'ok', 5000);
    await salidaRender();
  } catch(e){ toast('Error: '+e.message,'er'); }
  finally { btn.disabled=false; btn.textContent='🔀 Registrar Salida'; }
}
