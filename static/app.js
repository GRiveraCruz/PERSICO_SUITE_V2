...
function capLegendRenderRows(){
  ...
  while(padded.length<10) padded.push({codigo:'',descripcion:'',color:''});
...
async function capLegendSave(){
  ...
  const rows=Array.from(tb.children).map(tr=>{
    ...
    return {codigo:inps[0].value.trim().toUpperCase(), descripcion:inps[1].value.trim(), color:inps[2].value};
  });
  try{
...
function capLegendMap(){
  const map={};
  capLegendRows.forEach(r=>{ if(r.codigo && r.color) map[r.codigo.trim().toUpperCase()]=r.color; });
  return map;
}
