...
function rptSwitchTab(tab) {
  rptCurrentTab = tab;
  const isFin = tab === 'fin';
  document.getElementById('rpt-tab-fin').style.background  = isFin  ? 'var(--red)' : 'rgba(255,255,255,.06)';
  document.getElementById('rpt-tab-fin').style.color       = isFin  ? '#fff' : 'var(--muted)';
  document.getElementById('rpt-tab-op').style.background   = !isFin ? 'var(--red)' : 'rgba(255,255,255,.06)';
  document.getElementById('rpt-tab-op').style.color        = !isFin ? '#fff' : 'var(--muted)';
  if(rptData) {
    if(isFin) rptRender(rptData);
    else       rptRenderOperativo(rptData);
  }
}
...
function mrptSwitchTab(tab) {
  mrptCurrentTab = tab;
  const isFin = tab === 'fin';
  document.getElementById('mrpt-tab-fin').style.background  = isFin  ? 'var(--red)' : 'rgba(255,255,255,.06)';
  document.getElementById('mrpt-tab-fin').style.color       = isFin  ? '#fff' : 'var(--muted)';
  document.getElementById('mrpt-tab-op').style.background   = !isFin ? 'var(--red)' : 'rgba(255,255,255,.06)';
  document.getElementById('mrpt-tab-op').style.color        = !isFin ? '#fff' : 'var(--muted)';
  if(typeof mrptData !== 'undefined' && mrptData) {
    if(isFin) mrptRender(mrptData);
    else       mrptRenderOperativo(mrptData);
  }
}

async function rptGetPresupuestoDisponible(jobNumber) {
  try {
    const d = await fetch(`/api/projconfig?q=${encodeURIComponent(jobNumber)}`).then(r=>r.json());
    const cfg = (d.records||[]).find(r => (r.jobs||[]).some(j=>j.job_number===jobNumber));
    if(!cfg) return null;
    const jobCfg = cfg.jobs.find(j=>j.job_number===jobNumber);
    return jobCfg?.presupuesto_disponible ?? null;
  } catch(e){ return null; }
}
...
async function rptRenderOperativo(d) {
  ...
  const presLabel = presDisp !== null ? 'Presupuesto Disponible' : 'Presupuesto Disponible (sin config.)';
  card('rpt-card-rev', presLabel, rptFmtMoney(base),
    presDisp !== null ? d.customer||'—' : '⚠ Configura el proyecto para ver el presupuesto', 'rpt-card-blue');
  card('rpt-card-wh', 'Work Hours Cost', rptFmtMoney(d.amount_wh),
    rptFmtHrs(d.accum_hours)+' acumuladas · '+d.workers.length+' empleado(s)', 'rpt-card-dark');
  card('rpt-card-pur', 'Purchasings Total', rptFmtMoney(d.purchasing_total),
  ...
  card('rpt-card-gm', 'Gross Margin Operativo', rptFmtMoney(grossOp),
  ...
  // Workers and PO tables are the same — call existing rptRender logic for those sections only
  rptRender(d); // renders workers + po tables
  // Re-override the cards (rptRender overwrites them)
  card('rpt-card-rev', presLabel, rptFmtMoney(base),
    presDisp !== null ? d.customer||'—' : '⚠ Configura el proyecto', 'rpt-card-blue');
  card('rpt-card-gm', 'Gross Margin Operativo', rptFmtMoney(grossOp),
    `${gmSign} ${gmPctOp.toFixed(1)}%`, gmClass);
}

async function mrptRenderOperativo(reports) {
  // Load presupuesto disponible for all jobs
  const presMap = {};
  const cfgResp = await fetch('/api/projconfig').then(r=>r.json());
  const allCfgs = cfgResp.records || [];
  allCfgs.forEach(cfg => {
    (cfg.jobs||[]).forEach(j => {
      presMap[j.job_number] = j.presupuesto_disponible || 0;
    });
  });

  const fmt = v => '$'+Number(v||0).toLocaleString('en-US',{minimumFractionDigits:2});
  let totBase=0, totWH=0, totPur=0, totGM=0, totReas=0, totRecov=0;

  const rows = reports.map(r => {
    const base  = presMap[r.job_number] ?? r.revenue;
    ...
    totGM    += grossOp;
    const noPresp = presMap[r.job_number] === undefined;
    return `<tr>
      ...
      <td style="padding:8px 10px;text-align:right;font-family:'DM Mono',monospace;color:${noPresp?'var(--muted)':'var(--gold)'}">
        ${noPresp?'<span title="Sin config.">—</span>':fmt(base)}</td>
  ...
  mcard('mrpt-card-rev', 'Presupuesto Disponible Total', fmt(totBase), reports.length+' jobs', 'rpt-card-blue');
  ...
  mcard('mrpt-card-gm', 'Gross Margin Operativo', fmt(totGMop), `${totGMpct.toFixed(1)}%`, gmClass);
...
function applyPermsToDom(d) {
  ...
  // ── Hide Resultado Financiero tab for non-full report users
  const reportLevel = lvl('report');
  const mrptLevel   = lvl('multirpt');
  if(reportLevel !== 'full') {
    const tf = document.getElementById('rpt-tab-fin');
    if(tf) tf.style.display='none';
    // Auto-switch to operativo
    rptCurrentTab = 'op';
    const to = document.getElementById('rpt-tab-op');
    if(to){ to.style.background='var(--red)'; to.style.color='#fff'; }
  }
  if(mrptLevel !== 'full') {
    const mf = document.getElementById('mrpt-tab-fin');
    if(mf) mf.style.display='none';
    mrptCurrentTab = 'op';
    const mo = document.getElementById('mrpt-tab-op');
    if(mo){ mo.style.background='var(--red)'; mo.style.color='#fff'; }
  }
...
function pcCalc(idx) {
  ...
  const montoAhorro = presA * ahorPct / 100;
  const presB       = presA - montoAhorro;
