...
async function loadSueldos(){
  try{ sueldosMask=await(await fetch('/api/sueldos')).json(); }catch(e){ sueldosMask={}; }
  salSueldosRender();
}
...
function salSueldosRender(){
  ...
  document.getElementById('sal-conf2').textContent=Object.values(sueldosMask).filter(Boolean).length;
