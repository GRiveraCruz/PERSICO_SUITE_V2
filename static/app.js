...
async function loadAreas(){
  try{ areasList=await(await fetch('/api/areas')).json(); }catch(e){ areasList=[]; }
  areaRenderList();
  areaPopulateSelects();
  areaRenderOrgChart();
}
...
let personalRecords=[], personalCurrentRow=null, personalVisibleCount=100, tImpFile=null;

async function loadPersonal(){
  try{
    personalRecords=await(await fetch('/api/personal')).json();
...
async function loadCapacidad(){
  try{
    capacidadData=await(await fetch('/api/capacidad')).json();
...
function capRender(){
  ...
  if(!groups.length){
    wrap.innerHTML='<div class="es"><div class="ei">👥</div><p>No hay trabajadores registrados todavía.<br>Ve a Recursos Humanos → Control de Personal → Listado de Trabajadores.</p></div>';
    return;
  }
...
async function loadCapLegend(){
  try{ capLegendRows=await(await fetch('/api/capacidad/codigos')).json(); }catch(e){ capLegendRows=[]; }
  capLegendRenderRows();
  capApplyColors();
}
