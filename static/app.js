...
function closePanel(){
  ...
  ['j-panel','r-panel','q-panel','t-panel'].forEach(id=>document.getElementById(id).classList.remove('on'));
  _currentPanel=null;
...
function perfilRenderList(){
  ...
  c.innerHTML=perfilesList.map((p,idx)=>`<div style="display:flex;align-items:center;justify-content:space-between;background:var(--card);border:1px solid var(--border);border-radius:var(--r);padding:10px 14px">
...
async function perfilCreate(){
  ...
  try{
    const r=await(await fetch('/api/perfiles',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({nombre})})).json();
    if(r.error){toast(r.error,'er');return;}
    inp.value=''; toast('Perfil agregado ✓','ok'); await loadPerfiles();
  }catch(e){toast('Error: '+e,'er');}
}
async function perfilDelete(idx){
  if(!confirm('¿Eliminar el perfil "'+perfilesList[idx].nombre+'"?'))return;
