...
function capLegendRenderRows(){
  ...
  while(padded.length<10) padded.push({codigo:'',descripcion:'',color:'',colorFondo:''});
  tb.innerHTML=padded.map(row=>`<tr>
    ...
    <td style="padding:4px 8px!important"><input type="color" value="${row.color||'#8a8a8e'}" style="width:40px;height:26px;border:1px solid var(--border);border-radius:4px;background:none;cursor:pointer;padding:0" onchange="capLegendSave()"></td>
    <td style="padding:4px 8px!important"><input type="color" value="${row.colorFondo||'#ffffff'}" style="width:40px;height:26px;border:1px solid var(--border);border-radius:4px;background:none;cursor:pointer;padding:0" onchange="capLegendSave()"></td>
  </tr>`).join('');
  if(!permCanCreate('ops-capacidad')){
    tb.querySelectorAll('input').forEach(inp=>inp.disabled=true);
  }
}
...
async function capLegendSave(){
  ...
  const rows=Array.from(tb.children).map(tr=>{
    ...
    return {codigo:inps[0].value.trim().toUpperCase(), descripcion:inps[1].value.trim(), color:inps[2].value, colorFondo:inps[3].value};
  });
  try{
...
function capLegendMap(){
  const map={};
  capLegendRows.forEach(r=>{ if(r.codigo && (r.color||r.colorFondo)) map[r.codigo.trim().toUpperCase()]={color:r.color||'',fondo:r.colorFondo||''}; });
  return map;
}
...
function capApplyColors(){
  ...
  document.querySelectorAll('#cap-wrap .cap-input').forEach(inp=>{
    ...
    const m=map[v];
    inp.style.color=m?m.color:'';
    inp.style.backgroundColor=m?m.fondo:'';
  });
}

async function capSaveCell(tid,fecha,value,el){
  const codigo=value.trim().toUpperCase();
  if(el){ const m=capLegendMap()[codigo]; el.style.color=m?m.color:''; el.style.backgroundColor=m?m.fondo:''; }
  try{
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-qimp','mo-pt-new','mo-pt-confirm','mo-sv-new','mo-stk-imp','mo-stk-ing','mo-reassign','mo-prov-new','mo-prov-imp','mo-cat-new','mo-cat-imp','mo-cpo-new','mo-cpo-imp','mo-po-imp','mo-wh-imp','mo-ivp-imp','mo-fx-imp','mo-refuse','mo-award','mo-tnew','mo-timp','mo-vac-add','mo-asis-link','mo-pm-new','mo-sal-unlock','mo-ctrl-export','mo-sal-import-excel','mo-sal-export-excel','mo-isr-import','mo-np-periodo','mo-np-generar','mo-np-recibo','mo-cpc-new','mo-os','mo-ta'];
...
function pcApplyProjconfigFieldPerms() {
  ...
  // Amarillo: Markup/Monto Markup/Presupuesto Operativo + Estimados por Área — solo Control Total
