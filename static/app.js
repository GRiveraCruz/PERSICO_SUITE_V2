...
document.addEventListener('DOMContentLoaded',async ()=>{ await loadAreas(); await loadPerfiles(); await loadPersonal(); await loadCapLegend(); await loadCapacidad(); });
...
function clDiasVacacionesLFT(anios){
  ...
  if(anios<=29) return 28;
  return 28 + Math.floor((anios-25)/5)*2;
}
...
function clAntiguedadChange(){
  ...
  document.getElementById('cl-dias-vac').value=clDiasVacacionesLFT(anios);
  clCalcular();
}
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-qimp','mo-pt-new','mo-pt-confirm','mo-sv-new','mo-stk-imp','mo-stk-ing','mo-reassign','mo-prov-new','mo-prov-imp','mo-cat-new','mo-cat-imp','mo-cpo-new','mo-cpo-imp','mo-po-imp','mo-wh-imp','mo-ivp-imp','mo-fx-imp','mo-refuse','mo-award','mo-tnew','mo-timp'];
