...
async function mrptRenderOperativo(reports) {
  ...
  // Update table header to include Servicios column
  const thead = document.querySelector('#mrpt-tb')?.closest('table')?.querySelector('thead tr');
  if(thead && !thead.querySelector('.mrpt-svc-th')) {
    const afterPur = thead.querySelectorAll('th')[6]; // after Purchasings
    if(afterPur) {
      const th = document.createElement('th');
      th.className='mrpt-svc-th';
      th.style.cssText='text-align:right;color:var(--amber);min-width:90px';
      th.textContent='Servicios';
      afterPur.insertAdjacentElement('afterend', th);
    }
  }
