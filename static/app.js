...
async function loadAreas(){
  try{
    const d=await(await fetch('/api/areas')).json();
    areasList=Array.isArray(d)?d:[];
  }catch(e){ areasList=[]; }
  areaRenderList();
  areaPopulateSelects();
  areaRenderOrgChart();
...
async function loadPersonal(){
  try{
    const d=await(await fetch('/api/personal')).json();
    if(!Array.isArray(d)){
      personalRecords=[];
      personalLoadError=(d&&d.error)?d.error:'Sin permiso';
...
function capRender(){
  ...
  if(!groups.length){
    wrap.innerHTML=personalLoadError
      ? `<div class="es"><div class="ei">🔒</div><p>No se pudo cargar el listado de trabajadores (${esc(personalLoadError)}).<br>Puede que tu usuario no tenga acceso a Control de Personal — contacta al administrador.</p></div>`
      : '<div class="es"><div class="ei">👥</div><p>No hay trabajadores registrados todavía.<br>Ve a Recursos Humanos → Control de Personal → Listado de Trabajadores.</p></div>';
    return;
  }
