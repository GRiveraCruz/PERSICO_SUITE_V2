...
async function rptRenderOperativo(d) {
  ...
  const presDisp = await rptGetPresupuestoDisponible(rptCurrentJob);
  ...
  // Then override the 4 summary cards with Operativo values
  function setCard(id, label, val, sub, extraClass=''){
...
const MODULE_GROUPS = [
  ...
  { label: '📄 Documentos de Compra', mods: ['gpo','po','ivp'] },
  { label: '🏬 Almacenes',            mods: ['stock','recovery','reassign'] },
...
async function pcSave() {
  ...
  try {
    const d = await fetch('/api/projconfig',{
      method:'POST', headers:{'Content-Type':'application/json'},
      body: JSON.stringify({ptsv: pcCurrentPTSV, jobs: jobsData})
    }).then(r=>r.json());
    if(d.error){toast(d.error,'er');return;}
