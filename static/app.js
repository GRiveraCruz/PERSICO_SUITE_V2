...
function catRender(tipo) {
  ...
  const rows = catData[tipo].filter(r=>
    [r.brand,r.part_number,r.description,r.code].join(' ').toLowerCase().includes(gs));
...
function catOpenNew(tipo) {
  ...
  ['cn-code','cn-brand','cn-pnum','cn-desc','cn-price'].forEach(id=>document.getElementById(id).value='');
