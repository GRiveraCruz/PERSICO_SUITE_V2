...
async function pcSelectPTSV(item) {
  ...
  try {
    ...
    const existing = (d.records||[]).find(r=>r.ptsv===item.label);
    if(existing) (existing.jobs||[]).forEach(j => { savedRows[j.job_number] = j; });
  } catch(e){}
  ...
  const savedTiming = (existing?.timing) || [];
  pcRenderTiming(savedTiming);
...
function pcRenderTiming(savedRows) {
  ...
  // Build a map of saved data by activity name
  const savedMap = {};
  (savedRows||[]).forEach(r => { if(r.actividad) savedMap[r.actividad] = r; });

  // Always render the 16 default activities first with saved values if available
  const defaultSet = new Set(PC_DEFAULT_ACTIVITIES);
  PC_DEFAULT_ACTIVITIES.forEach(a => {
    pcAddTimingRow(savedMap[a] || {actividad: a});
  });

  // Then add any extra manual activities not in the defaults
  (savedRows||[]).forEach(r => {
...
function pcAddTimingRow(data={}, insertIdx=null) {
  const tb = document.getElementById('pc-timing-body');
  if(!tb) return;
  const idx = insertIdx !== null ? insertIdx : tb.rows.length;
  const actOpts = PC_DEFAULT_ACTIVITIES.map(a=>`<option value="${esc(a)}" ${(data.actividad||'')==a?'selected':''}>${esc(a)}</option>`).join('');
  ...
  tr.innerHTML = `
    ...
    <td id="pc-t-cond-${idx}" style="font-family:'DM Mono',monospace;font-size:10px;color:var(--muted2);text-align:center">—</td>
    <td id="pc-t-obj-${idx}"  style="font-family:'DM Mono',monospace;font-size:10px;color:var(--gold);text-align:center;font-weight:600">—</td>
    <td style="text-align:center">
    ...
    <td id="pc-t-status-${idx}" style="text-align:center;font-size:10px">—</td>
    <td style="text-align:center">
...
function pcUpdateTimingCalcs() {
  ...
  rows.forEach((tr, i) => {
    ...
    const condEl = document.getElementById(`pc-t-cond-${i}`);
    const objEl  = document.getElementById(`pc-t-obj-${i}`);
    const statEl = document.getElementById(`pc-t-status-${i}`);
...
function pcGetTimingData() {
  ...
  return [...tb.rows].map((tr,i) => {
    ...
    return {
      ...
      fecha_condicionada:  document.getElementById(`pc-t-cond-${i}`)?.textContent||'',
      fecha_objetivo:      document.getElementById(`pc-t-obj-${i}`)?.textContent||'',
