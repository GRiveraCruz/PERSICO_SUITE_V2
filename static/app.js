...
// ════════════════════════════════════════════════════════
let osRecords=[], osCurrentId=null, osSvList=[], osSvJobsDisponibles=[];
let osDraft={};

function osBlankDraft(){
  return {
    sv_number:'', jobs:[], cliente:'', ubicacion_planta:'', tipo_relacion:'Propio',
...
function osRender(){
  ...
  if(q) rows=rows.filter(r=>(r.id||'').toLowerCase().includes(q)||(r.sv_number||'').toLowerCase().includes(q)||
    (r.jobs||[]).join(' ').toLowerCase().includes(q)||(r.cliente||'').toLowerCase().includes(q));
  ...
  tb.innerHTML=rows.map(r=>`
    <tr onclick="osOpenEdit('${r.id}')" style="cursor:pointer;${r.estatus==='Cancelado'?'opacity:.55':''}">
      ...
      <td style="font-family:'DM Mono',monospace">${esc(r.sv_number||'—')}</td>
...
function osRenderJobsList(){
  ...
  if(!osSvJobsDisponibles.length){ box.innerHTML='<span style="font-size:11px;color:var(--muted)">Selecciona primero una SV para ver sus jobs.</span>'; return; }
...
async function osOpenNew(){
  ...
  await loadSvListForOS();
  document.getElementById('os-sv').value='';
...
async function osOpenEdit(id){
  ...
  osCurrentId=id;
  osDraft={
    sv_number:r.sv_number||'', jobs:[...(r.jobs||[])], cliente:r.cliente||'', ubicacion_planta:r.ubicacion_planta||'',
...
function osCollectFromForm(){
  ...
  osDraft.sv_number=document.getElementById('os-sv').value;
  if(osCurrentId) osDraft.estatus=document.getElementById('os-estatus').value;
}

async function osSave(){
  osCollectFromForm();
  if(!osDraft.sv_number||!osDraft.jobs.length){ toast('Selecciona una SV y al menos un Job','er'); osSwitchTab('gen'); return; }
