    ...
    return`<tr class="${sc}${r.row===quoteCurrentRow?' sel':''}${r.refused?' refused-row':''}" onclick="quoteOpen(${r.row})">
      ...
      <td onclick="event.stopPropagation()">
        <div style="display:flex;flex-direction:column;gap:4px;padding:4px 0">
          ...
          <input type="text" value="${esc(r.technicalSales||'')}" placeholder="Technical Sales" onclick="event.stopPropagation()" onchange="qSaveInlineField(${r.row},'technicalSales',this.value)" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--text);padding:4px 6px;font-size:11px;outline:none">
          <input type="text" value="${esc(r.keyAccountManager||'')}" placeholder="Key Account Manager" onclick="event.stopPropagation()" onchange="qSaveInlineField(${r.row},'keyAccountManager',this.value)" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--text);padding:4px 6px;font-size:11px;outline:none">
        </div>
      </td>
    </tr>`;
  }).join('');
...
async function qSaveInlineField(row,field,value){
  const r=quotes.find(x=>x.row===row);if(!r)return;
  const prev=r[field];r[field]=value.trim();
  try{
    const res=await apiCall('PUT','/quotes/'+row,{...r,[field]:r[field]});
    if(res.error)throw new Error(res.error);
    toast('Guardado ✓','ok');
  }catch(e){r[field]=prev;toast('Error al guardar: '+e,'er');quoteRender();}
}
