...
// ════════════════════════════════════════════════════════
let ctrlMode='week', ctrlReportRows=[], ctrlReportKey='', ctrlFirmas={};
...
async function ctrlLoadReport(){
  ...
  try{
    ...
    const d=await r.json();
    if(d.error){ toast(d.error,'er'); return; }
    ctrlReportRows=d.rows; ctrlReportKey=d.reportKey; ctrlFirmas=d.firmas||{};
    ...
    if(ctrlMode==='week'){
      ...
      const isAuth=!!(ctrlFirmas.gerencia && ctrlFirmas.rrhh);
...
function clCalcular(){
  ...
  clLastData={diario,mensual,diasAguinaldo,diasVac,primaVacPct,uma,primaRiesgoPct,cvPct,aplicarTope,isnPct,
    ...
    costoMensualTotal,costoAnualTotal,rows};
  ...
  results.innerHTML=`
    ...
    <div class="cl-kpis" style="grid-template-columns:1fr 1fr">
...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-qimp','mo-pt-new','mo-pt-confirm','mo-sv-new','mo-stk-imp','mo-stk-ing','mo-reassign','mo-prov-new','mo-prov-imp','mo-cat-new','mo-cat-imp','mo-cpo-new','mo-cpo-imp','mo-po-imp','mo-wh-imp','mo-ivp-imp','mo-fx-imp','mo-refuse','mo-award','mo-tnew','mo-timp','mo-vac-add','mo-asis-link','mo-pm-new'];
...
function whRender(){
  ...
  const more=rows.length>whVisibleCount?`<tr><td colspan="7" style="text-align:center;padding:12px">
    <span style="font-size:11px;color:var(--muted)">Mostrando ${display.length} de ${rows.length}</span>
...
const MODULE_LABELS = {
  ...
  'rrhh-salario':'Calcular Salario', 'rrhh-nomina':'Nómina',
...
const MODULE_GROUPS = [
  ...
  { label: '🧑‍💼 Recursos Humanos',    mods: ['rrhh-asistencia','rrhh-vacaciones','rrhh-permisos','rrhh-salario','rrhh-nomina','personal-areas','personal-perfiles','personal-listado'] },
