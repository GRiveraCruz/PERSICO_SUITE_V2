...
async function salidaCargarApartados() {
  ...
  try {
    // Load apartados and pending salidas in parallel
    const [dApt, dSal] = await Promise.all([
      fetch(`/api/apartados?job=${encodeURIComponent(job)}`).then(r=>r.json()),
      fetch('/api/salida').then(r=>r.json()),
    ]);

    const all = dApt.records||[];
    salidaAptItems = [];
    all.forEach(r=>{
      ...
      if(jobEntry && jobEntry.quantity>0) {
        salidaAptItems.push({
          ...
          label_code: r.label_code||'', available: jobEntry.quantity,
          unit_cost: jobEntry.unit_cost||0,
        });
      }
    });

    // Pending WOs for this job (not yet surtidas)
    const pendingWOs = (dSal.records||[]).filter(r=>
      r.status==='Pendiente' && (r.job||'').toUpperCase()===job.toUpperCase());

    if(!salidaAptItems.length && !pendingWOs.length){
      document.getElementById('sal-apt-empty').textContent=`No hay materiales apartados ni salidas pendientes para el Job ${job}`;
      document.getElementById('sal-apt-empty').style.display='';
      document.getElementById('sal-apt-content').style.display='none';
      return;
    }
...
function salidaRenderItems(items, pendingWOs=[]) {
  ...
  if(!wo_wrap) return;
  if(!pendingWOs.length){ wo_wrap.style.display='none'; return; }
