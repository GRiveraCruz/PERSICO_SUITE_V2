...
function mrptRender(d){
  ...
  // Also add Servicios header if not present
  const mrptThead = document.querySelector('#mrpt-tb')?.closest('table')?.querySelector('thead tr');
  if(mrptThead && !mrptThead.querySelector('.mrpt-svc-fin-th')) {
    const purTh = [...mrptThead.querySelectorAll('th')].find(th=>th.textContent.trim()==='PURCHASINGS');
    if(purTh) {
      const th=document.createElement('th'); th.className='mrpt-svc-fin-th';
      th.style.cssText='text-align:right;color:var(--amber);min-width:90px';
      th.textContent='SERVICIOS'; purTh.insertAdjacentElement('afterend',th);
    }
  }
  // Footer totals
