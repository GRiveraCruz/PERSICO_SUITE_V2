...
function pcRenderTiming(savedRows) {
  ...
  const rows = savedRows && savedRows.length
    ? savedRows
    : PC_DEFAULT_ACTIVITIES.map(a=>({actividad:a,prev:'',fecha_ini:'',dias:0,cumplido:false,milestone:false,pct_fact:''}));

  rows.forEach((r,i) => pcAddTimingRow(r, i));
  pcUpdateTimingCalcs();
}
