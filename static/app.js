...
function personalRender(){
  ...
  if(!rows.length){
    tb.innerHTML='<tr><td colspan="8"><div class="es"><span class="ei">👥</span><br>Sin registros</div></td></tr>';
    return;
  }
  const display=rows.slice(0,personalVisibleCount);
  const moreRow=rows.length>personalVisibleCount?`<tr><td colspan="8" style="text-align:center;padding:12px">
