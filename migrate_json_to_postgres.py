...
def migrar_por_anio_por_hash(session, rep, nombre_coleccion, carpeta, patron, modelo, extra_cols_fn):
    """Como migrar_por_anio, pero para colecciones sin llave natural (usa
    ContentHashMixin) — ej. purchase_orders, donde 'clave' se puede repetir
    dentro del mismo año cuando el costo de un PO se divide en varias líneas."""
    c = rep.nueva_coleccion(nombre_coleccion)
    for year, f in archivos_por_anio(carpeta, patron):
        data, checksum = leer_json(f, [])
        if not isinstance(data, list):
            ...
            continue
        c["archivos_procesados"].append({"ruta": str(f), "registros": len(data), "checksum": checksum or ""})
        upsert_lista_por_hash(session, modelo, data, lambda r, y=year: dict(extra_cols_fn(r), year=y), c)
...
def ejecutar_migracion(session, rep, solo=None):
    ...
    if incluir("purchase_orders"):
        with _punto_control(session, rep):
            migrar_por_anio_por_hash(session, rep, "purchase_orders", Path(DATA_DIR) / "IPOs", r"^po_(\d{4})\.json$",
                                      db.PurchaseOrder, lambda r: {"po_number": str(r.get("clave") or ""), "job": r.get("entregar_a")})
    ...
    if incluir("ops_capacidad_codigos"):
        with _punto_control(session, rep):
            migrar_lista_simple_por_hash(session, rep, "ops_capacidad_codigos", "capacidad_codigos.json", db.CapacidadCodigo,
                                          lambda r: {"codigo": r.get("codigo") or None})
