...
def ejecutar_migracion(session, rep, solo=None):
    ...
    if incluir("jobs"): migrar_jobs(session, rep)
    ...
    if incluir("fx_rates"): migrar_fx(session, rep)
    ...
    if incluir("apartados"):
        migrar_lista_simple(session, rep, "apartados", "apartados.json", db.Apartado, "part_number", lambda r: {})
    if incluir("salidas"):
