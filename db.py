...
class PurchaseOrder(Base, JSONBMixin, ContentHashMixin):
    """clave/po_number NO es único dentro del mismo año en los datos reales —
    un mismo PO puede aparecer repetido cuando su costo se divide en varias
    líneas (por job, por partida, etc). Se usa huella de contenido."""
...
class RequisicionCompra(Base, JSONBMixin):
    """Un renglón de BOM (Bill of Materials) subido para un Job — eléctrico,
    mecánico, componentes mayores, o manufactura. Se construye directo sobre
    PostgreSQL (módulo nuevo, no requiere migración desde JSON)."""
    __tablename__ = "requisiciones_compra"
    item_id = Column(String, unique=True, index=True)
    job = Column(String, index=True)
    tipo = Column(String, index=True)   # electrico | mecanico | componentes_mayores | manufactura
    status = Column(String, index=True)
...
class CapacidadCodigo(Base, JSONBMixin, ContentHashMixin):
    """Puede haber filas con descripción pero sin 'codigo' todavía (se llenan
    en la interfaz de forma incremental) — se usa huella de contenido en vez
    de asumir que 'codigo' siempre viene lleno y es único."""
...
class Apartado(Base, JSONBMixin, ContentHashMixin):
    """part_number NO es único en los datos reales de producción — hay registros
    duplicados acumulados con el tiempo. Se usa huella de contenido en vez de
    asumir unicidad de negocio que no se sostiene."""
...
_SCHEMA_FIXES = [
    ...
    # Corrección inversa: part_number/clave NO son únicos en los datos reales
    # (se repiten cuando un costo se divide en varias líneas) — se libera la
    # restricción vieja y se usa huella de contenido en su lugar. No hace
    # falta rellenar content_hash aquí: como estas tablas siempre se guardan
    # completas (borra todo + reinserta todo), el próximo guardado ya lo puebla.
                ...
                try:
                    if paso[0] == "rename":
                        ...
                        if not ya_tiene_indice:
                            conn.execute(text(f'CREATE UNIQUE INDEX IF NOT EXISTS "{idx_name}" ON "{tabla}" ("{col}")'))
                            log.append(f"{tabla}: índice único creado en '{col}'")
                    elif paso[0] == "drop_unique_add_hash":
                        _, col_vieja_a_liberar = paso
                        for uc in insp.get_unique_constraints(tabla):
                            if col_vieja_a_liberar in uc["column_names"]:
                                conn.execute(text(f'ALTER TABLE "{tabla}" DROP CONSTRAINT "{uc["name"]}"'))
                                log.append(f"{tabla}: restricción única vieja en '{col_vieja_a_liberar}' liberada")
                        for ix in insp.get_indexes(tabla):
                            if col_vieja_a_liberar in ix["column_names"] and ix.get("unique"):
                                conn.execute(text(f'DROP INDEX IF EXISTS "{ix["name"]}"'))
                                log.append(f"{tabla}: índice único viejo en '{col_vieja_a_liberar}' liberado")
                        if "content_hash" not in cols_actuales:
                            conn.execute(text(f'ALTER TABLE "{tabla}" ADD COLUMN "content_hash" VARCHAR'))
                            log.append(f"{tabla}: columna 'content_hash' agregada (se puebla sola en el próximo guardado)")
                            cols_actuales.add("content_hash")
                        existing_idx = insp.get_indexes(tabla)
                        ya_tiene_indice = any("content_hash" in ix["column_names"] and ix.get("unique") for ix in existing_idx)
                        if not ya_tiene_indice:
